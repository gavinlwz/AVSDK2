...
YouMeErrorCode CYouMeVoiceEngine::startAvSessionManager(bool needMic, bool outputToSpeaker ,bool bJoinRoom)
{
    ...
    if (bRet == true)
    {
        ...
        #if TARGET_OS_IPHONE
        m_iosDeliverMode = Config_GetInt("IOS_REPLAYKIT_PROCESS_MODE", 0);  // 0:同步; 1:异步
        TSK_DEBUG_INFO ("set ios replay kit mode:%u", m_iosDeliverMode);
        startIosVideoThread();
        #endif
#endif
...
{
    TSK_DEBUG_INFO("processIosVideothreadFunc() thread start");
        ...
        if (1 == m_iosDeliverMode) {
            {
                ...
                while (m_videoQueueStarted && m_videoQueue.empty()) {
                    m_videoQueueCond.wait(queueLock);
                }
            ...
            tempWidth = frame->width, tempHeight = frame->height;
            ...
            ret = ICameraManager::getInstance()->videoDataOutputGLESForIOS(frame->data, tempWidth, tempHeight, frame->fmt, frame->rotate, 0, frame->timestamp, 0);
...

            {
                ...
                if (m_videoQueue.empty()) {
                    if (m_refreshTimeout > 0 && m_videoQueueCond.wait_for(queueLock, std::chrono::milliseconds(m_refreshTimeout)) == std::cv_status::timeout) {
...

            {
                ...
                if (m_videoQueueCond.wait_for(queueLock, std::chrono::milliseconds(m_refreshTimeout)) == std::cv_status::timeout) {
                    ...
                    if (m_videoQueueStarted && m_videoQueue.empty()) {
                        continue;
                    }
    ...
    TSK_DEBUG_INFO("processIosVideothreadFunc() thread exits");
}
#endif
...
//        std::string share_uid = "youme_share";
        ...
        if(NULL == renderInfo) {
            ...
            if (userInfo && !userInfo->userId.empty()) {
                ...
                CVideoChannelManager::getInstance()->insertUser(sessionId, userInfo->userId, videoid);
            }else{
            ...
            if(renderInfo->m_isFristVideo){
                ...
                CYouMeVoiceEngine::getInstance()->OnReciveOtherVideoDataOnNotify(userId,  CNgnTalkManager::getInstance()->m_strTalkRoomID, 2, i4Width, i4Height);
            }
        }
    } else {
...
void CYouMeVoiceEngine::videoShareCb(void* handle, char *data, uint32_t len, uint64_t timestamp)
{
    int width = CYouMeVoiceEngine::getInstance()->shareWidth;
    int height = CYouMeVoiceEngine::getInstance()->shareHeight;

    // convert timestamp unit from ns to ms
	uint64_t timestampMs = tsk_gettimeofday_ms();
	
    CYouMeVoiceEngine::getInstance()->inputVideoFrameForShare(data, len, width, height, VIDEO_FMT_YV12, 0, 0, timestampMs);
...
{
    // convert timestamp unit from ns to ms
    //TSK_DEBUG_INFO("== videoShareCbForMac %lld",timestamp);
    timestamp = tsk_gettimeofday_ms();
    CYouMeVoiceEngine::getInstance()->inputVideoFrameForMacShare(data, width, height, 0, 0, 0, timestamp);
...
YouMeErrorCode CYouMeVoiceEngine::startShareStream(int mode, HWND renderHandle, HWND captureHandle)
{
    ...
    TSK_DEBUG_INFO("@@ startShareStream mode:%d", mode);
    if (m_bStartshare != false) {
...
YouMeErrorCode CYouMeVoiceEngine::getWindowInfoList(char *pWinOwner, char *pWinName, int *pWinId, int *winCount)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        memcpy(&pWinOwner[i*256], window_info[i].owner_name, 256);
        memcpy(&pWinName[i*256], window_info[i].window_name, 256);
