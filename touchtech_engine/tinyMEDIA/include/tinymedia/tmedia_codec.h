...
/** Convert time in ms to RTP clock ticks.
    For opus, see rfc7587 rtp-payload-format-for-opus 4.1 RTP Header Usage.
*/
#define TMEDIA_CODEC_RTP_CLOCK_RATE_AUDIO_DECODING(self)  (TMEDIA_CODEC_RATE_DECODING ((self)) * (float)TMEDIA_CODEC_AUDIO ((self))->out.timestamp_multiplier)
#define TMEDIA_CODEC_MS_TO_RTP_TICK_AUDIO_DECODING(self, time_in_ms)  ((time_in_ms) * TMEDIA_CODEC_RTP_CLOCK_RATE_AUDIO_DECODING(self) / 1000)
...
typedef struct tmedia_codec_plugin_def_s
{
    ...
    //! open the codec
    int (*open)(tmedia_codec_t *, const void* context);
    //! close the codec
    int (*close)(tmedia_codec_t *);
    
    //! encode data new, for video
    tsk_size_t (*encode_new)(tmedia_codec_t *, const void *in_data, tsk_size_t in_size, void **out_data, tsk_size_t *out_max_size, int64_t timestamp, int video_id, tsk_bool_t force_key_frame, const void* context);
    //! decode data
    tsk_size_t (*decode_new)(tmedia_codec_t *, const void *in_data, tsk_size_t in_size, void **out_data, tsk_size_t *out_max_size, tsk_object_t *proto_hdr, const void* context);

    //! encode data, for audio
    tsk_size_t (*encode)(tmedia_codec_t *, const void *in_data, tsk_size_t in_size, void **out_data, tsk_size_t *out_max_size);
    //! decode data
    tsk_size_t (*decode)(tmedia_codec_t *, const void *in_data, tsk_size_t in_size, void **out_data, tsk_size_t *out_max_size, tsk_object_t *proto_hdr);
    ...
    //! gets sdp attribute value. e.g. "mode-set=0,2,5,7; mode-change-period=2;
    // mode-change-neighbor=1"
    char *(*sdp_att_get)(const tmedia_codec_t *, const char *att_name);
    
    tsk_bool_t is_codec_hw;
...
TINYMEDIA_API int tmedia_codec_open (tmedia_codec_t *self, const void* context);
...
#define TMEDIA_CODEC_MSRP(self) ((tmedia_codec_msrp_t *)(self))/** BFCP codec */
