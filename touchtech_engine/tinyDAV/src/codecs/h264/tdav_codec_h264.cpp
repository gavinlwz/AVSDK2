...
#define USE_FFMPEG_SOFT_DECODE 1
...
static int tdav_codec_h264_create_dump_file(h264_encoder_manager_t* encoder_mgr, int type, tsk_boolean_t HWEncode);
		...
		const youme::webrtc::RTPFragmentationHeader* fragmentation)
	{
        ...
        if (video_enc_mgr->pfWrite)
        {
            if (video_enc_mgr->file_size > video_enc_mgr->max_size) {
                tdav_codec_h264_create_dump_file(video_enc_mgr, _video_id, true);
            }
...
#if H264_DATA_DUMP
        if (!video_enc_mgr->pfWrite) {
            tdav_codec_h264_create_dump_file(video_enc_mgr, video_id, true);
        }
#endif
        return 0;
	}
...
#if H264_DATA_DUMP
        if (!video_enc_mgr->pfWrite) {
            tdav_codec_h264_create_dump_file(video_enc_mgr, video_id, false);
        }
#endif
		return 0;
	}
...
static int tdav_codec_h264_encode_check_param(tmedia_codec_t* self, h264_encoder_manager_t * video_enc_mgr, const void* context)
{
	...
	if (fps_leveL != video_enc_mgr->fps_level
...
suc:
    ...
    video_enc_mgr->max_size = tmedia_defaults_get_h264_file_size_kb();
    if (video_enc_mgr->max_size > 0)
    {
        tdav_codec_h264_create_dump_file(video_enc_mgr, video_id, HWEncode);
    }
#endif
...
#if H264_DATA_DUMP
int tdav_codec_h264_create_dump_file(h264_encoder_manager_t* encoder_mgr, int type, tsk_boolean_t HWEncode) {
    if (!encoder_mgr) {
        return -1;
    }
