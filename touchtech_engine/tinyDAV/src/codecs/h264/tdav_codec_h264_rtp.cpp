...
int tdav_codec_h264_get_pay(const void* in_data, tsk_size_t in_size, const void** out_data, tsk_size_t *out_size, tsk_bool_t* append_scp, tsk_bool_t* end_of_unit)
{
	...
	*/
	switch ((nal_type = (pdata[0] & 0x1F))) {
		...
		case H264_NAL_UNDEFINE_30:
		case H264_NAL_UNDEFINE_31:
		case H264_NAL_STAP_A:
		case H264_NAL_STAP_B:
		case H264_NAL_MTAP_16:
		case H264_NAL_MTAP_24:
		case H264_NAL_FU_B:
			break;
		case H264_NAL_FU_A:
...
void tdav_codec_h264_rtp_callback(struct tdav_codec_h264_common_s *self, const void *data, tsk_size_t size, tsk_bool_t marker)
{
	...
	if (self->pack_mode_local == Single_NAL_Unit_Mode || size < H264_RTP_PAYLOAD_SIZE) {
		if (self->pack_mode_local == Single_NAL_Unit_Mode && size > H264_RTP_PAYLOAD_SIZE) {
			TSK_DEBUG_WARN("pack_mode=Single_NAL_Unit_Mode but size(%lu) > H264_RTP_PAYLOAD_SIZE(%d). Did you forget to set \"avctx->rtp_payload_size\"?", size, H264_RTP_PAYLOAD_SIZE);
		}
		...
		fua_hdr[0] = pdata[0] & 0x60/* NRI */, fua_hdr[0] |= H264_NAL_FU_A;
