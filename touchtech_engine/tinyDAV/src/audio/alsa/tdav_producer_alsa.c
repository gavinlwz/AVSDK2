...
static void *TSK_STDCALL _tdav_producer_alsa_record_thread (void *param)
{
    ...
    int buf_size = TMEDIA_PRODUCER (p_alsa)->audio.rate * TMEDIA_PRODUCER (p_alsa)->audio.channels * 2 * 20 /1000;
    char *buf = (char *)calloc(buf_size, 1);
    

    ALSA_DEBUG_INFO ("__record_thread -- START: buf size:%d", buf_size);

    tsk_thread_set_priority_2 (TSK_THREAD_PRIORITY_TIME_CRITICAL);

#ifdef OS_LINUX
    prctl(PR_SET_NAME, "producer_alsa_record");
#endif
    while (p_alsa->b_started)
    {
        // tdav_common_alsa_lock (&p_alsa->alsa_common);
        // if ((err = snd_pcm_readi (p_alsa->alsa_common.p_handle, p_alsa->alsa_common.p_buff_ptr,
        //                           p_alsa->alsa_common.n_buff_size_in_samples)) != p_alsa->alsa_common.n_buff_size_in_samples)
        // {
        //     ALSA_DEBUG_ERROR ("Failed to read data from audio interface failed (%d->%s)", err,
        //                       snd_strerror (err));
        ...
        {
            TMEDIA_PRODUCER (p_alsa)->enc_cb.callback (TMEDIA_PRODUCER (p_alsa)->enc_cb.callback_data,
                                                       buf, buf_size);
        }
        // tdav_common_alsa_unlock (&p_alsa->alsa_common);
        //ARMV7板块休眠时间偏差太大，比x86多10ms左右，需要尽快取数据
    #ifdef ARMV7
	    usleep(10000);
    #else
        usleep(20000);
...
static int tdav_producer_alsa_get (tmedia_producer_t *self, tmedia_param_t *param)
{
    // return tdav_producer_audio_get (TDAV_PRODUCER_AUDIO (self), param);
    return 0;
}
...
static int tdav_producer_alsa_prepare (tmedia_producer_t *self, const tmedia_codec_t *codec)
{
    ...
    // Prepare
    err = tdav_common_alsa_prepare (&p_alsa->alsa_common, tsk_true /*is_capture*/,
                                    TMEDIA_PRODUCER (p_alsa)->audio.ptime,
                                    TMEDIA_PRODUCER (p_alsa)->audio.channels,
                                    TMEDIA_PRODUCER (p_alsa)->audio.rate);
    if (err)
    {
        goto bail;
    }
...
/* plugin definition*/
static const tmedia_producer_plugin_def_t tdav_producer_alsa_plugin_def_s = { &tdav_producer_alsa_def_s,

                                                                              tmedia_audio,
                                                                              "Linux ALSA producer",

                                                                              tdav_producer_alsa_set,
                                                                              tdav_producer_alsa_get,
