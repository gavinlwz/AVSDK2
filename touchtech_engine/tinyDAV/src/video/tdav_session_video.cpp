...
#else
#	define TDAV_SESSION_VIDEO_AVPF_FIR_HONOR_INTERVAL_MIN		150 // millis
#endif
...
static int tdav_session_video_rtcp_cb(const void* callback_data, const trtp_rtcp_packet_t* packet)
{
    ...
    if((blocks = (packet->header->type == trtp_rtcp_packet_type_rr) ? ((const trtp_rtcp_report_rr_t*)packet)->blocks :
        ...
        Config_SetInt("video_dn_lossrate", 0);
        ...
        tsk_list_foreach(item, blocks) {
            ...
            if(base->rtp_manager->rtp.ssrc.local == block->ssrc) {
                ...
                video_up_lossrate = (int)(100*block->video_fraction/256.0);
                if (Config_GetInt("video_up_lossRate", 0) < video_up_lossrate) {
                    Config_SetInt("video_up_lossRate", video_up_lossrate );
                }
                ...
                if (Config_GetInt("video_dn_lossrate", 0) < video_dn_lossrate) {
                    Config_SetInt("video_dn_lossrate", video_dn_lossrate );
                }
    ...
    if (audio_up_lossrate_max != video->lastUpAudioLoss_max) {
        ...
        video->lastUpAudioLoss_max = audio_up_lossrate_max;
        Config_SetInt("audio_up_lossRate", audio_up_lossrate_max);
...
static int tdav_session_video_clear(tmedia_session_t* self, int sessionId)
{
    ...
    do {
        ...
        tsk_list_item_t *item = tsk_null;
        tsk_list_lock(video->rscode_list);
        tsk_list_foreach(item, video->rscode_list) {
            if((TDAV_RSCODE(item->data)->session_id == sessionId || abs(TDAV_RSCODE(item->data)->session_id) == sessionId) && (TDAV_RSCODE(item->data)->rs_type == RSC_TYPE_DECODE)) {
