...
/**@ingroup tsk_time_group
 */
uint64_t tsk_gettimeofday_us()
{
    struct timeval tv;
    tsk_gettimeofday(&tv, tsk_null);
    return ( (((uint64_t)tv.tv_sec)*(uint64_t)1000*1000) + ((uint64_t)tv.tv_usec) );
}

/**@ingroup tsk_time_group
 */
uint64_t tsk_timeval_diff_us(struct timeval *tv1, struct timeval *tv2)
{
    return ( (((uint64_t)tv2->tv_sec - 1 - (uint64_t)tv1->tv_sec) * (uint64_t)1000 * 1000)
            + ((uint64_t)tv2->tv_usec + ((uint64_t)1000*1000) - (uint64_t)tv1->tv_usec) );
}
