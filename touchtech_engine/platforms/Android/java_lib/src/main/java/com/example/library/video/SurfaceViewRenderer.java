...
public class SurfaceViewRenderer
        extends SurfaceView implements SurfaceHolder.Callback, VideoBaseRenderer.Callbacks {
    ...
    private int bgColor  = Color.argb(255,0,0,0);
    
    public  enum GLRenderPosition
    {
    	GLRenderPositionALl,
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        updateSurfaceSize();
        logD("SurfaceViewRender:"+this+ "view width:"+(right - left)  + " height:"+(bottom - top));
    }
    ...
    private void updateSurfaceSize() {
        ...
        synchronized (layoutLock) {
            if (enableFixedSize && rotatedFrameWidth != 0 && rotatedFrameHeight != 0 && getWidth() != 0
                ...
                logD("SurfaceViewRender updateSurfaceSize. Layout size: " + getWidth() + "x" + getHeight() + ", frame size: "
    ...
    public void surfaceCreated(final SurfaceHolder holder) {
        ...
        updateSurfaceSize();
        logD("surfaceCreated");
        eglRenderer.setBackgroundColor( Color.red( bgColor ), Color.green( bgColor ), Color.blue( bgColor ), Color.alpha( bgColor ));
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ThreadUtils.checkIsOnMainThread();
        eglRenderer.setRenderSize(width, height);
    ...
    private void updateFrameDimensionsAndReportEvents(VideoBaseRenderer.I420Frame frame) {
        synchronized (layoutLock) {
            if (!isFirstFrameRendered) {
                isFirstFrameRendered = true;
                logD("Reporting first rendered frame.");
                if (rendererEvents != null) {
                    rendererEvents.onFirstFrameRendered(frame.width, frame.height, frame.rotationDegree);
                }
            }
    ...
    public void resetLayout(int width, int height, int rotationDegree) {
    	if (rotatedFrameWidth != width || rotatedFrameHeight != height || frameRotation != rotationDegree) {
            logD("Reporting frame resolution changed to " + width + "x" + height + " with rotation " + rotationDegree);
            if (rendererEvents != null) {
                rendererEvents.onFrameResolutionChanged(width,  height,  rotationDegree);
            }
    ...
    public void SetRenderPosition(SurfaceViewRenderer.GLRenderPosition position){
    	
    	eglRenderer.setRenderPosition(position.ordinal());
    }
    ...
    {
        bgColor = Color.argb( alpha, r, g , b);

        eglRenderer.setBackgroundColor( r, g ,b ,alpha );
    }
    
    private void logD(String string) {
        Log.i(TAG, resourceName +string);
    }
