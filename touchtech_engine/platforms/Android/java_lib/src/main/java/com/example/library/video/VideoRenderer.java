 ...
 *
 */
public class VideoRenderer implements VideoFrameCallbackInterface {
...
//	}
	...
	public void onVideoFrameCallbackGLES(String userId, int type, int texture, float[] matrix, int width, int height, long timestamp){
		SurfaceViewRenderer view = renderers.get(userId);
		if (!mPause && view != null) {
			VideoBaseRenderer.I420Frame frame = new VideoBaseRenderer.I420Frame(width, height, 0, texture, matrix, type == YOUME_VIDEO_FMT.VIDEO_FMT_TEXTURE_OES);
			view.renderFrame(frame);
		}
	}
