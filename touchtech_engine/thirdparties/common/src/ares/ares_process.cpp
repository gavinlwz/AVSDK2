  ...
  for (t = channel->last_timeout_processed; t <= now->tv_sec; t++)
    {
      ...
      for (list_node = list_head->next; list_node != list_head; )
        {
          query = (struct query*)list_node->data;
                           ...
                           struct timeval *now)
{
  ...
  if (channel->flags & ARES_FLAG_EDNS)
  {
      ...
      if (rcode == NOTIMP || rcode == FORMERR || rcode == SERVFAIL)
      {
          ...
          query->tcpbuf = (unsigned char*)realloc(query->tcpbuf, query->tcplen);
  ...
  if (query->using_tcp)
    {
      ...
      sendreq = (struct send_request*)calloc(1, sizeof(struct send_request));
      if (!sendreq)
        {
...
static int open_tcp_socket(ares_channel channel, struct server_state *server)
{
  ...
  switch (server->addr.family)
    {
      case AF_INET:
        sa = (struct sockaddr *)(void *)&saddr.sa4;
        ...
        break;
      case AF_INET6:
        sa = (struct sockaddr *)(void *)&saddr.sa6;
            ...
            if (status == ARES_SUCCESS)
              {
                 ...
                 */
               sendreq->data_storage = (unsigned char *)malloc(sendreq->len);
