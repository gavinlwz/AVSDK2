...
int PushResampler<T>::Resample(const T* src, size_t src_length, T* dst,
                               size_t dst_capacity) {
  CheckExpectedBufferSizes(src_length, dst_capacity, num_channels_, src_sample_rate_hz_, dst_sample_rate_hz_);
  ...
  if (num_channels_ == 2) {
    ...
    T* deinterleaved[] = {src_left_.get(), src_right_.get()};
      ::webrtc::Deinterleave(src, src_length_mono, num_channels_, deinterleaved);
    ...
    ::webrtc::Interleave(deinterleaved, dst_length_mono, num_channels_, dst);
...
template <typename T>
    int PushResampler<T>::GetSrcSampleRateHz() {
        return src_sample_rate_hz_;
    }

template <typename T>
    int PushResampler<T>::GetDstSampleRateHz() {
        return dst_sample_rate_hz_;
    }
