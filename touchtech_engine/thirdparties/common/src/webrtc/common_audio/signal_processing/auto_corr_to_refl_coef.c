...
/*
 * This file contains the function YoumeWebRtcSpl_AutoCorrToReflCoef().
...
void YoumeWebRtcSpl_AutoCorrToReflCoef(const int32_t *R, int use_order, int16_t *K)
{
    int i, n;
    int16_t tmp;
    const int32_t *rptr;
    ...
    *pptr++ = *acfptr++;
    ...
    for (n = 1; n <= use_order; n++, K++)
    {
        ...
        // Division: YoumeWebRtcSpl_div(tmp, *P)
        *K = 0;
        if (tmp != 0)
        {
            L_num = tmp;
        ...
        *pptr = YoumeWebRtcSpl_AddSatW16(*pptr, tmp);
        pptr++;
        for (i = 1; i <= use_order - n; i++)
        {
            tmp = (int16_t)(((int32_t)*wptr * (int32_t)*K + 16384) >> 15);
            *pptr = YoumeWebRtcSpl_AddSatW16(*(pptr + 1), tmp);
            pptr++;
            tmp = (int16_t)(((int32_t)*pptr * (int32_t)*K + 16384) >> 15);
            *wptr = YoumeWebRtcSpl_AddSatW16(*wptr, tmp);
            wptr++;
        }
    }
}
