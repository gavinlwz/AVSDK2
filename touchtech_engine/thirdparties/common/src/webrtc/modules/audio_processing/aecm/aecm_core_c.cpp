...
static void WindowAndFFT(AecmCore* aecm,
  ...
  ::YoumeWebRtcSpl_RealForwardFFT(aecm->real_fft, fft, (int16_t*)freq_signal);
  for (i = 0; i < PART_LEN; i++) {
...
static void InverseFFTAndWindow(AecmCore* aecm,
  ...
  outCFFT = ::YoumeWebRtcSpl_RealInverseFFT(aecm->real_fft, fft, ifft_out);
  for (i = 0; i < PART_LEN; i++) {
...
static int TimeToFrequencyDomain(AecmCore* aecm,
  ...
  tmp16no1 = YoumeWebRtcSpl_MaxAbsValueW16(time_signal, PART_LEN2);
  time_signal_scaling = YoumeWebRtcSpl_NormW16(tmp16no1);
#endif
  ...
  for (i = 1; i < PART_LEN; i++)
  {
    ...
    else
    {
      ...
      tmp32no2 = YoumeWebRtcSpl_AddSatW32(tmp32no1, tmp32no2);
#endif // WEBRTC_ARCH_ARM_V7
      tmp32no1 = YoumeWebRtcSpl_SqrtFloor(tmp32no2);
...
int WebRtcAecm_ProcessBlock(AecmCore* aecm,
  ...
  // Re_Calculate the LOG energy of estimate signal (paul)
  WebRtcAecm_ReCalcSetChEnergies(aecm, far_spectrum_ptr, zerosXBuf);
  ...
  for (i = 0; i < PART_LEN1; i++)
  {
    ...
    zeros32 = YoumeWebRtcSpl_NormW32(aecm->echoFilt[i]) + 1;
    zeros16 = YoumeWebRtcSpl_NormW16(supGain) + 1;
    ...
    zeros16 = YoumeWebRtcSpl_NormW16(aecm->nearFilt[i]);
    ...
    tmp16no2 += tmp16no1;
    zeros16 = YoumeWebRtcSpl_NormW16(tmp16no2);
    ...
    } else
    {
      ...
      tmpU32 = YoumeWebRtcSpl_DivU32U16(echoEst32Gained,
...
static void ComfortNoise(AecmCore* aecm,
  ...
  YoumeWebRtcSpl_RandUArray(randW16, PART_LEN, &aecm->seed);
  ...
  for (i = 0; i < PART_LEN1; i++)
  {
    out[i].real = YoumeWebRtcSpl_AddSatW16(out[i].real, uReal[i]);
    out[i].imag = YoumeWebRtcSpl_AddSatW16(out[i].imag, uImag[i]);
  }
}
