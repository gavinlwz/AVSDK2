...
BlockBuffer::BlockBuffer() {
  buffer_ = WebRtc_youme_CreateBuffer(kBufferSizeBlocks, sizeof(float) * PART_LEN);
  RTC_CHECK(buffer_);
  ReInit();
}

BlockBuffer::~BlockBuffer() {
  WebRtc_youme_FreeBuffer(buffer_);
}

void BlockBuffer::ReInit() {
  WebRtc_youme_InitBuffer(buffer_);
}

void BlockBuffer::Insert(const float block[PART_LEN]) {
  WebRtc_youme_WriteBuffer(buffer_, block, 1);
}
...
void BlockBuffer::ExtractExtendedBlock(float extended_block[PART_LEN2]) {
  ...
  WebRtc_youme_MoveReadPtr(buffer_, -1);
  WebRtc_youme_ReadBuffer(buffer_, reinterpret_cast<void**>(&block_ptr),
                    &extended_block[0], 1);
...
int BlockBuffer::AdjustSize(int buffer_size_decrease) {
  return WebRtc_youme_MoveReadPtr(buffer_, buffer_size_decrease);
}

size_t BlockBuffer::Size() {
  return static_cast<int>(WebRtc_youme_available_read(buffer_));
}

size_t BlockBuffer::AvaliableSpace() {
  return WebRtc_youme_available_write(buffer_);
}
...
static void GenerateComplexNoise(uint32_t* seed, float noise[2][PART_LEN1]) {
  ...
  YoumeWebRtcSpl_RandUArray(randW16, PART_LEN, seed);
...
static void UpdateDelayMetrics(AecCore* self) {
  ...
  int delay_values = 0;
  int median = 0;
  int lookahead = youme::WebRtc_lookahead(self->delay_estimator);
...
static int SignalBasedDelayCorrection(AecCore* self) {
  ...
  last_delay = youme::WebRtc_last_delay(self->delay_estimator);
  //self->data_dumper->DumpRaw("aec_da_reported_delay", 1, &last_delay);
  if ((last_delay >= 0) && (last_delay != self->previous_delay) &&
      (youme::WebRtc_last_delay_quality(self->delay_estimator) >
       self->delay_quality_threshold)) {
    int delay = last_delay - youme::WebRtc_lookahead(self->delay_estimator);
  ...
  if (self->delay_correction_count > 0) {
    float delay_quality = youme::WebRtc_last_delay_quality(self->delay_estimator);
    delay_quality =
...
static void ProcessNearendBlock(
  ...
  if (aec->delay_logging_enabled) {
    if (youme::WebRtc_AddFarSpectrumFloat(aec->delay_estimator_farend,
                                   abs_far_spectrum, PART_LEN1) == 0) {
      int delay_estimate = youme::WebRtc_DelayEstimatorProcessFloat(
...
AecCore* WebRtcAec_CreateAec(int instance_count) {
  ...
  aec->delay_estimator_farend =
      youme::WebRtc_CreateDelayEstimatorFarend(PART_LEN1, kHistorySizeBlocks);
  ...
  aec->delay_estimator = youme::WebRtc_CreateDelayEstimator(
      aec->delay_estimator_farend, kHistorySizeBlocks);
  if (aec->delay_estimator == NULL) {
    WebRtcAec_FreeAec(aec);
    return NULL;
  }
#if 1//def WEBRTC_ANDROID
  ...
  // the lookahead when shifting is required.
    youme::WebRtc_set_lookahead(aec->delay_estimator, 0);
#else
  aec->delay_agnostic_enabled = 0;
  youme::WebRtc_set_lookahead(aec->delay_estimator, kLookaheadBlocks);
#endif
  ...
  WebRtcAec_InitAec_neon(); // [paul] Todo, android open neon, but we not use it in AEC
#endif
  return aec;
}
...
void WebRtcAec_FreeAec(AecCore* aec) {
  ...
  youme::WebRtc_FreeDelayEstimator(aec->delay_estimator);
  youme::WebRtc_FreeDelayEstimatorFarend(aec->delay_estimator_farend);
...
int WebRtcAec_InitAec(AecCore* aec, int sampFreq) {
  ...
  if (youme::WebRtc_InitDelayEstimatorFarend(aec->delay_estimator_farend) != 0) {
    return -1;
  }
  if (youme::WebRtc_InitDelayEstimator(aec->delay_estimator) != 0) {
    return -1;
  }
  ...
  youme::WebRtc_set_allowed_offset(aec->delay_estimator, aec->num_partitions / 2);
  ...
  // this line then.
  youme::WebRtc_enable_robust_validation(aec->delay_estimator, 1);
  aec->frame_count = 0;
...
void WebRtcAec_ProcessFrames(AecCore* aec,
  ...
  for (size_t j = 0; j < num_samples; j += FRAME_LEN) {
    ...
    if (!aec->delay_agnostic_enabled) {
      ...
      youme::WebRtc_SoftResetDelayEstimator(aec->delay_estimator, moved_elements);
      youme::WebRtc_SoftResetDelayEstimatorFarend(aec->delay_estimator_farend,
                                           moved_elements);
...
void WebRtcAec_enable_extended_filter(AecCore* self, int enable) {
  ...
  youme::WebRtc_set_allowed_offset(self->delay_estimator, self->num_partitions / 2);
}
