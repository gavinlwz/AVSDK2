 ...
 */
#include "echo_cancellation_new.h"
...
#include "aec_resampler_new.h"
#include "echo_cancellation_internal_new.h"
#include "webrtc/typedefs.h"

#ifdef DEBUG_RECORD
#include "stdio.h"
#include "stdlib.h"
static FILE * farBufferDebugFile = NULL;
static int iFileOpenFlag = 0;
#if defined(_WIN32)
...
void* WebRtcAec_Create() {
    Aec* aecpc = (Aec*)malloc(sizeof(Aec));
    ...
    aecpc->far_pre_buf =
    WebRtc_youme_CreateBuffer(PART_LEN2 + kResamplerBufferSize, sizeof(float));
    aecpc->media_pre_buf = 
    WebRtc_youme_CreateBuffer(PART_LEN2 + kResamplerBufferSize, sizeof(float));
...
void WebRtcAec_Free(void* aecInst) {
    ...
    WebRtc_youme_FreeBuffer(aecpc->far_pre_buf);
    WebRtc_youme_FreeBuffer(aecpc->media_pre_buf);
...
int32_t WebRtcAec_Init(void* aecInst, int32_t sampFreq, int32_t scSampFreq) {
    ...
    WebRtc_youme_InitBuffer(aecpc->far_pre_buf);
    WebRtc_youme_MoveReadPtr(aecpc->far_pre_buf, -PART_LEN);  // Start overlap.
    WebRtc_youme_InitBuffer(aecpc->media_pre_buf);
    WebRtc_youme_MoveReadPtr(aecpc->media_pre_buf, -PART_LEN);  // Start overlap.
    ...
    aecpc->startup_phase_delay_change = 0;
#ifdef WEBRTC_AEC_TO_PHONE_DEBUG_DEVICE_DELAY
    aecpc->FarBufTooShort = 0;
    aecpc->play_delay_ms = 0;
    aecpc->rec_delay_ms = 0;
    memset(aecpc->debug_tmparr, 0, sizeof(int16_t) * 320 * 200);
    memset(aecpc->debug_tmparr_cp, 0, sizeof(int16_t) * 320 * 200);
    aecpc->debug_tmparr_w_pos = 0;
    aecpc->debug_tmparr_r_pos = 0;
    aecpc->non_causality_cnt = 0;
#endif
    // Default settings.
...
int32_t WebRtcAec_BufferFarend(void* aecInst,
    ...
    WebRtc_youme_WriteBuffer(aecpc->far_pre_buf, farend_ptr, newNrOfSamples);

    // Transform to frequency domain if we have enough data.
    while (WebRtc_youme_available_read(aecpc->far_pre_buf) >= PART_LEN2) {
        // We have enough data to pass to the FFT, hence read PART_LEN2 samples.
        {
            float* ptmp = NULL;
            float tmp[PART_LEN2];
            WebRtc_youme_ReadBuffer(aecpc->far_pre_buf, (void**)&ptmp, tmp, PART_LEN2);
            WebRtcAec_BufferFarendPartition(aecpc->aec, ptmp);
            WebRtc_youme_WriteBuffer(
                (RingBuffer*)WebRtcAec_far_time_buf(aecpc->aec), &ptmp[PART_LEN], 1);
        }

        // Rewind |far_pre_buf| PART_LEN samples for overlap before continuing.
        WebRtc_youme_MoveReadPtr(aecpc->far_pre_buf, -PART_LEN);
    }
...
int32_t WebRtcAec_Process(void* aecInst,
    ...
    msInSndCardBuf = play_delay_ms + rec_delay_ms;
    if (msInSndCardBuf < 0) {
        msInSndCardBuf = 0;
...
int WebRtcAec_get_media_echo_delay(void* handle, int* media_delay) {
    Aec* self = (Aec*)handle;
    int echo_path_delay = 0;
    ...
    echo_path_delay = WebRtcAec_media_echo_delay(self->aec);
    echo_path_delay = echo_path_delay + self->startup_phase_delay_change;

    *media_delay = (echo_path_delay < 0) ? 0 : echo_path_delay;
    return 0;
}
...
static int ProcessNormal(Aec* aecpc,
    ...
    int msInSndCardBuf = play_delay_ms + rec_delay_ms;
    ...
    aecpc->play_delay_ms = play_delay_ms;
    aecpc->rec_delay_ms = rec_delay_ms;
    ...
    if (aecpc->startup_phase) {
        ...
        if (!aecpc->checkBuffSize) {
            ...
            aecpc->startup_phase_delay_change = aecpc->checkBufSizeCtr * (int)nBlocks10ms * 10
                                        - overhead_elements * 8 / aecpc->rate_factor;
        }
    } else {
        // AEC is enabled.
...
static void ProcessExtended(Aec* self,
    ...
    int16_t reported_delay_ms = play_delay_ms + rec_delay_ms;
...
#else  //#ifdef WEBRTC_AEC_TO_PHONE_DEBUG_DEVICE_DELAY
...
#if defined(ANDROID) && defined(WEBRTC_AEC_TO_PHONE_DEBUG_DEVICE_DELAY)
static void EstBufDelayNormal(Aec* aecpc) {
    //int nSampInPlayBuf = (aecpc->play_delay_ms + 20 + 10) * sampMsNb * aecpc->rate_factor;
    int nSampInPlayBuf = (aecpc->play_delay_ms + aecpc->rec_delay_ms + 10) * sampMsNb * aecpc->rate_factor;
    int sys_delay = WebRtcAec_system_delay(aecpc->aec);
    int accelerate_flag = 0;
#if 0
    int playoutDelta = WebRtcAec_FarPlayOutDelta(aecpc->aec);
    int current_delay2 = sys_delay - playoutDelta;
    if (current_delay2 < PART_LEN)
    {
        aecpc->FarBufTooShort += PART_LEN;
    }
    else
    {
        aecpc->FarBufTooShort = 0;
    }
    int delta_tmp = playoutDelta - PART_LEN*2;
    delta_tmp = delta_tmp < PART_LEN ? PART_LEN : delta_tmp;
    if (aecpc->FarBufTooShort > delta_tmp) {
        // We don't have enough data so we rewind PART_LEN sample.
        current_delay2 += WebRtcAec_MoveFarReadPtr(aecpc->aec, -1) * PART_LEN;
    }
#endif
    int current_delay = nSampInPlayBuf - WebRtcAec_system_delay(aecpc->aec);
    ...
    if (current_delay < PART_LEN)
    {
        if (aecpc->non_causality_cnt < 15)
        {
            aecpc->non_causality_cnt++;
        }
        if (aecpc->non_causality_cnt > 10)
        {
            current_delay += WebRtcAec_MoveFarReadPtr(aecpc->aec, 1) * PART_LEN;
            accelerate_flag = 200;
        }
...
#ifdef DEBUG_RECORD    
    if(farBufferDebugFile)
    {
        int16_t debug_tmparr[960];
        int i; 
        int16_t cp_num = sampMsNb * aecpc->rate_factor * 20;
        cp_num = cp_num > 960 ? 960 : cp_num;
        for (i = 0; i < cp_num; i+=4)
        {
            debug_tmparr[i] = (int16_t)aecpc->play_delay_ms;
            debug_tmparr[i+1] = (int16_t)sys_delay;
            debug_tmparr[i+2] = (int16_t)aecpc->rec_delay_ms;
            debug_tmparr[i+3] = (int16_t)nSampInPlayBuf;
            //debug_tmparr[i] = (int16_t)aecpc->play_delay_ms;
            //debug_tmparr[i+1] = (int16_t)sys_delay;
            //debug_tmparr[i +1] = (int16_t)aecpc->rec_delay_ms;
            //debug_tmparr[i+1] = (int16_t)accelerate_flag;
            //debug_tmparr[i] = (int16_t)aecpc->non_causality_cnt;
            //debug_tmparr[i+1] = (int16_t)current_delay;
            //debug_tmparr[i] = (int16_t)nSampInPlayBuf;
        }
        memcpy(aecpc->debug_tmparr + cp_num * aecpc->debug_tmparr_w_pos, debug_tmparr, sizeof(debug_tmparr));
        //(void)fwrite(debug_tmparr, sizeof(int16_t), cp_num, farBufferDebugFile);
    }
#endif

#ifdef DEBUG_RECORD
    aecpc->debug_tmparr_w_pos++;
    if (farBufferDebugFile)
    {
        aecpc->debug_tmparr_w_pos = 
        aecpc->debug_tmparr_w_pos > (200-1) ? 0 : aecpc->debug_tmparr_w_pos;
    }
    else
    {
        aecpc->debug_tmparr_w_pos = 0;
...
#ifdef DEBUG_RECORD
void WebRtcAec_start_recording(void* aecInst, char * file)
{
    Aec* aecpc = (Aec*)aecInst;
    char path[512];
    if (0 == iFileOpenFlag)
    {
        if(farBufferDebugFile)
        {
            fclose(farBufferDebugFile);farBufferDebugFile = NULL;
        }
        snprintf(path, 512,"%s_r2_farBufferDebugFile.pcm", file);
        farBufferDebugFile = fopen(path, "wb+");
        iFileOpenFlag = 1;
    }
    WebRtcAec_start_recording_c(aecpc->aec, file);
}

void WebRtcAec_RecordingToFile(void* aecInst, int16_t record_flag)
{
    Aec* aecpc = (Aec*)aecInst;
    int16_t tmp_read_pos = aecpc->debug_tmparr_r_pos;
    aecpc->debug_tmparr_r_pos = aecpc->debug_tmparr_w_pos;

    int16_t sample_num = aecpc->splitSampFreq/50;

    if (iFileOpenFlag)
    {
        int tmp_read_offset = tmp_read_pos * sample_num;
        int tmp_read_num = (aecpc->debug_tmparr_r_pos - tmp_read_pos) * sample_num;
        if (aecpc->debug_tmparr_r_pos < tmp_read_pos)
        {
            tmp_read_num = (200 - tmp_read_pos) * sample_num;
            memcpy(aecpc->debug_tmparr_cp, aecpc->debug_tmparr + tmp_read_offset, tmp_read_num * sizeof(int16_t));

            int tmp_read_num2 = aecpc->debug_tmparr_r_pos * sample_num;
            memcpy(aecpc->debug_tmparr_cp + tmp_read_num, aecpc->debug_tmparr, tmp_read_num2 * sizeof(int16_t));

            tmp_read_num = tmp_read_num + tmp_read_num2;
        }
        else
        {
            memcpy(aecpc->debug_tmparr_cp, aecpc->debug_tmparr + tmp_read_offset, tmp_read_num  * sizeof(int16_t));
        ...
        if (farBufferDebugFile && tmp_read_num)
        {
            (void)fwrite(aecpc->debug_tmparr_cp, sizeof(int16_t), tmp_read_num, farBufferDebugFile);
        }
    }

    WebRtcAec_RecordingToFile_c(aecpc->aec, record_flag);
}

void WebrtcAec_stop_recording(void* aecInst)
{
    Aec* aecpc = (Aec*)aecInst;
    if(farBufferDebugFile)
    {
    ...
    iFileOpenFlag = 0;
    WebRtcAec_stop_recording_c(aecpc->aec);
}
#endif


// only buffer L band for farend
int32_t EstEchoPathDelay_BufferFarend(void* aecInst,
                               const float* farend,
                               size_t nrOfSamples) {
    Aec* aecpc = (Aec*)aecInst;
    size_t newNrOfSamples = nrOfSamples;
    ...
    // Write the time-domain data to |media_pre_buf|.
    WebRtc_youme_WriteBuffer(aecpc->media_pre_buf, farend_ptr, newNrOfSamples);

    // Transform to frequency domain if we have enough data.
    while (WebRtc_youme_available_read(aecpc->media_pre_buf) >= PART_LEN2) {
        // We have enough data to pass to the FFT, hence read PART_LEN2 samples.
        {
            float* ptmp = NULL;
            float tmp[PART_LEN2];
            WebRtc_youme_ReadBuffer(aecpc->media_pre_buf, (void**)&ptmp, tmp, PART_LEN2);
            WebRtcAec_BufferFarendPartition_DelayEst(aecpc->aec, ptmp);
            WebRtc_youme_WriteBuffer(
                (RingBuffer*)WebRtcAec_media_time_buf(aecpc->aec), &ptmp[PART_LEN], 1);
        }

        // Rewind |media_pre_buf| PART_LEN samples for overlap before continuing.
        WebRtc_youme_MoveReadPtr(aecpc->media_pre_buf, -PART_LEN);
    }
