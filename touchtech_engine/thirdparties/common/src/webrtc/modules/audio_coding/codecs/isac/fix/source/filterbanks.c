...
void WebRtcIsacfix_AllpassFilter2FixDec16C(
  ...
  for (n = 0; n < length; n++) {
    ...
    a <<= 1;  // Q15 -> Q16
    b = YoumeWebRtcSpl_AddSatW32(a, state0_ch1);
    a = -factor_ch1[0] * (int16_t)(b >> 16);  // Q15
    state0_ch1 = YoumeWebRtcSpl_AddSatW32(a << 1, (uint32_t)in_out << 16);  // Q16
    in_out = (int16_t) (b >> 16);  // Save as Q0

    a = factor_ch1[1] * in_out;  // Q15 * Q0 = Q15
    a <<= 1; // Q15 -> Q16
    b = YoumeWebRtcSpl_AddSatW32(a, state1_ch1);  // Q16
    a = -factor_ch1[1] * (int16_t)(b >> 16);  // Q15
    state1_ch1 = YoumeWebRtcSpl_AddSatW32(a << 1, (uint32_t)in_out << 16);  // Q16
    ...
    a <<= 1;  // Q15 -> Q16
    b = YoumeWebRtcSpl_AddSatW32(a, state0_ch2);  // Q16
    a = -factor_ch2[0] * (int16_t)(b >> 16);  // Q15
    state0_ch2 = YoumeWebRtcSpl_AddSatW32(a << 1, (uint32_t)in_out << 16);  // Q16
    in_out = (int16_t) (b >> 16);  // Save as Q0

    a = factor_ch2[1] * in_out;  // Q15 * Q0 = Q15
    a <<= 1;  // Q15 -> Q16
    b = YoumeWebRtcSpl_AddSatW32(a, state1_ch2);  // Q16
    a = -factor_ch2[1] * (int16_t)(b >> 16);  // Q15
    state1_ch2 = YoumeWebRtcSpl_AddSatW32(a << 1, (uint32_t)in_out << 16);  // Q16
                                           ...
                                           int32_t *state)
{
  ...
  for (k=0; k<len; k++) {
    ...
    c = in + ((a1 + b1) >> 7);  // Q0.
    io[k] = (int16_t)YoumeWebRtcSpl_SatW32ToW16(c);  // Write output as Q0.
                                   ...
                                   PreFiltBankstr *prefiltdata)
{
  ...
  for (k=0; k<FRAMESAMPLES/2 + QLOOKAHEAD; k++) {
    ...
    LP16[k] = (int16_t)YoumeWebRtcSpl_SatW32ToW16(tmp3); /*low pass */
    tmp3 = (tmp1 - tmp2) >> 1;  /* High pass signal. */
    HP16[k] = (int16_t)YoumeWebRtcSpl_SatW32ToW16(tmp3); /*high pass */
  }
