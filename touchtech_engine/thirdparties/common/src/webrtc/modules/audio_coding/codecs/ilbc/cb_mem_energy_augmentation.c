...
void WebRtcIlbcfix_CbMemEnergyAugmentation(
  ...
  nrjRecursive = YoumeWebRtcSpl_DotProductWithScale( CBmemPtr-19, CBmemPtr-19, 15, scale);
  ppe = CBmemPtr - 20;
  ...
  for (lagcount=20; lagcount<=39; lagcount++) {
    ...
    /* interpolation */
    energy += YoumeWebRtcSpl_DotProductWithScale(interpSamplesPtr, interpSamplesPtr, 4, scale);
    interpSamplesPtr += 4;

    /* Compute energy for the remaining samples */
    pp = CBmemPtr - lagcount;
    energy += YoumeWebRtcSpl_DotProductWithScale(pp, pp, SUBL-lagcount, scale);

    /* Normalize the energy and store the number of shifts */
    (*enShPtr) = (int16_t)YoumeWebRtcSpl_NormW32(energy);
