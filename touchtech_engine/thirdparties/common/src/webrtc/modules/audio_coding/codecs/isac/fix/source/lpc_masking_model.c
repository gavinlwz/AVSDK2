                        ...
                        )
{
  ...
  for (m=useOrder-1; m>0; m--) {
    ...
    for (k=1; k<=m; k++) {
      ...
      tmp32[k] = YoumeWebRtcSpl_DivW32W16(tmp32b, tmp_inv_denum16); //Q27/Q15 = Q12
    }
...
int16_t YoumeWebRtcSpl_LevinsonW32_JSK(
  ...
  norm = YoumeWebRtcSpl_NormW32(R[0]);
  ...
  temp1W32  = YoumeWebRtcSpl_DivW32HiLow(temp3W32, R_hi[0], R_low[0]); /* abs(R[1])/R[0] in Q31 */
  ...
  Alpha_exp = YoumeWebRtcSpl_NormW32(temp1W32);
  temp1W32 <<= Alpha_exp;
  ...
  for (i=2; i<=order; i++)
  {
    ...
    */
    ...
    temp3W32 = YoumeWebRtcSpl_DivW32HiLow(temp2W32, Alpha_hi, Alpha_low); /* abs(temp1W32)/Alpha */
    ...
    norm = YoumeWebRtcSpl_NormW32(temp3W32);
    ...
    */
    ...
    norm = YoumeWebRtcSpl_NormW32(temp1W32);
    temp1W32 <<= norm;
...
static __inline int32_t log2_Q8_LPC( uint32_t x ) {
  ...
  zeros=YoumeWebRtcSpl_NormU32(x);
                           ...
                           uint32_t *oldEnergy, int16_t *varscale)
{
  ...
  divVal = YoumeWebRtcSpl_DivW32W16ResW16(expPg32, chngQ); /* Q22/Q12=Q10 */
...
int32_t WebRtcIsacfix_CalculateResidualEnergyC(int lpc_order,
  ...
  if(word32_high != 0) {
    shift_norm = 32 - YoumeWebRtcSpl_NormW32(word32_high);
    ...
    if((word32_low & 0x80000000) != 0) {
      ...
      shift_norm = YoumeWebRtcSpl_NormW32(word32_low);
  ...
  */
  ...
  for (k = 0; k < SUBFRAMES; k++) {
    ...
    /* The highest absolute value measured inside DataLo in the test set
       ...
       to YoumeWebRtcSpl_AutoCorrelation() as Q6 values (since 307*2^6 =
    ...
    */
    ...
    sh_lo = YoumeWebRtcSpl_NormW32(corrloQQ[0]);
    QdomLO += sh_lo;
    ...
    sh_hi = YoumeWebRtcSpl_NormW32(corrhiQQ[0]);
    QdomHI += sh_hi;
    ...
    for (n = 0; n <= ORDERLO; n++) {
      ...
      if (maskdata->CorrBufLoQQ[n] != 0) {
        shMem=YoumeWebRtcSpl_NormW32(maskdata->CorrBufLoQQ[n]);
    ...
    for (n = 0; n <= ORDERHI; n++) {
      ...
      if (maskdata->CorrBufHiQQ[n] != 0) {
        shMem=YoumeWebRtcSpl_NormW32(maskdata->CorrBufHiQQ[n]);
    ...
    stab=YoumeWebRtcSpl_LevinsonW32_JSK(corrlo2QQ, a_LOQ11, k_vecloQ15, ORDERLO);
    ...
    if (stab<0) {  // If unstable use lower order
      ...
      stab=YoumeWebRtcSpl_LevinsonW32_JSK(corrlo2QQ, a_LOQ11, k_vecloQ15, 8);
    }


    YoumeWebRtcSpl_LevinsonDurbin(corrhiQQ,  a_HIQ12,  k_vechiQ15, ORDERHI);
    ...
    sh = YoumeWebRtcSpl_NormW32(corrlo2QQ[0]);
    for (n = 0; n <= ORDERLO; n++) {
      corrlo2QQ[n] <<= sh;
    }
    ...
    YoumeWebRtcSpl_AToK_JSK(a_LOQ11, ORDERLO, rcQ15_lo);
    ...
    if( res_nrgQQ > 0 )
    {
      sqrt_nrg=YoumeWebRtcSpl_Sqrt(res_nrgQQ);
      ...
      sh = YoumeWebRtcSpl_NormW32(tmp32c);
      shft = 16 - sh;
      tmp16a = (int16_t) WEBRTC_SPL_SHIFT_W32(tmp32c, -shft); // Q(ssh-shft)  (denominator)

      tmp32b = YoumeWebRtcSpl_DivW32W16(tmp32a, tmp16a); // Q(24-ssh+shft)
      sh = ssh-shft-7;
    ...
    /* Convert to reflection coefficients */
    YoumeWebRtcSpl_LpcToReflCoef(polyHI, ORDERHI, rcQ15_hi);
