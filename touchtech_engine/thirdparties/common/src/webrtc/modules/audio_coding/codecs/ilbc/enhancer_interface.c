...
size_t WebRtcIlbcfix_EnhancerInterface( /* (o) Estimated lag in end of in[] */
  ...
  for(iblock = 0; iblock<new_blocks; iblock++){
    ...
    /* scaling */
    max16 = YoumeWebRtcSpl_MaxAbsValueW16(&regressor[-50], ENH_BLOCKL_HALF + 50 - 1);
    shifts = YoumeWebRtcSpl_GetSizeInBits((uint32_t)(max16 * max16)) - 25;
    shifts = WEBRTC_SPL_MAX(0, shifts);

    /* compute cross correlation */
    YoumeWebRtcSpl_CrossCorrelation(corr32, target, regressor, ENH_BLOCKL_HALF, 50,
                               shifts, -1);
    ...
    for (i=0;i<2;i++) {
      lagmax[i] = YoumeWebRtcSpl_MaxIndexW32(corr32, 50);
    ...
    lagmax[2] = YoumeWebRtcSpl_MaxIndexW32(corr32, 50);
    corrmax[2] = corr32[lagmax[2]];

    /* Calculate normalized corr^2 and ener */
    for (i=0;i<3;i++) {
      corrSh = 15-YoumeWebRtcSpl_GetSizeInBits(corrmax[i]);
      ener = YoumeWebRtcSpl_DotProductWithScale(regressor - lagmax[i],
                                           regressor - lagmax[i],
                                           ENH_BLOCKL_HALF, shifts);
      enerSh = 15-YoumeWebRtcSpl_GetSizeInBits(ener);
  ...
  if ((iLBCdec_inst->prev_enh_pl==1)||(iLBCdec_inst->prev_enh_pl==2)) {
    ...
    */
    ...
    /* scaling */
    max16 = YoumeWebRtcSpl_MaxAbsValueW16(regressor, plc_blockl + 3 - 1);
    if (max16>5000)
      shifts=2;
    else
      shifts=0;
    ...
    /* compute cross correlation */
    YoumeWebRtcSpl_CrossCorrelation(corr32, target, regressor, plc_blockl, 3, shifts,
                               1);

    /* find lag */
    lag=YoumeWebRtcSpl_MaxIndexW32(corr32, 3);
    lag+=tlag-1;
    ...
    if (iLBCdec_inst->prev_enh_pl==1) {
      ...
      */
      ...
      max=YoumeWebRtcSpl_MaxAbsValueW16(
          &enh_buf[ENH_BUFL-iLBCdec_inst->blockl-plc_blockl], plc_blockl);
      max16=YoumeWebRtcSpl_MaxAbsValueW16(plc_pred, plc_blockl);
      max = WEBRTC_SPL_MAX(max, max16);
      scale=22-(int16_t)YoumeWebRtcSpl_NormW32(max);
      scale=WEBRTC_SPL_MAX(scale,0);

      tmp2 = YoumeWebRtcSpl_DotProductWithScale(
          &enh_buf[ENH_BUFL-iLBCdec_inst->blockl-plc_blockl],
          &enh_buf[ENH_BUFL-iLBCdec_inst->blockl-plc_blockl],
          plc_blockl, scale);
      tmp1 = YoumeWebRtcSpl_DotProductWithScale(plc_pred, plc_pred,
                                           plc_blockl, scale);
      ...
      if ((tmp1>0)&&((tmp1>>2)>tmp2)) {
        ...
        */
        ...
        scale1=(int16_t)YoumeWebRtcSpl_NormW32(tmp1);
        tmp1=WEBRTC_SPL_SHIFT_W32(tmp1, (scale1-16)); /* using 15 bits */

        tmp2=WEBRTC_SPL_SHIFT_W32(tmp2, (scale1));
        EnChange = (int16_t)YoumeWebRtcSpl_DivW32W16(tmp2,
                                                      (int16_t)tmp1);

        /* Calculate the Sqrt of the energy in Q15 ((14+16)/2) */
        SqrtEnChange = (int16_t)YoumeWebRtcSpl_SqrtFloor(EnChange << 14);


        /* Multiply first part of vector with 2*SqrtEnChange */
        YoumeWebRtcSpl_ScaleVector(plc_pred, plc_pred, SqrtEnChange, plc_blockl-16,
                              14);
      ...
      */
      ...
      YoumeWebRtcSpl_MemSetW16(iLBCdec_inst->syntMem, 0, LPC_FILTERORDER);
      YoumeWebRtcSpl_MemSetW16(iLBCdec_inst->hpimemy, 0, 4);
      YoumeWebRtcSpl_MemSetW16(iLBCdec_inst->hpimemx, 0, 2);

      /* Initialize filter memory by filtering through 2 lags */
      WEBRTC_SPL_MEMCPY_W16(&synt[-LPC_FILTERORDER], iLBCdec_inst->syntMem,
                            LPC_FILTERORDER);
      YoumeWebRtcSpl_FilterARFastQ12(
          enh_bufPtr1,
          synt,
