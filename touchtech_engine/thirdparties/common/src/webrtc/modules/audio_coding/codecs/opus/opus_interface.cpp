...
int16_t WebRtcOpus_EncoderCreate(OpusEncInst** inst,
  ...
  int error;
  state->encoder = opus_encoder_create(fs_hz, channels, opus_app,
                                       &error);
...
int16_t WebRtcOpus_DecoderCreate(OpusDecInst** inst, int channels, int32_t fs_hz) {
  int error;
  OpusDecInst* state;
  ...
  if (inst != NULL) {
    ...
    /* Create new memory, the original WebRtc code set the sampling rate always at 48000 Hz.
       We modify it to be configurable. */
    state->decoder = opus_decoder_create(fs_hz, channels, &error);
    if (error == OPUS_OK && state->decoder != NULL) {
      /* Creation of memory all ok. */
      state->channels = channels;
      state->prev_decoded_samples = fs_hz / 50; // 20ms by default
      state->in_dtx_mode = 0;
      *inst = state;
      return 0;
    }
...
int WebRtcOpus_DecodeFec(OpusDecInst* inst, const uint8_t* encoded,
  ...
  fec_samples = opus_packet_get_samples_per_frame(encoded, fs_hz);
...
int WebRtcOpus_DurationEst(OpusDecInst* inst,
  ...
  samples = frames * opus_packet_get_samples_per_frame(payload, fs_hz);
  if (samples < (fs_hz / 400) || samples > (fs_hz * 120 / 100)) {
...
int WebRtcOpus_FecDurationEst(const uint8_t* payload,
  ...
  samples = opus_packet_get_samples_per_frame(payload, fs_hz);
  // peter TODO: check the opus spec for calculation of the  mininum sample number for FEC
  // if (samples < 160 || samples > 5760) {
  if (samples < (fs_hz / 400) || samples > (fs_hz * 120 / 100)) {
    /* Invalid payload duration. */
    return 0;
  }
