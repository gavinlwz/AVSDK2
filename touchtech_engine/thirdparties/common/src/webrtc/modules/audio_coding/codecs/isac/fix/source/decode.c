                             ...
                             size_t* current_framesamples)
{
  ...
  for (frame_nb = 0; frame_nb <= frame_mode; frame_nb++) {
    ...
    if( (ISACdec_obj->plcstr_obj).used == PLC_WAS_USED )
    {
      ...
      /* ---- Add-overlap ---- */
      YoumeWebRtcSpl_GetHanningWindow( overlapWin, RECOVERY_OVERLAP );
      for( k = 0; k < RECOVERY_OVERLAP; k++ )
        Vector_Word16_1[k] = YoumeWebRtcSpl_AddSatW16(
    ...
    for (k=0;k<FRAMESAMPLES/2;k++) {
      tmp_1 = (int16_t)YoumeWebRtcSpl_SatW32ToW16(((int32_t)Vector_Word16_1[k]+Vector_Word16_2[k] + 1)); /* Construct a new upper channel signal*/
      tmp_2 = (int16_t)YoumeWebRtcSpl_SatW32ToW16(((int32_t)Vector_Word16_1[k]-Vector_Word16_2[k])); /* Construct a new lower channel signal*/
