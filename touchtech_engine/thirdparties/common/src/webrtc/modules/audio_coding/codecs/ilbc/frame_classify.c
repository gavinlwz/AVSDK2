...
size_t WebRtcIlbcfix_FrameClassify(
  ...
  */
  ...
  max = YoumeWebRtcSpl_MaxAbsValueW16(residualFIX, iLBCenc_inst->blockl);
  ...
  for (n=(iLBCenc_inst->nsub-1); n>0; n--) {
    (*seqEnPtr) = YoumeWebRtcSpl_DotProductWithScale(ssqPtr, ssqPtr, 76, scale1);
    ssqPtr += 40;
    seqEnPtr++;
  }

  /* Scale to maximum 20 bits in order to allow for the 11 bit window */
  maxW32 = YoumeWebRtcSpl_MaxValueW32(ssqEn, iLBCenc_inst->nsub - 1);
  scale = YoumeWebRtcSpl_GetSizeInBits(maxW32) - 20;
  scale1 = WEBRTC_SPL_MAX(0, scale);
  ...
  */
  ...
  pos = YoumeWebRtcSpl_MaxIndexW32(ssqEn, iLBCenc_inst->nsub - 1) + 1;
