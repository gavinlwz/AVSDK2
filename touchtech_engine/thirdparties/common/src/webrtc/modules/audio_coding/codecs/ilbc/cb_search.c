...
void WebRtcIlbcfix_CbSearch(
  ...
  YoumeWebRtcSpl_MemSetW16(&buf[-LPC_FILTERORDER], 0, noOfZeros+LPC_FILTERORDER);
  ...
  temp1 = YoumeWebRtcSpl_MaxAbsValueW16(buf, lMem);
  temp2 = YoumeWebRtcSpl_MaxAbsValueW16(target, lTarget);

  if ((temp1>0)&&(temp2>0)) {
    temp1 = WEBRTC_SPL_MAX(temp1, temp2);
    scale = YoumeWebRtcSpl_GetSizeInBits((uint32_t)(temp1 * temp1));
  } else {
  ...
  targetEner = YoumeWebRtcSpl_DotProductWithScale(target, target, lTarget, scale);
  ...
  if(lTarget == SUBL) {
    ...
    YoumeWebRtcSpl_MemSetW16(energyW16+range, 0, (base_size-range));
    YoumeWebRtcSpl_MemSetW16(energyW16+range+base_size, 0, (base_size-range));
  }
  ...
  for (stage=0; stage<CB_NSTAGES; stage++) {
    ...
    YoumeWebRtcSpl_CrossCorrelation(cDotPtr, target, cb_vecPtr, lTarget, range, scale, -1);
    ...
    if (lTarget==SUBL) {
      ...
      YoumeWebRtcSpl_CrossCorrelation(cDotPtr, target, cb_vecPtr, lTarget,
                                 eInd - i + 1, scale, -1);
    ...
    */
    ...
    YoumeWebRtcSpl_AddAffineVectorToVector(target, pp, (int16_t)(-bestGain),
  ...
  codedEner = YoumeWebRtcSpl_DotProductWithScale(codedVec, codedVec, lTarget, scale);

  j=gain_index[0];

  temp1 = (int16_t)YoumeWebRtcSpl_NormW32(codedEner);
  temp2 = (int16_t)YoumeWebRtcSpl_NormW32(targetEner);
