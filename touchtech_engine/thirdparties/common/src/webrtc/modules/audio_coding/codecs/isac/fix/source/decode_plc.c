    ...
    int16_t rshift )
{
  ...
  for (i = 0; i < len; i++)
  {
    ...
    for (j = 0;j < Blen; j++)
    {
      o = YoumeWebRtcSpl_AddSatW32(o, *b_ptr * *x_ptr);
      b_ptr++;
      x_ptr--;
    }

    /* to round off correctly */
    o = YoumeWebRtcSpl_AddSatW32(o, 1 << (rshift - 1));
                            ...
                            size_t lenOut)
{
  ...
  resOut = YoumeWebRtcSpl_DivW32W16ResW16( (int32_t)n, (int16_t)(lenOut-1) );
                                 ...
                                 size_t *current_framesamples )
{
  ...
  if( (ISACdec_obj->plcstr_obj).used != PLC_WAS_USED )
  {
    ...
    if ((FRAMESAMPLES_HALF - 10) > 2 * lag0)
    {
      ...
      for( i = 0; i < 21; i++ )
      {
        ...
        for( k = 0; k < lag0; k++ )
        {
          corr = YoumeWebRtcSpl_AddSatW32(corr, WEBRTC_SPL_ABS_W32(
    ...
    YoumeWebRtcSpl_AutoCorrelation(
        &(ISACdec_obj->plcstr_obj).prevPitchInvIn[FRAMESAMPLES_HALF - lag0],
  ...
  if( (ISACdec_obj->plcstr_obj).B == 0 )
  {
    ...
    maxCoeff = YoumeWebRtcSpl_MaxAbsValueW32(
  ...
  else
  {
    ...
    else
    {
      for( i = 0, noiseIndex = 0; i < FRAMESAMPLES_HALF; i++, noiseIndex++ )
      {
        ...
        /* ------ Sum the noisy and periodic signals  ------ */
        Vector_Word16_1[i] = YoumeWebRtcSpl_AddSatW16(wNoisyLP, wPriodicLP);
        Vector_Word32_2[i] = YoumeWebRtcSpl_AddSatW32(wNoisyHP, wPriodicHP);
      }
    }
  }
