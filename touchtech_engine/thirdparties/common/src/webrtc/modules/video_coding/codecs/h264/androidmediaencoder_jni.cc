...
//using youme::webrtc::QualityScaler;
...
#define ALOGV(...)   __android_log_print(ANDROID_LOG_VERBOSE, TAG_ENCODER, __VA_ARGS__)
#else
#define ALOGV(...)
#endif
        
#define ALOGD YM_LOG(LS_INFO)
#define ALOGW YM_LOG(LS_WARNING)
#define ALOGE YM_LOG(LS_ERROR)
...
//#define ALOGE LOG_TAG(rtc::LS_ERROR, TAG_ENCODER)
       ...
       // }  // namespace
        ...
        private:
            ...
            int32_t InitEncodeOnCodecThread(int width, int height, int kbps, int fps, int interval,
                                            bool use_surface, int max_quality, int min_quality, int profile_level);
            ...
            int32_t SetBitratePercentOnCodecThread( float abravg_bitrate_percent, float abrmax_bitrate_percent );
            ...
            jmethodID j_encode_texture_oes_method_;
            jmethodID j_encode_texture_rgb_method_;
        ...
        MediaCodecVideoEncoder::MediaCodecVideoEncoder(JNIEnv* jni, VideoCodecType codecType, jobject egl_context)
        :codecType_(codecType),
        callback_(NULL),
        codec_thread_(new Thread()),
        j_media_codec_video_encoder_class_(
                                           jni,
                                           FindClass(jni, "com/youme/webrtc/MediaCodecVideoEncoder")),
        ...
        egl_context_(egl_context) {
            ...
            FindClass(jni, "com/youme/webrtc/MediaCodecVideoEncoder$OutputBufferInfo");
            j_init_encode_method_ = GetMethodID(
                                                jni,
                                                *j_media_codec_video_encoder_class_,
                                                "initEncode",
                                                "(Lcom/youme/webrtc/MediaCodecVideoEncoder$VideoCodecType;"
                                                "IIIIILcom/youme/webrtc/EglBase14$Context;IIIFF)Z");
            ...
            j_encode_texture_oes_method_ = GetMethodID(
                                                   jni, *j_media_codec_video_encoder_class_, "encodeTextureOes",
                                                   "(ZI[FJ)Z");
            
            j_encode_texture_rgb_method_ = GetMethodID(
                                                   jni, *j_media_codec_video_encoder_class_, "encodeTextureRgb",
                                                   "(ZI[FJ)Z");
            ...
            j_dequeue_output_buffer_method_ = GetMethodID(
                                                          ...
                                                          *j_media_codec_video_encoder_class_,
                                                          "dequeueOutputBuffer",
                                                          "()Lcom/youme/webrtc/MediaCodecVideoEncoder$OutputBufferInfo;");
        ...
        int32_t MediaCodecVideoEncoder::InitEncode(
            ...
            int init_interval = codec_settings->codecSpecific.H264.keyFrameInterval/codec_settings->maxFramerate;
            ALOGD << "InitEncode request: " << init_width << " x " << init_height << ", gop: " << init_interval;
            ...
            //InitEncodeOnCodecThread参数太多放不下来，单独设置
            codec_thread_->Invoke<int32_t>(
                                                  Bind(&MediaCodecVideoEncoder::SetBitratePercentOnCodecThread, this,
                                                       codec_settings->abr_avg_bitrate_percent, codec_settings->abr_max_bitrate_percent));
            
            return codec_thread_->Invoke<int32_t>(
                                                  Bind(&MediaCodecVideoEncoder::InitEncodeOnCodecThread, this, init_width,
                                                       init_height, codec_settings->startBitrate,
                                                       codec_settings->maxFramerate, init_interval,
                                                        egl_context_ != NULL,
                                                       codec_settings->max_quality,
                                                       codec_settings->min_quality,
                                                       codec_settings->profile_level));
        }
        ...
        int32_t MediaCodecVideoEncoder::Encode(
                                               ...
                                               const webrtc::CodecSpecificInfo* /* codec_specific_info */,
                                               const std::vector<webrtc::FrameType>* frame_types) {
           return  codec_thread_->Invoke<int32_t>(Bind(&MediaCodecVideoEncoder::EncodeOnCodecThread, this,
                                                       frame, frame_types, rtc::Time()));
        //    return 0;
        }
        ...
        int32_t MediaCodecVideoEncoder::SetRates(uint32_t new_bit_rate,
                                                 uint32_t frame_rate) {
            return codec_thread_->Invoke<int32_t>(
                                                  Bind(&MediaCodecVideoEncoder::SetRatesOnCodecThread, this,
        ...
        bool MediaCodecVideoEncoder::ResetCodecOnCodecThread() {
            ...
            if (ReleaseOnCodecThread() != WEBRTC_VIDEO_CODEC_OK ||
                InitEncodeOnCodecThread(width_, height_, 0, 0, 0, false, -1, -1, 0) !=
        ...
        int32_t MediaCodecVideoEncoder::InitEncodeOnCodecThread(int width, int height, int kbps, int fps, int interval, bool use_surface, int max_quality,
                                                                int min_quality, int profile_level ) {
            ...
            " kbps. Fps: " << fps <<
            "max_qa: "<< max_quality <<", min_qa: "<< min_quality <<
            ", use high profile: " << profile_level
            <<",abr_avg_bitrate_percent:" <<abr_avg_bitrate_percent <<
            ",abr_max_bitrate_percent:"<<abr_max_bitrate_percent;
            if (kbps == 0) {
            ...
            const bool encode_status = jni->CallBooleanMethod(
                                                              *j_media_codec_video_encoder_, j_init_encode_method_,
                                                              j_video_codec_enum, width, height, kbps, fps, interval_,
                                                              (use_surface ? egl_context_ : nullptr), max_quality, min_quality, profile_level,
                                                              abr_avg_bitrate_percent, abr_max_bitrate_percent);
            if (!encode_status) {
            ...
            inited_ = true;
            ALOGV("webrtc mediacodec encoder create suc!!");
            return WEBRTC_VIDEO_CODEC_OK;
        }
        ...
        bool MediaCodecVideoEncoder::MaybeReconfigureEncoderOnCodecThread(
            ...
            if (!reconfigure_due_to_format && !reconfigure_due_to_size)
                return true;
            ALOGV("MaybeReconfigureEncoderOnCodecThread ----false");
            ReleaseOnCodecThread();
            
            return InitEncodeOnCodecThread(width_, height_, 0, 0, 0, is_texture_frame, -1, -1, 0) ==
            WEBRTC_VIDEO_CODEC_OK;
        }
        ...
        bool MediaCodecVideoEncoder::EncodeByteBufferOnCodecThread(JNIEnv* jni,
            ...
            RTC_CHECK(!libyuv::ConvertFromI420(
                                               frame.video_frame_buffer()->data(PlaneType::kYPlane),
                                               frame.video_frame_buffer()->stride(PlaneType::kYPlane),
                                               frame.video_frame_buffer()->data(PlaneType::kUPlane),
                                               frame.video_frame_buffer()->stride(PlaneType::kUPlane),
                                               frame.video_frame_buffer()->data(PlaneType::kVPlane),
                                               frame.video_frame_buffer()->stride(PlaneType::kVPlane),
                                               yuv_buffer, width_, width_, height_, encoder_fourcc_))
            << "ConvertFromI420 failed";
            
            //ALOGV("mediacodec encoder input pts:%d", current_timestamp_us_);
        ...
        bool MediaCodecVideoEncoder::EncodeTextureOnCodecThread(JNIEnv* jni,
            ...
            CodecSource * source = (CodecSource *)frame.video_frame_buffer()->native_handle();
            int fmt = source->fmt;
            int texture = source->textureid;
            float* sampling_matrix = source->matrix;
            jfloatArray jArray = jni->NewFloatArray(16);
            jni->SetFloatArrayRegion(jArray, 0, 16, (const jfloat *)sampling_matrix);
            bool encode_status = false;
            if (fmt == 6) {
                encode_status = jni->CallBooleanMethod(*j_media_codec_video_encoder_,
                                                       j_encode_texture_rgb_method_,
                                                       key_frame,
                                                       texture,
                                                       jArray,
                                                       current_timestamp_us_);
        ...
        int32_t MediaCodecVideoEncoder::SetBitratePercentOnCodecThread( float abravg_bitrate_percent, float abrmax_bitrate_percent )
        {
            RTC_DCHECK(codec_thread_checker_.CalledOnValidThread());
            
            abr_avg_bitrate_percent = abravg_bitrate_percent;
            abr_max_bitrate_percent = abrmax_bitrate_percent;
            return WEBRTC_VIDEO_CODEC_OK;
        }
        ...
        jlong MediaCodecVideoEncoder::GetOutputBufferInfoPresentationTimestampUs(
                                                                                 JNIEnv* jni,
                                                                                 jobject j_output_buffer_info) {
            return GetLongField(jni, j_output_buffer_info, j_info_presentation_timestamp_us_field_);
        }
        ...
        bool MediaCodecVideoEncoder::DeliverPendingOutputs(JNIEnv* jni) {
            ...
            while (true) {
                jobject j_output_buffer_info = jni->CallObjectMethod(*j_media_codec_video_encoder_, j_dequeue_output_buffer_method_);
                CHECK_EXCEPTION(jni);
                ...
                if (encoding_start_time_ms > 0) {
                    frame_encoding_time_ms = rtc::Time() - encoding_start_time_ms;
                }
                if (frames_encoded_ < kMaxEncodedLogFrames) {
                    int current_latency =
        ...
        void MediaCodecVideoEncoder::LogStatistics(bool force_log) {
            int statistic_time_ms = rtc::Time() - stat_start_time_ms_;
        ...
        MediaCodecVideoEncoderFactory::MediaCodecVideoEncoderFactory()
        {
            JNIEnv* jni = AttachCurrentThreadIfNeeded();
            ScopedLocalRefFrame local_ref_frame(jni);
            jclass j_encoder_class = FindClass(jni, "com/youme/webrtc/MediaCodecVideoEncoder");
        ...
        */
        jobject MediaCodecVideoEncoderFactory::eglbase_ = nullptr;
        jobject MediaCodecVideoEncoderFactory::egl_context_ = nullptr;
        void MediaCodecVideoEncoderFactory::SetEGLContext(jobject egl_context) {
            ...
            //ALOGV("mediacodec SetEGLContext egl_context_: %x", egl_context_);
        }

        bool MediaCodecVideoEncoderFactory::IsSupported()
        {
            ...
            bool is_h264_hw_supported = jni->CallStaticBooleanMethod(
                                                                     ...
                                                                     GetStaticMethodID(jni, j_encoder_class, "isH264HwSupported", "()Z"));
            CHECK_EXCEPTION(jni);
            ALOGV("mediacodec encoder is_h264_hw_supported: %d", is_h264_hw_supported);
            return is_h264_hw_supported;
        }
        
        bool MediaCodecVideoEncoderFactory::IsSupportedGLES()
        {
            JNIEnv* jni = AttachCurrentThreadIfNeeded();
            ScopedLocalRefFrame local_ref_frame(jni);
            jclass j_encoder_class = FindClass(jni, "com/youme/webrtc/MediaCodecVideoEncoder");
            bool is_h264_hw_supported = jni->CallStaticBooleanMethod(
                                                                     j_encoder_class,
                                                                     GetStaticMethodID(jni, j_encoder_class, "isH264HwSupportedUsingTextures", "()Z"));
            CHECK_EXCEPTION(jni);
            //ALOGV("mediacodec encoder is_h264_hw_supported: %d", is_h264_hw_supported);
            return is_h264_hw_supported;
        }
        webrtc::VideoEncoder* MediaCodecVideoEncoderFactory::CreateVideoEncoder()
        {
            if(!IsSupported())
                return NULL;
            //ALOGV("mediacodec CreateVideoEncoder egl_context_: %x", egl_context_);
            return new MediaCodecVideoEncoder(AttachCurrentThreadIfNeeded(), kVideoCodecH264, eglbase_);
        }
