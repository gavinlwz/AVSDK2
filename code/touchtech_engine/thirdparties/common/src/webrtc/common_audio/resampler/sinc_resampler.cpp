...
void SincResampler::InitializeCPUSpecificFeatures() {
    convolve_proc_ = Convolve_C;//WebRtc_GetCPUInfo(kSSE2) ? Convolve_SSE : Convolve_C;
}
#endif
...
SincResampler::SincResampler(double io_sample_rate_ratio,
    ...
    : io_sample_rate_ratio_(io_sample_rate_ratio),
      ...
      kernel_storage_((float *)tsk_malloc(sizeof(float) * kKernelStorageSize)),
      kernel_pre_sinc_storage_((float *)tsk_malloc(sizeof(float) * kKernelStorageSize)),
      kernel_window_storage_((float *)tsk_malloc(sizeof(float) * kKernelStorageSize)),
      input_buffer_((float *)tsk_malloc(sizeof(float) * input_buffer_size_)),
#if defined(WEBRTC_CPU_DETECTION)
      convolve_proc_(nullptr),
#endif
      r1_(input_buffer_),
      r2_(input_buffer_ + kKernelSize / 2) {
#if defined(WEBRTC_CPU_DETECTION)
  InitializeCPUSpecificFeatures();
  ...
  memset(kernel_storage_, 0,
         sizeof(*kernel_storage_) * kKernelStorageSize);
  memset(kernel_pre_sinc_storage_, 0,
         sizeof(*kernel_pre_sinc_storage_) * kKernelStorageSize);
  memset(kernel_window_storage_, 0,
         sizeof(*kernel_window_storage_) * kKernelStorageSize);

  InitializeKernel();
}

SincResampler::~SincResampler() {
    TSK_FREE(kernel_storage_);
    TSK_FREE(kernel_pre_sinc_storage_);
    TSK_FREE(kernel_window_storage_);
    TSK_FREE(input_buffer_);
}

void SincResampler::UpdateRegions(bool second_load) {
  // Setup various region pointers in the buffer (see diagram above).  If we're
  // on the second load we need to slide r0_ to the right by kKernelSize / 2.
  r0_ = input_buffer_ + (second_load ? kKernelSize : kKernelSize / 2);
  r3_ = r0_ + request_frames_ - kKernelSize;
  r4_ = r0_ + request_frames_ - kKernelSize / 2;
  block_size_ = r4_ - r2_;

  // r1_ at the beginning of the buffer.
  RTC_DCHECK_EQ(r1_, input_buffer_);
...
void SincResampler::Resample(size_t frames, float* destination) {
  ...
  const float* const kernel_ptr = kernel_storage_;
  while (remaining_frames) {
    ...
    memcpy(r1_, r3_, sizeof(*input_buffer_) * kKernelSize);
