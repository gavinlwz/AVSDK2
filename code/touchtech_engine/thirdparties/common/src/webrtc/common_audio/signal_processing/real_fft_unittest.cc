...
TEST_F(RealFFTTest, CreateFailsOnBadInput) {
  RealFFT* fft = YoumeWebRtcSpl_CreateRealFFT(11);
  EXPECT_TRUE(fft == NULL);
  fft = YoumeWebRtcSpl_CreateRealFFT(-1);
  EXPECT_TRUE(fft == NULL);
}
...
TEST_F(RealFFTTest, RealAndComplexMatch) {
  ...
  RealFFT* fft = YoumeWebRtcSpl_CreateRealFFT(kOrder);
  EXPECT_TRUE(fft != NULL);
  EXPECT_EQ(0, YoumeWebRtcSpl_RealForwardFFT(fft, real_fft_time, real_fft_freq));

  // Run complex forward FFT.
  YoumeWebRtcSpl_ComplexBitReverse(complex_fft_buff, kOrder);
  EXPECT_EQ(0, YoumeWebRtcSpl_ComplexFFT(complex_fft_buff, kOrder, 1));
  ...
  int real_scale = YoumeWebRtcSpl_RealInverseFFT(fft, real_fft_freq, real_fft_time);
  EXPECT_GE(real_scale, 0);

  // Run complex inverse FFT.
  YoumeWebRtcSpl_ComplexBitReverse(complex_fft_buff, kOrder);
  int complex_scale = YoumeWebRtcSpl_ComplexIFFT(complex_fft_buff, kOrder, 1);
  ...
  YoumeWebRtcSpl_FreeRealFFT(fft);
}
