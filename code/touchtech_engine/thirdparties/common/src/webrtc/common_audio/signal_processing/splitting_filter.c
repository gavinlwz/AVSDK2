...
static const uint16_t YoumeWebRtcSpl_kAllPassFilter1[3] = {6418, 36982, 57261};
static const uint16_t YoumeWebRtcSpl_kAllPassFilter2[3] = {21333, 49062, 63010};

///////////////////////////////////////////////////////////////////////////////////////////////
// YoumeWebRtcSpl_AllPassQMF(...)
//
...
void YoumeWebRtcSpl_AllPassQMF(int32_t* in_data, size_t data_length,
                          ...
                          int32_t* filter_state)
{
    ...
    // diff = (x[0] - y_1[-1])
    diff = YoumeWebRtcSpl_SubSatW32(in_data[0], filter_state[1]);
    ...
    {
        // diff = (x[n] - y_1[n-1])
        diff = YoumeWebRtcSpl_SubSatW32(in_data[k], out_data[k - 1]);
    ...
    // diff = (y_1[0] - y_2[-1])
    diff = YoumeWebRtcSpl_SubSatW32(out_data[0], filter_state[3]);
    // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
    in_data[0] = WEBRTC_SPL_SCALEDIFF32(filter_coefficients[1], diff, filter_state[2]);
    for (k = 1; k < data_length; k++)
    {
        // diff = (y_1[n] - y_2[n-1])
        diff = YoumeWebRtcSpl_SubSatW32(out_data[k], in_data[k - 1]);
        // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
    ...
    // diff = (y_2[0] - y[-1])
    diff = YoumeWebRtcSpl_SubSatW32(in_data[0], filter_state[5]);
...
void YoumeWebRtcSpl_AnalysisQMF(const int16_t* in_data, size_t in_data_length,
                           ...
                           int32_t* filter_state1, int32_t* filter_state2)
{
    ...
    YoumeWebRtcSpl_AllPassQMF(half_in1, band_length, filter1,
                         YoumeWebRtcSpl_kAllPassFilter1, filter_state1);
    YoumeWebRtcSpl_AllPassQMF(half_in2, band_length, filter2,
                         YoumeWebRtcSpl_kAllPassFilter2, filter_state2);
    ...
    for (i = 0; i < band_length; i++)
    {
        ...
        low_band[i] = YoumeWebRtcSpl_SatW32ToW16(tmp);

        tmp = (filter1[i] - filter2[i] + 1024) >> 11;
        high_band[i] = YoumeWebRtcSpl_SatW32ToW16(tmp);
    }
}

void YoumeWebRtcSpl_SynthesisQMF(const int16_t* low_band, const int16_t* high_band,
                            ...
                            int32_t* filter_state1, int32_t* filter_state2)
{
    ...
    YoumeWebRtcSpl_AllPassQMF(half_in1, band_length, filter1,
                         YoumeWebRtcSpl_kAllPassFilter2, filter_state1);
    YoumeWebRtcSpl_AllPassQMF(half_in2, band_length, filter2,
                         YoumeWebRtcSpl_kAllPassFilter1, filter_state2);
    ...
    for (i = 0, k = 0; i < band_length; i++)
    {
        ...
        out_data[k++] = YoumeWebRtcSpl_SatW32ToW16(tmp);
