...
// NEON intrinsics version of YoumeWebRtcSpl_DownsampleFast()
...
int YoumeWebRtcSpl_DownsampleFastNeon(const int16_t* data_in,
  ...
  for (; i < endpos; i += factor) {
    ...
    for (j = 0; j < coefficients_length; j++) {
      out_s32 = YoumeWebRtcSpl_MulAccumW16(coefficients[j], data_in[i - j], out_s32);
    }

    // Saturate and store the output.
    out_s32 >>= 12;
    *data_out++ = YoumeWebRtcSpl_SatW32ToW16(out_s32);
  }
