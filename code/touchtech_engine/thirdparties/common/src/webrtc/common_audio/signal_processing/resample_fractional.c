...
static void YoumeWebRtcSpl_ResampDotProduct(const int32_t *in1, const int32_t *in2,
...
void YoumeWebRtcSpl_Resample44khzTo32khz(const int32_t *In, int32_t *Out, size_t K)
{
    ...
    for (m = 0; m < K; m++)
    {
        ...
        // sum and accumulate filter coefficients and input samples
        YoumeWebRtcSpl_ResampDotProduct(&In[0], &In[17], kCoefficients44To32[0], &Out[1], &Out[7]);

        // sum and accumulate filter coefficients and input samples
        YoumeWebRtcSpl_ResampDotProduct(&In[2], &In[15], kCoefficients44To32[1], &Out[2], &Out[6]);

        // sum and accumulate filter coefficients and input samples
        YoumeWebRtcSpl_ResampDotProduct(&In[3], &In[14], kCoefficients44To32[2], &Out[3], &Out[5]);
