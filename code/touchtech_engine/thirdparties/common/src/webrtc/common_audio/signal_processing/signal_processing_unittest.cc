...
TEST_F(SplTest, InlineTest) {
    ...
    EXPECT_EQ(17, YoumeWebRtcSpl_GetSizeInBits(a32));

    EXPECT_EQ(0, YoumeWebRtcSpl_NormW32(0));
    EXPECT_EQ(31, YoumeWebRtcSpl_NormW32(-1));
    EXPECT_EQ(0, YoumeWebRtcSpl_NormW32(WEBRTC_SPL_WORD32_MIN));
    EXPECT_EQ(14, YoumeWebRtcSpl_NormW32(a32));

    EXPECT_EQ(0, YoumeWebRtcSpl_NormW16(0));
    EXPECT_EQ(15, YoumeWebRtcSpl_NormW16(-1));
    EXPECT_EQ(0, YoumeWebRtcSpl_NormW16(WEBRTC_SPL_WORD16_MIN));
    EXPECT_EQ(4, YoumeWebRtcSpl_NormW16(b32));
    for (int ii = 0; ii < 15; ++ii) {
      int16_t value = 1 << ii;
      EXPECT_EQ(14 - ii, YoumeWebRtcSpl_NormW16(value));
      EXPECT_EQ(15 - ii, YoumeWebRtcSpl_NormW16(-value));
    }

    EXPECT_EQ(0, YoumeWebRtcSpl_NormU32(0u));
    EXPECT_EQ(0, YoumeWebRtcSpl_NormU32(0xffffffff));
    EXPECT_EQ(15, YoumeWebRtcSpl_NormU32(static_cast<uint32_t>(a32)));

    EXPECT_EQ(104, YoumeWebRtcSpl_AddSatW16(a16, b16));
    EXPECT_EQ(138, YoumeWebRtcSpl_SubSatW16(a16, b16));

    EXPECT_EQ(109410, YoumeWebRtcSpl_AddSatW32(a32, b32));
    EXPECT_EQ(112832, YoumeWebRtcSpl_SubSatW32(a32, b32));

    a32 = 0x80000000;
    b32 = 0x80000000;
    // Cast to signed int to avoid compiler complaint on gtest.h.
    EXPECT_EQ(static_cast<int>(0x80000000), YoumeWebRtcSpl_AddSatW32(a32, b32));
    a32 = 0x7fffffff;
    b32 = 0x7fffffff;
    EXPECT_EQ(0x7fffffff, YoumeWebRtcSpl_AddSatW32(a32, b32));
    a32 = 0;
    b32 = 0x80000000;
    EXPECT_EQ(0x7fffffff, YoumeWebRtcSpl_SubSatW32(a32, b32));
    a32 = 0x7fffffff;
    b32 = 0x80000000;
    EXPECT_EQ(0x7fffffff, YoumeWebRtcSpl_SubSatW32(a32, b32));
    a32 = 0x80000000;
    b32 = 0x7fffffff;
    EXPECT_EQ(static_cast<int>(0x80000000), YoumeWebRtcSpl_SubSatW32(a32, b32));
}
...
TEST_F(SplTest, MathOperationsTest) {
    ...
    int32_t den = -5;
    uint16_t denU = 5;
    EXPECT_EQ(33700, YoumeWebRtcSpl_Sqrt(A));
    EXPECT_EQ(33683, YoumeWebRtcSpl_SqrtFloor(A));


    EXPECT_EQ(-91772805, YoumeWebRtcSpl_DivResultInQ31(den, num));
    EXPECT_EQ(-23, YoumeWebRtcSpl_DivW32W16ResW16(num, (int16_t)den));
    EXPECT_EQ(-23, YoumeWebRtcSpl_DivW32W16(num, (int16_t)den));
    EXPECT_EQ(23u, YoumeWebRtcSpl_DivU32U16(num, denU));
    EXPECT_EQ(0, YoumeWebRtcSpl_DivW32HiLow(128, 0, 256));
}
...
TEST_F(SplTest, BasicArrayOperationsTest) {
    ...
    YoumeWebRtcSpl_MemSetW16(b16, 3, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(3, b16[kk]);
    }
    YoumeWebRtcSpl_ZerosArrayW16(b16, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(0, b16[kk]);
    }
    YoumeWebRtcSpl_MemSetW32(b32, 3, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(3, b32[kk]);
    }
    YoumeWebRtcSpl_ZerosArrayW32(b32, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(0, b32[kk]);
    }
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
...
//    }
    YoumeWebRtcSpl_CopyFromEndW16(b16, kVectorSize, 2, bTmp16);
    ...
    YoumeWebRtcSpl_VectorBitShiftW32ToW16(bTmp16, kVectorSize, b32, 1);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((B[kk]>>1), bTmp16[kk]);
    }
    YoumeWebRtcSpl_VectorBitShiftW16(bTmp16, kVectorSize, b16, 1);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((B[kk]>>1), bTmp16[kk]);
    }
    YoumeWebRtcSpl_VectorBitShiftW32(bTmp32, kVectorSize, b32, 1);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((B[kk]>>1), bTmp32[kk]);
    }

    YoumeWebRtcSpl_MemCpyReversedOrder(&bTmp16[3], b16, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
...
TEST_F(SplTest, MinMaxOperationsTest) {
  ...
  EXPECT_EQ(WEBRTC_SPL_WORD16_MIN,
            YoumeWebRtcSpl_MinValueW16(vector16, kVectorSize));
  EXPECT_EQ(WEBRTC_SPL_WORD32_MIN,
            YoumeWebRtcSpl_MinValueW32(vector32, kVectorSize));
  EXPECT_EQ(kVectorSize - 1, YoumeWebRtcSpl_MinIndexW16(vector16, kVectorSize));
  EXPECT_EQ(kVectorSize - 1, YoumeWebRtcSpl_MinIndexW32(vector32, kVectorSize));
  ...
  EXPECT_EQ(WEBRTC_SPL_WORD16_MAX,
            YoumeWebRtcSpl_MaxAbsValueW16(vector16, kVectorSize));
  EXPECT_EQ(WEBRTC_SPL_WORD16_MAX,
            YoumeWebRtcSpl_MaxValueW16(vector16, kVectorSize));
  EXPECT_EQ(WEBRTC_SPL_WORD32_MAX,
            YoumeWebRtcSpl_MaxAbsValueW32(vector32, kVectorSize));
  EXPECT_EQ(WEBRTC_SPL_WORD32_MAX,
            YoumeWebRtcSpl_MaxValueW32(vector32, kVectorSize));
  EXPECT_EQ(kVectorSize - 1, YoumeWebRtcSpl_MaxAbsIndexW16(vector16, kVectorSize));
  EXPECT_EQ(kVectorSize - 1, YoumeWebRtcSpl_MaxIndexW16(vector16, kVectorSize));
  EXPECT_EQ(kVectorSize - 1, YoumeWebRtcSpl_MaxIndexW32(vector32, kVectorSize));
  ...
  EXPECT_EQ(WEBRTC_SPL_WORD32_MIN,
            YoumeWebRtcSpl_MinValueW32(vector32, kVectorSize));
  EXPECT_EQ(6u, YoumeWebRtcSpl_MaxAbsIndexW16(vector16, kVectorSize));
  EXPECT_EQ(1u, YoumeWebRtcSpl_MaxIndexW16(vector16, kVectorSize));
  EXPECT_EQ(1u, YoumeWebRtcSpl_MaxIndexW32(vector32, kVectorSize));
  EXPECT_EQ(6u, YoumeWebRtcSpl_MinIndexW16(vector16, kVectorSize));
  EXPECT_EQ(6u, YoumeWebRtcSpl_MinIndexW32(vector32, kVectorSize));
}
...
TEST_F(SplTest, VectorOperationsTest) {
    ...
    YoumeWebRtcSpl_AffineTransformVector(bTmp16, b16, 3, 7, 2, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((B[kk]*3+7)>>2, bTmp16[kk]);
    }
    YoumeWebRtcSpl_ScaleAndAddVectorsWithRound(b16, 3, b16, 2, 2, bTmp16, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((B[kk]*3+B[kk]*2+2)>>2, bTmp16[kk]);
    }

    YoumeWebRtcSpl_AddAffineVectorToVector(bTmp16, b16, 3, 7, 2, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(((B[kk]*3+B[kk]*2+2)>>2)+((b16[kk]*3+7)>>2), bTmp16[kk]);
    }

    YoumeWebRtcSpl_ScaleVector(b16, bTmp16, 13, kVectorSize, 2);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((b16[kk]*13)>>2, bTmp16[kk]);
    }
    YoumeWebRtcSpl_ScaleVectorWithSat(b16, bTmp16, 13, kVectorSize, 2);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((b16[kk]*13)>>2, bTmp16[kk]);
    }
    YoumeWebRtcSpl_ScaleAndAddVectors(a16, 13, 2, b16, 7, 2, bTmp16, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(((a16[kk]*13)>>2)+((b16[kk]*7)>>2), bTmp16[kk]);
    }

    YoumeWebRtcSpl_AddVectorsAndShift(bTmp16, a16, b16, kVectorSize, 2);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ(B[kk] >> 1, bTmp16[kk]);
    }
    YoumeWebRtcSpl_ReverseOrderMultArrayElements(bTmp16, a16, &b16[3], kVectorSize, 2);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((a16[kk]*b16[3-kk])>>2, bTmp16[kk]);
    }
    YoumeWebRtcSpl_ElementwiseVectorMult(bTmp16, a16, b16, kVectorSize, 6);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
        EXPECT_EQ((a16[kk]*b16[kk])>>6, bTmp16[kk]);
    }

    YoumeWebRtcSpl_SqrtOfOneMinusXSquared(b16, kVectorSize, bTmp16);
    for (size_t kk = 0; kk < kVectorSize - 1; ++kk) {
        EXPECT_EQ(32767, bTmp16[kk]);
    }
    EXPECT_EQ(32749, bTmp16[kVectorSize - 1]);

    EXPECT_EQ(0, YoumeWebRtcSpl_GetScalingSquare(b16, kVectorSize, 1));
}
...
TEST_F(SplTest, EstimatorsTest) {
  ...
  EXPECT_EQ(0, YoumeWebRtcSpl_LevinsonDurbin(unstable_filter, lpc, refl, kOrder));
  EXPECT_EQ(1, YoumeWebRtcSpl_LevinsonDurbin(stable_filter, lpc, refl, kOrder));
...
TEST_F(SplTest, FilterTest) {
    ...
    YoumeWebRtcSpl_ZerosArrayW16(bState, kVectorSize);
    YoumeWebRtcSpl_ZerosArrayW16(bStateLow, kVectorSize);
    ...
    // data (one sample).
    YoumeWebRtcSpl_FilterMAFastQ12(&data_in[kFilterOrder], data_out, B,
                              kFilterOrder + 1, 1);
    EXPECT_EQ(0, data_out[0]);
    // AR filters.
    // Note that the output data has |kFilterOrder| states before the actual
    // data (one sample).
    YoumeWebRtcSpl_FilterARFastQ12(data_in, &data_out[kFilterOrder], A,
                              kFilterOrder + 1, 1);
    EXPECT_EQ(0, data_out[kFilterOrder]);

    EXPECT_EQ(kVectorSize, YoumeWebRtcSpl_FilterAR(A5,
                                              5,
                                              data_in,
                                              kVectorSize,
                                              bState,
...
TEST_F(SplTest, RandTest) {
    ...
    EXPECT_EQ(7086, YoumeWebRtcSpl_RandU(&bSeed));
    EXPECT_EQ(31565, YoumeWebRtcSpl_RandU(&bSeed));
    EXPECT_EQ(-9786, YoumeWebRtcSpl_RandN(&bSeed));
    EXPECT_EQ(kVectorSize, YoumeWebRtcSpl_RandUArray(b16, kVectorSize, &bSeed));
    for (int kk = 0; kk < kVectorSize; ++kk) {
...
TEST_F(SplTest, DotProductWithScaleTest) {
  EXPECT_EQ(605362796, YoumeWebRtcSpl_DotProductWithScale(vector16,
...
TEST_F(SplTest, CrossCorrelationTest) {
  ...
  YoumeWebRtcSpl_CrossCorrelation(vector32, vector16, kVector16, kSeqDimension,
                             kCrossCorrelationDimension, kShift, kStep);

  // YoumeWebRtcSpl_CrossCorrelationC() and YoumeWebRtcSpl_CrossCorrelationNeon()
  // are not bit-exact.
  ...
  if (YoumeWebRtcSpl_CrossCorrelation != YoumeWebRtcSpl_CrossCorrelationC) {
...
TEST_F(SplTest, AutoCorrelationTest) {
  ...
  EXPECT_EQ(kVector16Size,
            YoumeWebRtcSpl_AutoCorrelation(vector16, kVector16Size,
...
//    YoumeWebRtcSpl_ReflCoefToLpc(b16, kVectorSize, bTmp16);
////    for (int kk = 0; kk < kVectorSize; ++kk) {
////        EXPECT_EQ(aTmp16[kk], bTmp16[kk]);
////    }
//    YoumeWebRtcSpl_LpcToReflCoef(bTmp16, kVectorSize, b16);
////    for (int kk = 0; kk < kVectorSize; ++kk) {
////        EXPECT_EQ(a16[kk], b16[kk]);
////    }
//    YoumeWebRtcSpl_AutoCorrToReflCoef(b32, kVectorSize, bTmp16);
////    for (int kk = 0; kk < kVectorSize; ++kk) {
////        EXPECT_EQ(aTmp16[kk], bTmp16[kk]);
////    }

    YoumeWebRtcSpl_GetHanningWindow(bTmp16, kVectorSize);
    for (size_t kk = 0; kk < kVectorSize; ++kk) {
    ...
    EXPECT_EQ(11094 , YoumeWebRtcSpl_Energy(b16, kVectorSize, &bScale));
    EXPECT_EQ(0, bScale);
}
...
TEST_F(SplTest, FFTTest) {
    ...
    EXPECT_EQ(0, YoumeWebRtcSpl_ComplexFFT(B, 3, 1));
//    for (int kk = 0; kk < 16; ++kk) {
//        EXPECT_EQ(A[kk], B[kk]);
//    }
    EXPECT_EQ(0, YoumeWebRtcSpl_ComplexIFFT(B, 3, 1));
//    for (int kk = 0; kk < 16; ++kk) {
//        EXPECT_EQ(A[kk], B[kk]);
//    }
    YoumeWebRtcSpl_ComplexBitReverse(B, 3);
    for (int kk = 0; kk < 16; ++kk) {
        //EXPECT_EQ(A[kk], B[kk]);
    }
}
...
TEST_F(SplTest, Resample48WithSaturationTest) {
  ...
  YoumeWebRtcSpl_Resample48khzTo32khz(kVectorSaturated, out_vector, kBlockSize);
  YoumeWebRtcSpl_VectorBitShiftW32ToW16(out_vector_w16, 2 * kBlockSize, out_vector,
                                   15);
