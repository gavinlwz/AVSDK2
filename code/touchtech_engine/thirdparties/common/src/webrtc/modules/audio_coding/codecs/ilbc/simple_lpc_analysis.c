...
void WebRtcIlbcfix_SimpleLpcAnalysis(
  ...
  for (k = 0; k < iLBCenc_inst->lpc_n; k++) {
    ...
    if (k < (iLBCenc_inst->lpc_n - 1)) {
      ...
      YoumeWebRtcSpl_ElementwiseVectorMult(windowedData, iLBCenc_inst->lpc_buffer, WebRtcIlbcfix_kLpcWin, BLOCKL_MAX, 15);
    } else {

      /* Hanning table WebRtcIlbcfix_kLpcAsymWin[] is in Q15-domain so the output is right-shifted 15 */
      YoumeWebRtcSpl_ElementwiseVectorMult(windowedData, iLBCenc_inst->lpc_buffer+is, WebRtcIlbcfix_kLpcAsymWin, BLOCKL_MAX, 15);
    }

    /* Compute autocorrelation */
    YoumeWebRtcSpl_AutoCorrelation(windowedData, BLOCKL_MAX, LPC_FILTERORDER, R, &scale);
    ...
    stability=YoumeWebRtcSpl_LevinsonDurbin(R, A, rc, LPC_FILTERORDER);
    ...
    */
    if (stability!=1) {
      A[0]=4096;
      YoumeWebRtcSpl_MemSetW16(&A[1], 0, LPC_FILTERORDER);
    }
