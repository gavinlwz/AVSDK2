...
void WebRtcIsacfix_InitPlc( PLCstr *State )
{
  ...
  YoumeWebRtcSpl_ZerosArrayW16(State->overlapLP, RECOVERY_OVERLAP);
  YoumeWebRtcSpl_ZerosArrayW16(State->lofilt_coefQ15, ORDERLO);
  YoumeWebRtcSpl_ZerosArrayW16(State->hifilt_coefQ15, ORDERHI );

  State->AvgPitchGain_Q12 = 0;
  State->lastPitchGain_Q12 = 0;
  State->lastPitchLag_Q7 = 0;
  State->gain_lo_hiQ17[0]=State->gain_lo_hiQ17[1] = 0;
  YoumeWebRtcSpl_ZerosArrayW16(State->prevPitchInvIn, FRAMESAMPLES/2);
  YoumeWebRtcSpl_ZerosArrayW16(State->prevPitchInvOut, PITCH_MAX_LAG + 10 );
  YoumeWebRtcSpl_ZerosArrayW32(State->prevHP, PITCH_MAX_LAG + 10 );
  State->pitchCycles = 0;
  State->A = 0;
  State->B = 0;
  State->pitchIndex = 0;
