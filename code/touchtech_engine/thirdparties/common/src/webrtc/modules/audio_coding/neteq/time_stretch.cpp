...
TimeStretch::ReturnCodes TimeStretch::Process(const int16_t* input,
  ...
  max_input_value_ = YoumeWebRtcSpl_MaxAbsValueW16(signal, signal_len);
  ...
  // without overflowing.
  int scaling = 31 - YoumeWebRtcSpl_NormW32(max_input_value_ * max_input_value_) -
      YoumeWebRtcSpl_NormW32(static_cast<int32_t>(peak_index));
  ...
  // |peak_index| samples.
  int32_t vec1_energy =
      YoumeWebRtcSpl_DotProductWithScale(vec1, vec1, peak_index, scaling);
  int32_t vec2_energy =
      YoumeWebRtcSpl_DotProductWithScale(vec2, vec2, peak_index, scaling);

  // Calculate cross-correlation between |vec1| and |vec2|.
  int32_t cross_corr =
      YoumeWebRtcSpl_DotProductWithScale(vec1, vec2, peak_index, scaling);
  ...
  if (!active_speech) {
    ...
    int energy1_scale = std::max(0, 16 - YoumeWebRtcSpl_NormW32(vec1_energy));
    int energy2_scale = std::max(0, 16 - YoumeWebRtcSpl_NormW32(vec2_energy));
    ...
    int16_t sqrt_energy_prod = YoumeWebRtcSpl_SqrtFloor(vec1_energy_int16 *
                                                   vec2_energy_int16);
    ...
    best_correlation = YoumeWebRtcSpl_DivW32W16(cross_corr, sqrt_energy_prod);
...
void TimeStretch::AutoCorrelation() {
  ...
  int scaling = kLogCorrelationLen - YoumeWebRtcSpl_NormW32(
  ...
  YoumeWebRtcSpl_CrossCorrelation(auto_corr, &downsampled_input_[kMaxLag],
  ...
  int32_t max_corr = YoumeWebRtcSpl_MaxAbsValueW32(auto_corr, kCorrelationLen);
  scaling = std::max(0, 17 - YoumeWebRtcSpl_NormW32(max_corr));
  YoumeWebRtcSpl_VectorBitShiftW32ToW16(auto_correlation_, kCorrelationLen,
                                   auto_corr, scaling);
}
...
bool TimeStretch::SpeechDetection(int32_t vec1_energy, int32_t vec2_energy,
  ...
  int right_scale = 16 - YoumeWebRtcSpl_NormW32(right_side);
  ...
  if (YoumeWebRtcSpl_NormW32(left_side) < 2 * scaling) {
    // Cannot scale only |left_side|, must scale |right_side| too.
    int temp_scale = YoumeWebRtcSpl_NormW32(left_side);
