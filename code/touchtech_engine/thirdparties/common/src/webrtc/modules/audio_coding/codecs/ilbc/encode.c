...
void WebRtcIlbcfix_EncodeImpl(
  ...
  YoumeWebRtcSpl_MemSetW16 (  (int16_t *) iLBCbits_inst, 0,
  ...
  if (iLBCenc_inst->section != 0){
    ...
    max=YoumeWebRtcSpl_MaxAbsValueW16(&residual[index], 2*SUBL);
    scale = YoumeWebRtcSpl_GetSizeInBits((uint32_t)(max * max));
    ...
    en1=YoumeWebRtcSpl_DotProductWithScale(&residual[index], &residual[index],
                                      iLBCenc_inst->state_short_len, scale);
    index += diff;
    en2=YoumeWebRtcSpl_DotProductWithScale(&residual[index], &residual[index],
                                      iLBCenc_inst->state_short_len, scale);
    if (en1 > en2) {
    ...
    if (iLBCbits_inst->state_first) { /* put adaptive part in the end */
      ...
      YoumeWebRtcSpl_MemSetW16(mem, 0, CB_MEML - iLBCenc_inst->state_short_len);
      ...
      YoumeWebRtcSpl_MemCpyReversedOrder(&reverseResidual[diff-1],
                                    &residual[(iLBCbits_inst->startIdx+1)*SUBL-STATE_LEN], diff);

      /* setup memory */

      meml_gotten = iLBCenc_inst->state_short_len;
      YoumeWebRtcSpl_MemCpyReversedOrder(&mem[CB_MEML-1], &decresidual[start_pos], meml_gotten);
      YoumeWebRtcSpl_MemSetW16(mem, 0, CB_MEML - iLBCenc_inst->state_short_len);
      ...
      YoumeWebRtcSpl_MemCpyReversedOrder(&decresidual[start_pos-1], reverseDecresidual, diff);
    }
  ...
  if( Nfor > 0 ){
    ...
    YoumeWebRtcSpl_MemSetW16(mem, 0, CB_MEML-STATE_LEN);
  ...
  if (iLBCbits_inst->startIdx > 1) {
    ...
    */
    ...
    YoumeWebRtcSpl_MemCpyReversedOrder(&reverseResidual[Nback*SUBL-1], residual, Nback*SUBL);
    ...
    YoumeWebRtcSpl_MemCpyReversedOrder(&mem[CB_MEML-1], &decresidual[Nback*SUBL], meml_gotten);
    YoumeWebRtcSpl_MemSetW16(mem, 0, CB_MEML - meml_gotten);
    ...
    YoumeWebRtcSpl_MemCpyReversedOrder(&decresidual[SUBL*Nback-1], reverseDecresidual, SUBL*Nback);
  }
  /* end encoding part */
