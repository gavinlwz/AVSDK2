...
#ifndef WEBRTC_MODULES_AUDIO_CODING_NETEQ_DECISION_LOGIC_GAME_VOICE_H_
#define WEBRTC_MODULES_AUDIO_CODING_NETEQ_DECISION_LOGIC_GAME_VOICE_H_
...
// kPlayoutStreaming.
class DecisionLogicGameVoice : public DecisionLogic {
 public:
  // Constructor.
  DecisionLogicGameVoice(int fs_hz,
 ...
 protected:
  ...
  static const int kReinitAfterPLCs = 100;
  ...
  // Override to calculate for PLCs instead of Expand
  virtual void ExpandDecision(Operations operation) override;

  // Override to indicate that we will use codec internal PLC instead of the Expand class
  virtual bool UsePlcInsteadOfExpand() const override { return true; }
 ...
 private:
  ...
  // to exceeding kReinitAfterPLCs will be done.
  bool ReinitAfterPLCs(uint32_t timestamp_leap) const;

  // Checks if we still have not done enough plc to cover the distance from
  ...
  RTC_DISALLOW_COPY_AND_ASSIGN(DecisionLogicGameVoice);
};

}  // namespace webrtc
}  // namespace youme
#endif  // WEBRTC_MODULES_AUDIO_CODING_NETEQ_DECISION_LOGIC_GAME_VOICE_H_
