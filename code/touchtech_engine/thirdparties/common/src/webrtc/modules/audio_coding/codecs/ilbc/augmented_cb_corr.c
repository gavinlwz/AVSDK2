                              ...
                              the crossDot */
{
  ...
  for (lagcount=low; lagcount<=high; lagcount++) {
    ...
    (*crossDotPtr) = YoumeWebRtcSpl_DotProductWithScale(target, buffer-lagcount, ilow, scale);

    /* Compute dot product on the interpolated samples */
    (*crossDotPtr) += YoumeWebRtcSpl_DotProductWithScale(target+ilow, iSPtr, 4, scale);
    targetPtr = target + lagcount;
    iSPtr += lagcount-ilow;

    /* Compute dot product for the remaining samples */
    (*crossDotPtr) += YoumeWebRtcSpl_DotProductWithScale(targetPtr, buffer-lagcount, SUBL-lagcount, scale);
    crossDotPtr++;
  }
}
