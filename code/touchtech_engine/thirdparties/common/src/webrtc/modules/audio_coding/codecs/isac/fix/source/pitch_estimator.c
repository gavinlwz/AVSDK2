...
static __inline void Intrp1DQ8(int32_t *x, int32_t *fx, int32_t *y, int32_t *fy) {
  ...
  if ((fx[0]>0) && (fx[2]>0)) {
    ...
    /* (Signs are removed because YoumeWebRtcSpl_DivResultInQ31 can't handle negative numbers) */
    /* t in Q31, without signs */
    t32 = YoumeWebRtcSpl_DivResultInQ31(nom32 * sign1, den32 * sign2);
                                ...
                                )
{
  ...
  YoumeWebRtcSpl_FilterARFastQ12(&buf_dec16[start],&buf_dec16[start],(int16_t*)kACoefQ12,3, PITCH_FRAME_LEN/2);
