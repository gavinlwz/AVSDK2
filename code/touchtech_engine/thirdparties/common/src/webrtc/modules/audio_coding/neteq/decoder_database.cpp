...
int DecoderDatabase::RegisterPayload(uint8_t rtp_payload_type,
  ...
  if ((8000 != fs_hz) && (16000 != fs_hz) && (24000 != fs_hz) && (32000 != fs_hz) && (48000 != fs_hz)) {
    fs_hz = CodecSampleRateHz(codec_type);
  }
...
AudioDecoder* DecoderDatabase::GetDecoder(uint8_t rtp_payload_type) {
  ...
  if (!info->decoder) {
    ...
    AudioDecoder* decoder = CreateAudioDecoder(info->codec_type, info->fs_hz);
