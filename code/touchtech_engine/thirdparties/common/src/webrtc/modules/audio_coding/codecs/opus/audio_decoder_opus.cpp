...
AudioDecoderOpus::AudioDecoderOpus(size_t num_channels, int32_t fs_hz)
    : channels_(num_channels)
    , sample_rate_hz_(fs_hz) {
  RTC_DCHECK(num_channels == 1 || num_channels == 2);
  WebRtcOpus_DecoderCreate(&dec_state_, static_cast<int>(channels_), sample_rate_hz_);
...
int AudioDecoderOpus::PacketDuration(const uint8_t* encoded,
  ...
  return WebRtcOpus_DurationEst(dec_state_, encoded, encoded_len, sample_rate_hz_);
}
...
bool AudioDecoderOpus::HasDecodePlc() const {
  return true;
}
  
size_t AudioDecoderOpus::DecodePlc(size_t num_frames, int16_t* decoded) {
  int ret = WebRtcOpus_DecodePlc(dec_state_, decoded, num_frames);
  if (ret >= 0) {
    return ret;
  } else {
    return -1;
