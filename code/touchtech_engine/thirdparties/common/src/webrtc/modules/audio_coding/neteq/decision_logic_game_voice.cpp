...
#include "webrtc/modules/audio_coding/neteq/decision_logic_game_voice.h"
...
Operations DecisionLogicGameVoice::GetDecisionSpecialized(
    ...
    bool play_dtmf,
    bool* reset_decoder) {
  assert(playout_mode_ == kPlayoutGameVoice);
  ...
  // If the plc period was very long, reset NetEQ since it is likely that the
  // sender was restarted.
  if (num_consecutive_expands_ > kReinitAfterPLCs) {
    *reset_decoder = true;
    return kNormal;
  }
...
Operations DecisionLogicGameVoice::CngOperation(Modes prev_mode,
...
Operations DecisionLogicGameVoice::NoPacket(bool play_dtmf) {
...
void DecisionLogicGameVoice::ExpandDecision(Operations operation) {
  if (operation == kPlc) {
    num_consecutive_expands_++;
  } else if (operation != kNoPacket) {
    num_consecutive_expands_ = 0;
  }
}

Operations DecisionLogicGameVoice::ExpectedPacketAvailable(Modes prev_mode,
                                                        bool play_dtmf) {
  if (prev_mode != kModeExpand && prev_mode != kModePlc && !play_dtmf) {
...
Operations DecisionLogicGameVoice::FuturePacketAvailable(
    const SyncBuffer& sync_buffer,
    const Expand& expand,
    size_t decoder_frame_length,
    Modes prev_mode,
  ...
  if ((prev_mode == kModePlc) &&
      !ReinitAfterPLCs(timestamp_leap) &&
      !MaxWaitForPacket() &&
...
bool DecisionLogicGameVoice::UnderTargetLevel() const {
  return buffer_level_filter_->filtered_current_level() <=
      delay_manager_->TargetLevel();
}

bool DecisionLogicGameVoice::ReinitAfterPLCs(uint32_t timestamp_leap) const {
  return timestamp_leap >=
      static_cast<uint32_t>(output_size_samples_ * kReinitAfterPLCs);
}

bool DecisionLogicGameVoice::PacketTooEarly(uint32_t timestamp_leap) const {
  return timestamp_leap >
      static_cast<uint32_t>(output_size_samples_ * num_consecutive_expands_);
}

bool DecisionLogicGameVoice::MaxWaitForPacket() const {
