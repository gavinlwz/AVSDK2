  ...
  for (size_t channel_ix = 0; channel_ix < num_channels_; ++channel_ix) {
    ...
    if ((!vad.running() &&
      ...
      if (auto_correlation[0] > 0) {
        ...
        if (YoumeWebRtcSpl_LevinsonDurbin(auto_correlation, lpc_coefficients,
      ...
      YoumeWebRtcSpl_FilterMAFastQ12(temp_signal + kVecLen - kResidualLength,
                                fiter_output, lpc_coefficients,
                                kMaxLpcOrder + 1, kResidualLength);
      int32_t residual_energy = YoumeWebRtcSpl_DotProductWithScale(fiter_output,
                                                              fiter_output,
                                                              kResidualLength,
                                                              0);
...
void BackgroundNoise::SetFilterState(size_t channel, const int16_t* input,
                                     size_t length) {
  assert(channel < num_channels_);
  size_t maxLpcOrder = kMaxLpcOrder;
  length = std::min(length, maxLpcOrder);
...
int32_t BackgroundNoise::CalculateAutoCorrelation(
  ...
  int16_t signal_max = YoumeWebRtcSpl_MaxAbsValueW16(signal, length);
  int correlation_scale = kLogVecLen -
      YoumeWebRtcSpl_NormW32(signal_max * signal_max);
  correlation_scale = std::max(0, correlation_scale);

  static const int kCorrelationStep = -1;
  YoumeWebRtcSpl_CrossCorrelation(auto_correlation, signal, signal, length,
...
void BackgroundNoise::SaveParameters(size_t channel,
  ...
  int16_t norm_shift = YoumeWebRtcSpl_NormW32(residual_energy) - 1;
  if (norm_shift & 0x1) {
  ...
  parameters.scale = static_cast<int16_t>(YoumeWebRtcSpl_SqrtFloor(residual_energy));
