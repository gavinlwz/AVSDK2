...
static int16_t CalcLogN(int32_t arg) {
  ...
  zeros=YoumeWebRtcSpl_NormU32(arg);
                          ...
                          int32_t *CurveQ16)
{
  ...
  sh=YoumeWebRtcSpl_NormW32(CorrQ11[1]);
  if (CorrQ11[1]==0) /* Use next correlation */
    sh=YoumeWebRtcSpl_NormW32(CorrQ11[2]);
                              ...
                              uint16_t *CurveQ8)
{
  ...
  res = 1 << (YoumeWebRtcSpl_GetSizeInBits(in_sqrt) >> 1);
                             ...
                             int16_t AvgPitchGain_Q12)
{
  ...
  YoumeWebRtcSpl_ReflCoefToLpc(RCQ15, AR_ORDER, ARCoefQ12);
  ...
  if (AvgPitchGain_Q12 <= 614)
  {
    ...
    {
      gainQ10 = YoumeWebRtcSpl_DivW32W16ResW16(30 << 10,
  ...
  else
  {
    for (k = 0; k < FRAMESAMPLES; k += 4)
    {
      gainQ10 = YoumeWebRtcSpl_DivW32W16ResW16(36 << 10,
                             ...
                             int16_t AvgPitchGain_Q12)
{
  ...
  lft_shft = YoumeWebRtcSpl_NormW32(CorrQ7[0]) - 18;
  ...
  YoumeWebRtcSpl_AutoCorrToReflCoef(CorrQ7_norm, AR_ORDER, RCQ15);
  ...
  if(nrg>131072)
    gain2_Q10 = YoumeWebRtcSpl_DivResultInQ31(FRAMESAMPLES >> 2, nrg);  /* also shifts 31 bits to the left! */
  else
