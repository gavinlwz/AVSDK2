...
NetEqImpl::NetEqImpl(const NetEq::Config& config,
    ...
    : crit_sect_(CriticalSectionWrapper::CreateCriticalSection()),
      ...
      ssrc_(0),
      first_packet_(true),
      first_decoded_packet_(true),
      output_silence_packet_count_(0),
      error_code_(0),
  ...
  last_operation_ = kNormal;
  last_operation_count_ = 0;
...
int NetEqImpl::RegisterPayloadType(NetEqDecoder codec,
  ...
  int ret = decoder_database_->RegisterPayload(rtp_payload_type, codec, fs_hz);
...
int NetEqImpl::InsertPacketInternal(const WebRtcRTPHeader& rtp_header,
  ...
  if ((main_header.ssrc != ssrc_) || first_packet_) {
    ...
    // Allow the decoder to buffer enough data again.
    first_decoded_packet_  = true;
    output_silence_packet_count_ = 0;
  }
...
int NetEqImpl::GetAudioInternal(size_t max_length,
  ...
  if (first_decoded_packet_) {
    size_t bufPacketNum = (packet_buffer_->NumPacketsInBuffer()) << 8; // to Q8, same as target level
    int targetLevel = delay_manager_->TargetLevel();
    if (bufPacketNum < targetLevel) {
      if ((output_silence_packet_count_ < 10) || ((output_silence_packet_count_ % 500) == 0) ){
        LOG_F(LS_INFO) << "Output silent data, bufPacketNum:" << bufPacketNum << ", targetLevel:" << targetLevel;
      }
      output_silence_packet_count_++;
      memset(output, 0, max_length * sizeof(int16_t));
    ...
    LOG_F(LS_INFO) << "Start decoding first packet, bufPacketNum:" << bufPacketNum << ", targetLevel:" << targetLevel;
    first_decoded_packet_ = false;
  }
  ...
  if (last_operation_ != operation) {
    // LOG(LS_VERBOSE) << OperationToName(last_operation_) << " count:" << last_operation_count_;
    // LOG(LS_VERBOSE) << OperationToName(operation) <<
    // ", packets:" << packet_list.size() <<
    // ", packet_buf:" << packet_buffer_->NumPacketsInBuffer() <<
    // ", sync_buf:" << sync_buffer_->FutureLength() <<
    // ", target_level:" << delay_manager_->TargetLevel();
    
    last_operation_ = operation;
    last_operation_count_ = 1;
  } else {
    if (kNormal != operation) {
      // LOG(LS_VERBOSE) << OperationToName(operation) <<
      // ", packets:" << packet_list.size() <<
      // ", packet buffer size:" << packet_buffer_->NumPacketsInBuffer() <<
      // ", sync buffer length:" << sync_buffer_->FutureLength() <<
      // ", target level:" << delay_manager_->TargetLevel();
    }

    last_operation_count_ ++;
  }
    
  if (kNoPacket == operation) {
...
int NetEqImpl::Decode(PacketList* packet_list, Operations* operation,
  ...
  *decoded_length = 0;
  ...
  if ((decision_logic_->UsePlcInsteadOfExpand() == false) && (*operation == kMerge) && decoder && decoder->HasDecodePlc()) {
  ...
  if (*decoded_length < 0) {
    ...
    *decoded_length = 0;
    ...
    if (decision_logic_->UsePlcInsteadOfExpand()) {
      *operation = kPlc;
    } else {
...
int NetEqImpl::DoPlc() {
  while ((sync_buffer_->FutureLength() - expand_->overlap_length()) <
    ...
    if (decoder->HasDecodePlc()) {
      decoded_length = decoder->DecodePlc(1, decoded_buffer_.get());
    }
    if (decoded_length <= 0) {
      break;
    }
    
    algorithm_buffer_->PushBackInterleaved(decoded_buffer_.get(), decoded_length);
    last_mode_ = kModePlc;
...
const char* NetEqImpl::OperationToName(Operations op)
{
  switch (op) {
    case kNormal:
      return "Normal";
    ...
    case kRfc3389Cng:
      return "Rfc3389Cng";
    case kRfc3389CngNoPacket:
      return "Rfc3389CngNoPacket";
    case kCodecInternalCng:
      return "CodecInternalCng";
    case kDtmf:
      return "Dtmf";
    case kAlternativePlc:
      return "AlternativePlc";
    case kAlternativePlcIncreaseTimestamp:
      return "AlternativePlcIncreaseTimestamp";
    case kAudioRepetition:
      return "AudioRepetition";
    case kAudioRepetitionIncreaseTimestamp:
      return "AudioRepetitionIncreaseTimestamp";
    case kPlc:
      return "PLC";
    case kNoPacket:
      return "NoPacket";
