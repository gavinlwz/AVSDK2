...
void WebRtcIlbcfix_Smooth(
  ...
  max1=YoumeWebRtcSpl_MaxAbsValueW16(current, ENH_BLOCKL);
  max2=YoumeWebRtcSpl_MaxAbsValueW16(surround, ENH_BLOCKL);
  maxtot=WEBRTC_SPL_MAX(max1, max2);

  scale=YoumeWebRtcSpl_GetSizeInBits(maxtot);
  scale = (int16_t)(2 * scale) - 26;
  scale=WEBRTC_SPL_MAX(0, scale);

  w00=YoumeWebRtcSpl_DotProductWithScale(current,current,ENH_BLOCKL,scale);
  w11=YoumeWebRtcSpl_DotProductWithScale(surround,surround,ENH_BLOCKL,scale);
  w10=YoumeWebRtcSpl_DotProductWithScale(surround,current,ENH_BLOCKL,scale);
  ...
  bitsw00 = YoumeWebRtcSpl_GetSizeInBits(w00);
  bitsw11 = YoumeWebRtcSpl_GetSizeInBits(w11);
  bitsw10 = YoumeWebRtcSpl_GetSizeInBits(WEBRTC_SPL_ABS_W32(w10));
  scale1 = 31 - bitsw00;
  scale2 = 15 - bitsw11;
  ...
  if (w11prim>64) {
    endiff = YoumeWebRtcSpl_DivW32W16(w00prim, w11prim) << 6;
    C = (int16_t)YoumeWebRtcSpl_SqrtFloor(endiff); /* C is in Q11 */
  } else {
    C = 1;
  }
  ...
  if (errs > crit) {
    ...
    if (w00w00>65536) {
      ...
      /* denom is in Q16 */
      denom = YoumeWebRtcSpl_DivW32W16(endiff, (int16_t)(w00w00 >> 16));
    } else {
      denom = 65536;
    }
    ...
    if( denom > 7){ /* eliminates numerical problems
      ...
      scale=YoumeWebRtcSpl_GetSizeInBits(denom)-15;
      ...
      A = (int16_t)YoumeWebRtcSpl_SqrtFloor(YoumeWebRtcSpl_DivW32W16(num, denomW16));
      ...
      if ((w00prim>0)&&(w10prim>0)) {
        w11_div_w00=YoumeWebRtcSpl_DivW32W16(w10prim, (int16_t)w00prim);

        if (YoumeWebRtcSpl_GetSizeInBits(w11_div_w00)+YoumeWebRtcSpl_GetSizeInBits(A)>31) {
          B_W32 = 0;
        } else {
    ...
    YoumeWebRtcSpl_ScaleAndAddVectors(surround, A, 9,
                                current, B, 14,
                                odata, ENH_BLOCKL);
  }
  return;
