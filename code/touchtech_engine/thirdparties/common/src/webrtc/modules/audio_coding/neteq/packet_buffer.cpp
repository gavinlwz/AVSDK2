...
int PacketBuffer::InsertPacket(Packet* packet) {
  ...
  if ((buffer_.size() > peak_number_of_packets_) && ((buffer_.size() - peak_number_of_packets_) >= 5) ) {
    peak_number_of_packets_ = buffer_.size();
    LOG(LS_INFO) << "Packet buffer peak:" << peak_number_of_packets_;
  }
      
  if (buffer_.size() >= max_number_of_packets_) {
    // Buffer is full. Flush it.
    Flush();
    LOG(LS_WARNING) << "Packet buffer flushed, max_packet_num:" << max_number_of_packets_;
    return_val = kFlushed;
  }
