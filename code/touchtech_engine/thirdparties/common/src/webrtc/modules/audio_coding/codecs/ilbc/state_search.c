...
void WebRtcIlbcfix_StateSearch(
  ...
  max = YoumeWebRtcSpl_MaxAbsValueW16(residual, iLBCenc_inst->state_short_len);
  scaleRes = YoumeWebRtcSpl_GetSizeInBits(max)-12;
  ...
  YoumeWebRtcSpl_MemSetW16(residualLong + iLBCenc_inst->state_short_len, 0, iLBCenc_inst->state_short_len);

  /* Run the Zero-Pole filter (Ciurcular convolution) */
  YoumeWebRtcSpl_MemSetW16(residualLongVec, 0, LPC_FILTERORDER);
  YoumeWebRtcSpl_FilterMAFastQ12(residualLong, sampleMa, numerator,
                            LPC_FILTERORDER + 1,
                            iLBCenc_inst->state_short_len + LPC_FILTERORDER);
  YoumeWebRtcSpl_MemSetW16(&sampleMa[iLBCenc_inst->state_short_len + LPC_FILTERORDER], 0, iLBCenc_inst->state_short_len - LPC_FILTERORDER);
  ...
  maxVal=YoumeWebRtcSpl_MaxAbsValueW16(sampleAr, iLBCenc_inst->state_short_len);
  ...
  YoumeWebRtcSpl_ScaleVectorWithSat(sampleAr, sampleAr, scale,
