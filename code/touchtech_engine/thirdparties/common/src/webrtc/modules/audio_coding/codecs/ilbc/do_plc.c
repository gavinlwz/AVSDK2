...
void WebRtcIlbcfix_DoThePlc(
  ...
  if (PLI == 1) {
    ...
    if (iLBCdec_inst->prevPLI != 1) {
      ...
      max = YoumeWebRtcSpl_MaxAbsValueW16((*iLBCdec_inst).prevResidual,
                                     iLBCdec_inst->blockl);
      scale3 = (YoumeWebRtcSpl_GetSizeInBits(max)<<1) - 25;
      if (scale3 < 0) {
        scale3 = 0;
      }
      ...
      shiftMax = YoumeWebRtcSpl_GetSizeInBits(WEBRTC_SPL_ABS_W32(cross))-15;
      ...
      for (j=inlag-2;j<=inlag+3;j++) {
        ...
        shift1 = YoumeWebRtcSpl_GetSizeInBits(WEBRTC_SPL_ABS_W32(cross_comp))-15;
        crossSquare = (int16_t)((
            (int16_t)WEBRTC_SPL_SHIFT_W32(cross_comp, -shift1) *
            (int16_t)WEBRTC_SPL_SHIFT_W32(cross_comp, -shift1)) >> 15);

        shift2 = YoumeWebRtcSpl_GetSizeInBits(ener)-15;
        measure = (int16_t)WEBRTC_SPL_SHIFT_W32(ener, -shift2) * crossSquare;

        shift3 = YoumeWebRtcSpl_GetSizeInBits(ener_comp)-15;
      ...
      */
      tmp2W32=YoumeWebRtcSpl_DotProductWithScale(&iLBCdec_inst->prevResidual[iLBCdec_inst->blockl-corrLen],
      ...
      if ((tmp2W32>0)&&(ener_comp>0)) {
        ...
        scale1=(int16_t)YoumeWebRtcSpl_NormW32(tmp2W32)-16;
        tmp1=(int16_t)WEBRTC_SPL_SHIFT_W32(tmp2W32, scale1);

        scale2=(int16_t)YoumeWebRtcSpl_NormW32(ener)-16;
        ...
        nom = tmp1 * tmp2;
        max_perSquare = (int16_t)YoumeWebRtcSpl_DivW32W16(nom, denom);
