...
int WebRtcIsac_DecodeSpec(Bitstr* streamdata, int16_t AvgPitchGain_Q12,
  ...
  res = 1 << (YoumeWebRtcSpl_GetSizeInBits(invARSpec2_Q16[0]) >> 1);
  ...
  switch (band) {
    case kIsacLowerBand: {
      ...
      for (k = 0; k < FRAMESAMPLES; k += 4) {
        gainQ10 = YoumeWebRtcSpl_DivW32W16ResW16(p1, (int16_t)(
...
int WebRtcIsac_EncodeSpec(const int16_t* fr, const int16_t* fi,
  ...
  shift_var = YoumeWebRtcSpl_NormW32(CorrQ7[0]) - 18;
  ...
  gain2_Q10 = YoumeWebRtcSpl_DivResultInQ31(FRAMESAMPLES_QUARTER, nrg);
