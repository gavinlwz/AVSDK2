...
void WebRtcIlbcfix_DecodeResidual(
  ...
  if (iLBC_encbits->state_first) { /* put adaptive part in the end */
    ...
    YoumeWebRtcSpl_MemSetW16(mem, 0, CB_MEML - iLBCdec_inst->state_short_len);
    ...
    YoumeWebRtcSpl_MemCpyReversedOrder(mem+CB_MEML-1,
    ...
    YoumeWebRtcSpl_MemCpyReversedOrder(&decresidual[start_pos-1],
  ...
  if (iLBC_encbits->startIdx > 1) {
    ...
    /* get decoded residual from reversed vector */
    YoumeWebRtcSpl_MemCpyReversedOrder(decresidual+SUBL*Nback-1,
