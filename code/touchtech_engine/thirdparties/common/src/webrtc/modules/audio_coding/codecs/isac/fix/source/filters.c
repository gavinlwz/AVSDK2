...
int WebRtcIsacfix_AutocorrC(int32_t* __restrict r,
  ...
  if(temp == 0) {
    ...
    scaling = 32 - YoumeWebRtcSpl_NormU32(temp);
  }
                                  ...
                                  int32_t          *FilterState) //Q16
{
  ...
  for (j=0; j<ALLPASSSECTIONS; j++) {
    for (n=0;n<lengthInOut;n+=2){
      ...
      a <<= 1;  // Q15 -> Q16
      b = YoumeWebRtcSpl_AddSatW32(a, FilterState[j]);  //Q16+Q16=Q16
      ...
      FilterState[j] = YoumeWebRtcSpl_AddSatW32(a << 1, (uint32_t)InOut16[n] << 16);
                                     ...
                                     int16_t *out)             /* array of size N/2 */
{
  ...
  for (n=0;n<N/2;n++) {
    out[n] = YoumeWebRtcSpl_AddSatW16(data_vec[2 * n], data_vec[2 * n + 1]);
  }
}
