...
int Normal::Process(const int16_t* input,
  ...
  const int fs_shift = 30 - YoumeWebRtcSpl_NormW32(fs_mult);
  ...
  if (last_mode == kModeExpand) {
    ...
    for (size_t channel_ix = 0; channel_ix < output->Channels(); ++channel_ix) {
      ...
      int16_t decoded_max =
          YoumeWebRtcSpl_MaxAbsValueW16(signal, length_per_channel);
      // Adjust muting factor if needed (to BGN level).
      size_t energy_length =
          std::min(static_cast<size_t>(fs_mult * 64), length_per_channel);
      int scaling = 6 + fs_shift
          - YoumeWebRtcSpl_NormW32(decoded_max * decoded_max);
      scaling = std::max(scaling, 0);  // |scaling| should always be >= 0.
      int32_t energy = YoumeWebRtcSpl_DotProductWithScale(signal, signal,
      ...
      if ((energy != 0) &&
        ...
        scaling = YoumeWebRtcSpl_NormW32(energy) - 16;
        ...
        int32_t ratio = YoumeWebRtcSpl_DivW32W16(bgn_energy, energy_scaled);
        mute_factor = YoumeWebRtcSpl_SqrtFloor(ratio << 14);
      } else {
