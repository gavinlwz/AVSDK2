...
int Expand::Process(AudioMultiVector* output) {
  ...
  static const size_t kNoiseLpcOrder = kMaxLpcOrder; // BackgroundNoise::kMaxLpcOrder;
  ...
  for (size_t channel_ix = 0; channel_ix < num_channels_; ++channel_ix) {
    ...
    if (current_lag_index_ == 0) {
      ...
      YoumeWebRtcSpl_ScaleAndAddVectorsWithRound(
...
//      expand_lags_[2] = fs_mult * 120;
    ...
    YoumeWebRtcSpl_AffineTransformVector(scaled_random_vector, random_vector,
                                    parameters.ar_gain, add_constant,
                                    parameters.ar_gain_scale,
                                    current_lag);
    YoumeWebRtcSpl_FilterARFastQ12(scaled_random_vector, unvoiced_vector,
    ...
    int temp_shift =
        (31 - YoumeWebRtcSpl_NormW32(rtc::checked_cast<int32_t>(max_lag_))) - 5;
    ...
    if ((consecutive_expands_ != 0) || !parameters.onset) {
      ...
      YoumeWebRtcSpl_AffineTransformVector(temp_data, temp_data,
...
void Expand::AnalyzeSignal(int16_t* random_vector) {
  ...
  for (size_t i = 0; i < kNumCorrelationCandidates; i++) {
    ...
    distortion_scale = std::max(16 - YoumeWebRtcSpl_NormW32(best_distortion_w32[i]),
                                distortion_scale);
  }
  // Shift the distortion values to fit in 16 bits.
  YoumeWebRtcSpl_VectorBitShiftW32ToW16(best_distortion, kNumCorrelationCandidates,
  ...
  for (size_t channel_ix = 0; channel_ix < num_channels_; ++channel_ix) {
    ...
    int16_t signal_max = YoumeWebRtcSpl_MaxAbsValueW16(
        &audio_history[signal_length - correlation_length - start_index
                       - correlation_lags],
                       correlation_length + start_index + correlation_lags - 1);
    correlation_scale = (31 - YoumeWebRtcSpl_NormW32(signal_max * signal_max)) +
        (31 - YoumeWebRtcSpl_NormW32(static_cast<int32_t>(correlation_length))) - 31;
    correlation_scale = std::max(0, correlation_scale);

    // Calculate the correlation, store in |correlation_vector2|.
    YoumeWebRtcSpl_CrossCorrelation(
        correlation_vector2,
    ...
    best_index = YoumeWebRtcSpl_MaxIndexW32(correlation_vector2, correlation_lags);
    ...
    // Calculate energies.
    int32_t energy1 = YoumeWebRtcSpl_DotProductWithScale(
        &(audio_history[signal_length - correlation_length]),
        &(audio_history[signal_length - correlation_length]),
        correlation_length, correlation_scale);
    int32_t energy2 = YoumeWebRtcSpl_DotProductWithScale(
    ...
    if ((energy1 > 0) && (energy2 > 0)) {
      int energy1_scale = std::max(16 - YoumeWebRtcSpl_NormW32(energy1), 0);
      int energy2_scale = std::max(16 - YoumeWebRtcSpl_NormW32(energy2), 0);
      ...
      int16_t sqrt_energy_product = static_cast<int16_t>(
          YoumeWebRtcSpl_SqrtFloor(scaled_energy1 * scaled_energy2));
      ...
      corr_coefficient = YoumeWebRtcSpl_DivW32W16(max_correlation,
                                             sqrt_energy_product);
      // Cap at 1.0 in Q14.
    ...
    energy1 = YoumeWebRtcSpl_DotProductWithScale(vector1, vector1, expansion_length,
                                            correlation_scale);
    energy2 = YoumeWebRtcSpl_DotProductWithScale(vector2, vector2, expansion_length,
                                            correlation_scale);
    ...
    if ((energy1 / 4 < energy2) && (energy1 > energy2 / 4)) {
      ...
      // accordingly.
      int32_t scaled_energy2 = std::max(16 - YoumeWebRtcSpl_NormW32(energy2), 0);
      int32_t scaled_energy1 = scaled_energy2 - 13;
      // Calculate scaled_energy1 / scaled_energy2 in Q13.
      int32_t energy_ratio = YoumeWebRtcSpl_DivW32W16(
      ...
      amplitude_ratio =
          static_cast<int16_t>(YoumeWebRtcSpl_SqrtFloor(energy_ratio << 13));
      ...
      YoumeWebRtcSpl_AffineTransformVector(&parameters.expand_vector1[0],
    ...
    correlation_scale = YoumeWebRtcSpl_MaxAbsValueW16(
        &(audio_history[signal_length - fs_mult_lpc_analysis_len]),
        fs_mult_lpc_analysis_len);

    correlation_scale = std::min(16 - YoumeWebRtcSpl_NormW32(correlation_scale), 0);
    ...
    YoumeWebRtcSpl_CrossCorrelation(auto_correlation,
    ...
    if (auto_correlation[0] > 0) {
      ...
      int16_t stability = YoumeWebRtcSpl_LevinsonDurbin(auto_correlation,
      ...
      if (stability != 1) {
        ...
        YoumeWebRtcSpl_MemSetW16(parameters.ar_filter + 1, 0, kUnvoicedLpcOrder);
      }
    }
    ...
    memcpy(unvoiced_vector - kUnvoicedLpcOrder,
           ...
           sizeof(int16_t) * kUnvoicedLpcOrder);
    YoumeWebRtcSpl_FilterMAFastQ12(&audio_history[signal_length - 128],
                              unvoiced_vector,
                              parameters.ar_filter,
                              kUnvoicedLpcOrder + 1,
                              128);
    int16_t unvoiced_prescale;
    if (YoumeWebRtcSpl_MaxAbsValueW16(unvoiced_vector, 128) > 4000) {
      unvoiced_prescale = 4;
    } else {
      unvoiced_prescale = 0;
    }
    int32_t unvoiced_energy = YoumeWebRtcSpl_DotProductWithScale(unvoiced_vector,
                                                            unvoiced_vector,
                                                            128,
                                                            unvoiced_prescale);

    // Normalize |unvoiced_energy| to 28 or 29 bits to preserve sqrt() accuracy.
    int16_t unvoiced_scale = YoumeWebRtcSpl_NormW32(unvoiced_energy) - 3;
    ...
    int16_t unvoiced_gain =
        static_cast<int16_t>(YoumeWebRtcSpl_SqrtFloor(unvoiced_energy));
    ...
    if (slope > 12288) {
      ...
      int temp_ratio = YoumeWebRtcSpl_DivW32W16(
          (slope - 8192) << 12,
      ...
      parameters.mute_slope = YoumeWebRtcSpl_DivW32W16(
...
void Expand::Correlation(const int16_t* input,
  ...
  int16_t max_value = YoumeWebRtcSpl_MaxAbsValueW16(downsampled_input,
                                               kDownsampledLength);
  int16_t norm_shift = 16 - YoumeWebRtcSpl_NormW32(max_value);
  YoumeWebRtcSpl_VectorBitShiftW16(downsampled_input, kDownsampledLength,
  ...
  int32_t max_correlation = YoumeWebRtcSpl_MaxAbsValueW32(correlation,
                                                     kNumCorrelationLags);
  int16_t norm_shift2 = static_cast<int16_t>(
      std::max(18 - YoumeWebRtcSpl_NormW32(max_correlation), 0));
  YoumeWebRtcSpl_VectorBitShiftW32ToW16(output, kNumCorrelationLags, correlation,
                                   norm_shift2);
...
void Expand::GenerateBackgroundNoise(int16_t* random_vector,
  ...
  if (background_noise_->initialized()) {
    ...
    YoumeWebRtcSpl_FilterARFastQ12(scaled_random_vector, noise_samples,
    ...
    if (bgn_mode == NetEq::kBgnFade && too_many_expands &&
      ...
      if (!stop_muting_ && bgn_mode != NetEq::kBgnOff &&
        ...
        YoumeWebRtcSpl_AffineTransformVector(noise_samples, noise_samples,
