...
int16_t Merge::SignalScaling(const int16_t* input, size_t input_length,
  ...
  *expanded_max = YoumeWebRtcSpl_MaxAbsValueW16(expanded_signal, mod_input_length);
  *input_max = YoumeWebRtcSpl_MaxAbsValueW16(input, mod_input_length);

  // Calculate energy of expanded signal.
  // |log_fs_mult| is log2(fs_mult_), but is not exact for 48000 Hz.
  int log_fs_mult = 30 - YoumeWebRtcSpl_NormW32(fs_mult_);
  int expanded_shift = 6 + log_fs_mult
      - YoumeWebRtcSpl_NormW32(*expanded_max * *expanded_max);
  expanded_shift = std::max(expanded_shift, 0);
  int32_t energy_expanded = YoumeWebRtcSpl_DotProductWithScale(expanded_signal,
                                                          expanded_signal,
                                                          mod_input_length,
                                                          expanded_shift);
  ...
  int input_shift = 6 + log_fs_mult -
      YoumeWebRtcSpl_NormW32(*input_max * *input_max);
  input_shift = std::max(input_shift, 0);
  int32_t energy_input = YoumeWebRtcSpl_DotProductWithScale(input, input,
                                                       mod_input_length,
                                                       input_shift);
  ...
  if (energy_input > energy_expanded) {
    ...
    int16_t temp_shift = YoumeWebRtcSpl_NormW32(energy_input) - 17;
    ...
    mute_factor = static_cast<int16_t>(
        YoumeWebRtcSpl_SqrtFloor((energy_expanded / energy_input) << 14));
  } else {
...
void Merge::Downsample(const int16_t* input, size_t input_length,
  ...
  YoumeWebRtcSpl_DownsampleFast(&expanded_signal[signal_offset],
  ...
  if (input_length <= length_limit) {
    ...
    YoumeWebRtcSpl_DownsampleFast(&input[signal_offset], temp_len,
    ...
    YoumeWebRtcSpl_DownsampleFast(&input[signal_offset],
...
size_t Merge::CorrelateAndPeakSearch(int16_t expanded_max, int16_t input_max,
  ...
  YoumeWebRtcSpl_CrossCorrelation(correlation, input_downsampled_,
  ...
  int norm_shift = std::max(0, 17 - YoumeWebRtcSpl_NormW32(max_correlation));
  YoumeWebRtcSpl_VectorBitShiftW32ToW16(correlation_ptr, stop_position_downsamp,
