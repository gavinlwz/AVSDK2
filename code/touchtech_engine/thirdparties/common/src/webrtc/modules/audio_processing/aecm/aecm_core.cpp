...
AecmCore* WebRtcAecm_CreateCore() {
    ...
    aecm->farFrameBuf = WebRtc_youme_CreateBuffer(FRAME_LEN + PART_LEN,
                                            sizeof(int16_t));
    if (!aecm->farFrameBuf)
    {
    ...
    aecm->nearNoisyFrameBuf = WebRtc_youme_CreateBuffer(FRAME_LEN + PART_LEN,
                                                  sizeof(int16_t));
    ...
    aecm->nearCleanFrameBuf = WebRtc_youme_CreateBuffer(FRAME_LEN + PART_LEN,
                                                  sizeof(int16_t));
    ...
    aecm->outFrameBuf = WebRtc_youme_CreateBuffer(FRAME_LEN + PART_LEN,
                                            sizeof(int16_t));
    if (!aecm->outFrameBuf)
    {
        WebRtcAecm_FreeCore(aecm);
    ...
    aecm->real_fft = YoumeWebRtcSpl_CreateRealFFT(PART_LEN_SHIFT);
...
int WebRtcAecm_InitCore(AecmCore* const aecm, int samplingFreq) {
    ...
    WebRtc_youme_InitBuffer(aecm->farFrameBuf);
    WebRtc_youme_InitBuffer(aecm->nearNoisyFrameBuf);
    WebRtc_youme_InitBuffer(aecm->nearCleanFrameBuf);
    WebRtc_youme_InitBuffer(aecm->outFrameBuf);
...
#ifdef __cplusplus
    static_assert(PART_LEN % 16 == 0, "PART_LEN % 16 != 0");
#else
...
void WebRtcAecm_FreeCore(AecmCore* aecm)
{
    ...
    WebRtc_youme_FreeBuffer(aecm->farFrameBuf);
    WebRtc_youme_FreeBuffer(aecm->nearNoisyFrameBuf);
    WebRtc_youme_FreeBuffer(aecm->nearCleanFrameBuf);
    WebRtc_youme_FreeBuffer(aecm->outFrameBuf);

    WebRtc_FreeDelayEstimator(aecm->delay_estimator);
    WebRtc_FreeDelayEstimatorFarend(aecm->delay_estimator_farend);
    YoumeWebRtcSpl_FreeRealFFT(aecm->real_fft);
...
int WebRtcAecm_ProcessFrame(AecmCore* aecm,
    ...
    WebRtc_youme_WriteBuffer(aecm->farFrameBuf, farFrame, FRAME_LEN);
    WebRtc_youme_WriteBuffer(aecm->nearNoisyFrameBuf, nearendNoisy, FRAME_LEN);
    if (nearendClean != NULL)
    {
        WebRtc_youme_WriteBuffer(aecm->nearCleanFrameBuf, nearendClean, FRAME_LEN);
    }

    // Process as many blocks as possible.
    while (WebRtc_youme_available_read(aecm->farFrameBuf) >= PART_LEN)
    {
        ...
        WebRtc_youme_ReadBuffer(aecm->farFrameBuf, (void**) &far_block_ptr, far_block,
                          PART_LEN);
        WebRtc_youme_ReadBuffer(aecm->nearNoisyFrameBuf,
        ...
        if (nearendClean != NULL)
        {
            ...
            WebRtc_youme_ReadBuffer(aecm->nearCleanFrameBuf,
        ...
        WebRtc_youme_WriteBuffer(aecm->outFrameBuf, outBlock, PART_LEN);
    }

    // Stuff the out buffer if we have less than a frame to output.
    // This should only happen for the first frame.
    size = (int) WebRtc_youme_available_read(aecm->outFrameBuf);
    if (size < FRAME_LEN)
    {
        WebRtc_youme_MoveReadPtr(aecm->outFrameBuf, size - FRAME_LEN);
    }

    // Obtain an output frame.
    WebRtc_youme_ReadBuffer(aecm->outFrameBuf, (void**) &out_ptr, out, FRAME_LEN);
    if (out_ptr != out) {
...
static int16_t LogOfEnergyInQ8(uint32_t energy, int q_domain) {
  ...
  if (energy > 0) {
    int zeros = YoumeWebRtcSpl_NormU32(energy);
...
///////////////////////////////////////////////////////////////////////////////
// WebRtcAecm_ReCalcSetChEnergies()
//
// This function re-calculates the log of energies for
// estimated echoes
//
// Inputs:
...
//
// Output:
void WebRtcAecm_ReCalcSetChEnergies(AecmCore* aecm,
                                    const uint16_t* far_spectrum,
                                    const int16_t far_q)
{
    int i   = 0;
    uint32_t u4EstEnergyStore = 0;
    uint32_t u4EstEnergyAdapt = 0;
    
    for (i = 0; i < PART_LEN1; i++) {
        u4EstEnergyStore += (uint32_t)(WEBRTC_SPL_MUL_16_U16(aecm->channelStored[i], far_spectrum[i]));
        u4EstEnergyAdapt += (uint32_t)(WEBRTC_SPL_MUL_16_U16(aecm->channelAdapt16[i], far_spectrum[i]));
    }
    
    aecm->echoAdaptLogEnergy[0]  = LogOfEnergyInQ8(u4EstEnergyAdapt, RESOLUTION_CHANNEL16 + far_q);
    aecm->echoStoredLogEnergy[0] = LogOfEnergyInQ8(u4EstEnergyStore, RESOLUTION_CHANNEL16 + far_q);
}
...
int16_t WebRtcAecm_CalcStepSize(AecmCore* const aecm) {
    ...
    } else if (aecm->startupState > 0)
    {
        ...
        } else
        {
            ...
            tmp32 = tmp16 * MU_DIFF;
            tmp32 = YoumeWebRtcSpl_DivW32W16(tmp32, aecm->farEnergyMaxMin);
...
void WebRtcAecm_UpdateChannel(AecmCore* aecm,
    ...
    if (mu)
    {
        for (i = 0; i < PART_LEN1; i++)
        {
            ...
            // multiplication
            zerosCh = YoumeWebRtcSpl_NormU32(aecm->channelAdapt32[i]);
            zerosFar = YoumeWebRtcSpl_NormU32((uint32_t)far_spectrum[i]);
            ...
            zerosNum = YoumeWebRtcSpl_NormU32(tmpU32no1);
            if (dfa[i])
            {
                zerosDfa = YoumeWebRtcSpl_NormU32((uint32_t)dfa[i]);
            } else
            {
                zerosDfa = 32;
            }
            ...
            zerosNum = YoumeWebRtcSpl_NormW32(tmp32no1);
            if ((tmp32no1) && (far_spectrum[i] > (CHANNEL_VAD << far_q)))
            {
                ...
                tmp32no2 = YoumeWebRtcSpl_DivW32W16(tmp32no2, i + 1);
                // Make sure we are in the right Q-domain
                shift2ResChan = shiftNum + shiftChFar - xfaQ - mu - ((30 - zerosFar) << 1);
                if (YoumeWebRtcSpl_NormW32(tmp32no2) < shift2ResChan)
                {
                ...
                aecm->channelAdapt32[i] =
                    YoumeWebRtcSpl_AddSatW32(aecm->channelAdapt32[i], tmp32no2);
    ...
    } else
    {
        ...
        if (dE < ENERGY_DEV_TOL)
        {
            ...
            if (dE < SUPGAIN_EPC_DT)
            {
                ...
                tmp16no1 = (int16_t)YoumeWebRtcSpl_DivW32W16(tmp32no1, SUPGAIN_EPC_DT);
            ...
            } else
            {
                ...
                tmp16no1 = (int16_t)YoumeWebRtcSpl_DivW32W16(tmp32no1, (ENERGY_DEV_TOL
                        - SUPGAIN_EPC_DT));
