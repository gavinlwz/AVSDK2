...
static void UpdateNoiseEstimateNeon(NoiseSuppressionFixedC* inst, int offset) {
  ...
  int16_t tmp16 = YoumeWebRtcSpl_MaxValueW16(inst->noiseEstLogQuantile + offset,
                                        inst->magnLen);
  ...
  for (ptr_noiseEstLogQuantile = &inst->noiseEstLogQuantile[offset],
    ...
    // tmp16 = YoumeWebRtcSpl_SatW32ToW16(tmp32no1);
  ...
  *ptr_noiseEstQuantile = YoumeWebRtcSpl_SatW32ToW16(tmp32no1);
}
    ...
    for (i = 0; i + 7 < inst->magnLen; i += 8) {
      ...
      for (j = 0; j < 8; j++) {
        if (inst->noiseEstDensity[offset + i + j] > 512) {
          ...
          int factor = YoumeWebRtcSpl_NormW16(inst->noiseEstDensity[offset + i + j]);
