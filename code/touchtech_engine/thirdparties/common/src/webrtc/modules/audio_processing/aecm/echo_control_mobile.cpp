...
void* WebRtcAecm_Create() {
    ...
    aecm->farendBuf = WebRtc_youme_CreateBuffer(kBufSizeSamp,
                                          sizeof(int16_t));
    if (!aecm->farendBuf)
    {
        WebRtcAecm_Free(aecm);
...
void WebRtcAecm_Free(void* aecmInst) {
  AecMobile* aecm = (AecMobile*)aecmInst;
    ...
    WebRtc_youme_FreeBuffer(aecm->farendBuf);
    free(aecm);
}
...
int32_t WebRtcAecm_Init(void *aecmInst, int32_t sampFreq)
{
  AecMobile* aecm = (AecMobile*)aecmInst;
    ...
    WebRtc_youme_InitBuffer(aecm->farendBuf);
...
int32_t WebRtcAecm_BufferFarend(void *aecmInst, const int16_t *farend,
  ...
  WebRtc_youme_WriteBuffer(aecm->farendBuf, farend, nrOfSamples);
                           ...
                           size_t nrOfSamples, int16_t msInSndCardBuf)
{
  AecMobile* aecm = (AecMobile*)aecmInst;
    ...
    if (aecm->ECstartup)
    {
        ...
        nmbrOfFilledBuffers =
            (short) WebRtc_youme_available_read(aecm->farendBuf) / FRAME_LEN;
        ...
        if (!aecm->checkBuffSize)
        {
            ...
            {
                if ((aecm->bufSizeStart * FRAME_LEN) > (aecm->msInSndCardBuf * nrOfSamples / 10)) {
                    WebRtc_youme_MoveReadPtr(aecm->farendBuf,
                                       (int) WebRtc_youme_available_read(aecm->farendBuf)
                                       - (int) aecm->bufSizeStart * FRAME_LEN);
                } else {
                    WebRtc_youme_MoveReadPtr(aecm->farendBuf,
                                       (int) WebRtc_youme_available_read(aecm->farendBuf)
                                       - (int) aecm->msInSndCardBuf * nrOfSamples / 10);
                }
                aecm->ECstartup = 0;
            }
        }
    ...
    } else
    {
        ...
        for (i = 0; i < nFrames; i++)
        {
            ...
            if (nmbrOfFilledBuffers > 0)
            {
                ...
                WebRtc_youme_ReadBuffer(aecm->farendBuf, (void**) &farend_ptr, farend,
                                  FRAME_LEN);
...
#ifdef AEC_DEBUG
    msInAECBuf = (short) WebRtc_youme_available_read(aecm->farendBuf) /
...
static int WebRtcAecm_EstBufDelay(AecMobile* aecm, short msInSndCardBuf) {
    ...
    short nSampFar = (short) WebRtc_youme_available_read(aecm->farendBuf);
    short diff;
    ...
    {
        WebRtc_youme_MoveReadPtr(aecm->farendBuf, FRAME_LEN);
        delayNew += FRAME_LEN;
    }
...
static int WebRtcAecm_DelayComp(AecMobile* aecm) {
    int nSampFar = (int) WebRtc_youme_available_read(aecm->farendBuf);
...
//                FRAME_LEN));
        ...
        // paul: I don't know why here nSampSndCard divide by 2 aboved, so I remove it
        nSampAdd = (int)(WEBRTC_SPL_MAX(delayNew, FRAME_LEN));
        
        nSampAdd = WEBRTC_SPL_MIN(nSampAdd, maxStuffSamp);

        WebRtc_youme_MoveReadPtr(aecm->farendBuf, -nSampAdd);
