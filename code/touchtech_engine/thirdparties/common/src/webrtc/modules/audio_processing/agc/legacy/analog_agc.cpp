...
//        17367};
static const int16_t kOffset1[8] = {21895, 20411, 18706, 17237, 16112, 15305, 14451,
    13867};
...
#define AGC_DEFAULT_TARGET_LEVEL 12
#define AGC_DEFAULT_COMP_GAIN 18
...
int WebRtcAgc_AddMic(void *state, int16_t* in_mic, size_t num_bands,
                     size_t samples)
{
    ...
    for (i = 0; i < kNumSubframes / 2; i++)
    {
        if (stt->fs == 16000)
        {
            YoumeWebRtcSpl_DownsampleBy2(&in_mic[i * 32],
                                    32,
                                    tmp_speech,
                                    stt->filterState);
        } else
        ...
        ptr[i] = YoumeWebRtcSpl_DotProductWithScale(tmp_speech, tmp_speech, 16, 4);
    }
...
int WebRtcAgc_VirtualMic(void *agcInst, int16_t* in_near,
                         ...
                         int32_t *micLevelOut)
{
    ...
    frameNrg = (uint32_t)(in_near[0] * in_near[0]);
    for (sampleCntr = 1; sampleCntr < samples; sampleCntr++)
    {
        ...
        {
          nrg = (uint32_t)(in_near[sampleCntr] * in_near[sampleCntr]);
          frameNrg += nrg;
        }
    ...
    micLevelTmp = micLevelIn << stt->scale; // =0 in digital adaptive mode
    /* Set desired level */
    gainIdx = stt->micVol; // micVol is the INDEX!!
    if (stt->micVol > stt->maxAnalog)
    {
    ...
    // Pre-Gain in mic channel, base on the index: gainIdx = stt->micVol!!
...
void WebRtcAgc_UpdateAgcThresholds(LegacyAgc* stt) {
    ...
    if (stt->micLvlSat)
    {
        ...
        zeros = YoumeWebRtcSpl_NormW32(stt->Rxx160_LPw32);
    ...
    tmp16 = YoumeWebRtcSpl_DivW32W16ResW16((int32_t)tmp16, ANALOG_TARGET_LEVEL);
    ...
    stt->analogTargetLevel = RXX_BUFFER_LEN * kTargetLevelTable[stt->targetIdx + 3]; /* ex. -20 dBov */
    stt->startUpperLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->targetIdx + 2];/* -19 dBov */
    stt->startLowerLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->targetIdx + 4];/* -21 dBov */
    ...
    stt->upperSecondaryLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->targetIdx - 2];/* -15 dBov */
    stt->lowerSecondaryLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->targetIdx + 8];/* -25 dBov */
    stt->upperLimit = stt->startUpperLimit;
    stt->lowerLimit = stt->startLowerLimit;
    stt->i2NewIdx = stt->targetIdx;
}
...
void WebRtcAgc_SaturationCtrl(LegacyAgc* stt,
    ...
    // stt->env[] is the sum-power(a0^2+a1^2+...) of the subframe.
    for (i = 0; i < 10; i++)
    {
                                ...
                                int16_t echo, uint8_t *saturationWarning)
{
    ...
    if ((inMicLevelTmp != stt->micVol) && (inMicLevelTmp < stt->minOutput)) // stt->minOutput is fix to 4% higher than 0(0 ~ 318)
    {
        tmp32 = ((stt->maxLevel - stt->minLevel) * 51) >> 9;
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        Rxx16w32 = stt->Rxx16w32_array[0][i];  // Q-4

        /* Rxx160w32 in Q(-7) */
        tmp32 = (Rxx16w32 - stt->Rxx16_vectorw32[stt->Rxx16pos]) >> 3; // Q-7
        stt->Rxx160w32 = stt->Rxx160w32 + tmp32;                       // Q-7
        stt->Rxx16_vectorw32[stt->Rxx16pos] = Rxx16w32;                // Q-7
        ...
        /* Rxx16_LPw32 in Q(-4) */
        tmp32 = (Rxx16w32 - stt->Rxx16_LPw32) >> kAlphaShortTerm;      // Q-4
        stt->Rxx16_LPw32 = (stt->Rxx16_LPw32) + tmp32;                 // Q-4
        ...
        if (vadLogRatio > stt->vadThreshold)
        {
            ...
            if (stt->activeSpeech < 250)
            {
                ...
                if (stt->Rxx16_LPw32 > stt->Rxx16_LPw32Max)            // Q-4
                {
                    stt->Rxx16_LPw32Max = stt->Rxx16_LPw32;            // Q-4
                }
            } else if (stt->activeSpeech == 250)
            {
                stt->activeSpeech += 2;
                tmp32 = stt->Rxx16_LPw32Max >> 3;                      // Q-7
                //stt->Rxx160_LPw32 = tmp32 * RXX_BUFFER_LEN;            // Q-7
            }

            tmp32 = (stt->Rxx160w32 - stt->Rxx160_LPw32) >> kAlphaLongTerm; // Q-7
            stt->Rxx160_LPw32 += tmp32;
            ...
            if (stt->Rxx160_LPw32 > stt->upperSecondaryLimit)
            {
                ...
                if ((stt->msTooHigh > stt->msecSpeechOuterChange) && (stt->i2PullDownCnt < 1))
                {
                    stt->msTooHigh = 0;

                    /* Lower the recording level */
                    /* Multiply by 0.75 which corresponds to decreasing ~0.8dB */
                    stt->Rxx160_LPw32 = (stt->Rxx160_LPw32 / 64) * 48;
            ...
            } else if ((stt->Rxx160_LPw32 > stt->upperLimit) && (stt->i2PullDownCnt < 1))
            {
                stt->msTooHigh += 2;
                stt->msTooLow = 0;
                stt->changeToSlowMode = 0;
            ...
            } else if ((stt->Rxx160_LPw32 < stt->lowerSecondaryLimit) && (stt->i2PullUpCnt < 1))
            {
                stt->msTooHigh = 0;
                stt->changeToSlowMode = 0;
                stt->msTooLow += 2;
                ...
                if (stt->msTooLow > stt->msecSpeechOuterChange)
                {
                    ...
                    weightFIX = kOffset1[index] -
                        (int16_t)((kSlope1[index] * volNormFIX) >> 13); // Q14
                    if (weightFIX < 16384) {
                        weightFIX = 16384;
                    }
                    
                    /* stt->Rxx160_LPw32 *= 1.2656 [~0.2 dB]; */
                    stt->Rxx160_LPw32 = (stt->Rxx160_LPw32 / 64) * 81;
            ...
            } else if ((stt->Rxx160_LPw32 < stt->lowerLimit) && (stt->i2PullUpCnt < 1))
            {
                stt->msTooHigh = 0;
                stt->changeToSlowMode = 0;
                stt->msTooLow += 2;
...
#endif
            if ((stt->i2PullUpCnt >= 1) && (!stt->i2PullUpLock)) {
                // Reset Counter
                stt->i2PullDownCnt = 0;
                stt->i2PullUpLock = 1;
                stt->i2PullDownLock = 0;
                // Means the mic volume is too low, Fine tune the threshold DOWN!!
                stt->i2NewIdx += 1;
                if ((stt->i2NewIdx >= 0) && (stt->i2NewIdx < 64)) {
                    stt->upperLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx + 2];
                    stt->lowerLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx + 4];
                    stt->upperSecondaryLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx - 2];
                    stt->lowerSecondaryLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx + 8];
                }
            } else if ((stt->i2PullDownCnt >= 1) && (!stt->i2PullDownLock)) {
                // Reset Counter
                stt->i2PullUpCnt = 0;
                stt->i2PullDownLock = 1;
                stt->i2PullUpLock = 0;
                // Means the mic volume is too high, Fine tune the threshold UP!!
                stt->i2NewIdx -= 1;
                if ((stt->i2NewIdx >= 0) && (stt->i2NewIdx < 64)) {
                    stt->upperLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx + 2];
                    stt->lowerLimit = RXX_BUFFER_LEN * kTargetLevelTable[stt->i2NewIdx + 4];
            ...
            if (stt->micVol < 127) {
                stt->i4LowLevelTimeCnt += 2;
            } else {
                stt->i4LowLevelTimeCnt = 0;
            }
            
            if (stt->i4LowLevelTimeCnt >= 11500) { // wait 11.5s
                stt->i4LowLevelTimeCnt = 0;
                stt->micVol = 127;
            }
        }
                   ...
                   int16_t agcMode, uint32_t fs)
{
    ...
    tmpNorm = YoumeWebRtcSpl_NormU32((uint32_t)maxLevel);
    ...
    YoumeWebRtcSpl_MemSetW32(stt->filterState, 0, 8);
