...
#define YOUME_RESTRICT_LOGGING_PRECONDITION(sev)  \
  sev < webrtc::LS_INFO ? (void) 0 :
#else
#define YOUME_RESTRICT_LOGGING_PRECONDITION(sev)
#endif

#define YOUME_LOG_SEVERITY_PRECONDITION(sev) \
  YOUME_RESTRICT_LOGGING_PRECONDITION(sev) !(webrtc::LogMessage::Loggable(sev)) \
    ? (void) 0 \
    : webrtc::LogMessageVoidify() &

#define YM_LOG(sev) \
  YOUME_LOG_SEVERITY_PRECONDITION(webrtc::sev) \
    webrtc::LogMessage(__FILE__, __LINE__, webrtc::sev).stream()

//#define LOG_TAG(sev, tag)  YM_LOG(sev)
...
#define LOG_F(sev) YM_LOG(sev) << __PRETTY_FUNCTION__ << ": "
#else
#define LOG_F(sev) YM_LOG(sev) << __FUNCTION__ << ": "
#endif
#endif
...
//    << ", " << #v3 << "=" << v3
//
//#define LOG_FERR0(sev, func) YM_LOG(sev) << #func << " failed"
