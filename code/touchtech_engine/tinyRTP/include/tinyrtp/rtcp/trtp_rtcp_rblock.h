...
#define TRTP_RTCP_RBLOCK_SIZE 36 	// 相对于标准，youme rtcp rblock分别统计audio和video qos字段，比标准多12byte
...
typedef struct trtp_rtcp_rblock_s {
    ...
    unsigned int audio_fraction:8;		/* audio fraction lost since last SR/RR */
    int audio_cumulative_no_lost:24;  	/* audio cumul. no. pkts lost (signed!) */
    uint32_t audio_last_seq;			/* audio extended last seq. no. received */
    uint32_t audio_jitter;				/* audio interarrival jitter */

    unsigned int video_fraction:8;		/* video fraction lost since last SR/RR */
    int video_cumulative_no_lost:24;  	/* video cumul. no. pkts lost (signed!) */
    uint32_t video_last_seq;			/* video extended last seq. no. received */
    uint32_t video_jitter;				/* video interarrival jitter */
