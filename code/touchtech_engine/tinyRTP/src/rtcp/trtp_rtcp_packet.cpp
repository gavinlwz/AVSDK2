...
{
    trtp_rtcp_packet_t *packet = (trtp_rtcp_packet_t *)self;
    if(packet) {
    }
    return self;
}
...
trtp_rtcp_packet_t* trtp_rtcp_packet_create(struct trtp_rtcp_header_s* header)
{
    ...
    if((packet = (trtp_rtcp_packet_t *)tsk_object_new(trtp_rtcp_packet_def_t)) && header) {
        packet->header = (struct trtp_rtcp_header_s*)tsk_object_ref(header);
    }
    return packet;
}
...
trtp_rtcp_packet_t* trtp_rtcp_packet_deserialize(const void* data, tsk_size_t size)
{
    ...
    default: {
        ...
        TSK_DEBUG_ERROR("type:%d not recognized as valid RTCP packet type, size:%u", (int)type, size);
...
int trtp_rtcp_packet_add_packet(trtp_rtcp_packet_t* self, trtp_rtcp_packet_t* packet, tsk_bool_t front)
{
    ...
    if(packets) {
        ...
        packet = (trtp_rtcp_packet_t*)tsk_object_ref(packet);
