...
{
    trtp_rtcp_report_bye_t *bye = (trtp_rtcp_report_bye_t *)self;
    if(bye) {
...
trtp_rtcp_report_bye_t* trtp_rtcp_report_bye_create(struct trtp_rtcp_header_s* header)
{
    trtp_rtcp_report_bye_t* bye;
    if((bye = (trtp_rtcp_report_bye_t*)tsk_object_new(trtp_rtcp_report_bye_def_t))) {
        TRTP_RTCP_PACKET(bye)->header = (struct trtp_rtcp_header_s*)tsk_object_ref(header);
    }
    return bye;
}
...
{
    uint32_t* ssrc_list = (uint32_t*)tsk_malloc(sizeof(uint32_t));
    if(ssrc_list) {
        trtp_rtcp_report_bye_t* bye;
        if((bye = (trtp_rtcp_report_bye_t*)tsk_object_new(trtp_rtcp_report_bye_def_t))) {
...
trtp_rtcp_report_bye_t* trtp_rtcp_report_bye_deserialize(const void* data, tsk_size_t _size)
{
    ...
    if(header->rc > 0) {
        ...
        if(!(bye->ssrc_list = (uint32_t *)tsk_calloc(header->rc, sizeof(uint32_t)))) {
            goto bail;
        }
