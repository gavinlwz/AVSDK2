                ...
                PERF_GL_async({
                    ...
                    if(tmpBufferRef && m_cameraPreviewCallback){
                        if (m_netMainHwEncode) { // hardware encode
                            ret = YouMeVideoMixerAdapter::getInstance()->pushVideoPreviewFrameGLES(tmpBufferRef,
                    ...
                    //net second
                    tmpBufferRef = netRender(GLES_MIXER_TYPE_NET_SECOND, current_fb, timestamp);
                    if(tmpBufferRef && m_cameraPreviewCallback){
                        if (m_netSecondHwEncode){
                            ...
                            sendRawVideoToEncoder(tmpBufferRef, timestamp, 1);
                        }
                    }
                    ...
                    //mix
                    tmpBufferRef = mixRender(GLES_MIXER_TYPE_MIX, current_fb, timestamp);
                    if(tmpBufferRef){
                        YouMeVideoMixerAdapter::getInstance()->pushVideoFrameMixedCallbackkForIOS(tmpBufferRef,
                                                                                                  CVPixelBufferGetWidth(tmpBufferRef),
                                                                                                  CVPixelBufferGetHeight(tmpBufferRef),
