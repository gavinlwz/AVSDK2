...
- (BOOL)doInit
{
    ...
    if (currentContext) {
        _glContext = [[EAGLContext alloc] initWithAPI:currentContext.API];
    }else{
    ...
    _renderBgColor = [[UIColor alloc] initWithRed:0.0 green:0.0  blue:0.0  alpha:1.0];
...
- (void)render
{
    ...
    glVertexAttribPointer(ATTRIB_TEXTURE, 2, GL_FLOAT, 0, 0, _coordVertices);
...
{
    if (GLRenderPositionAll == position) {
        memcpy(_coordVertices, coordVertices, sizeof(_coordVertices));
    }
    else if(GLRenderPositionLeft == position)
    {
        memcpy(_coordVertices, coordVertices, sizeof(_coordVertices));
        _coordVertices[2] = 0.5f;
        _coordVertices[6] = 0.5f;
    }
    else if(GLRenderPositionRight == position)
    {
        memcpy(_coordVertices, coordVertices, sizeof(_coordVertices));
