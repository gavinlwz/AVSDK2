...
static tsk_bool_t g_tnet_use_mobile_network = tsk_false;
/**@defgroup tnet_utils_group Network utility functions.
 */

TINYNET_API void tnet_set_usemobilenetWork (tsk_bool_t bEnable)
{
    g_tnet_use_mobile_network = bEnable;
}
...
void tnet_getlasterror (tnet_error_t *error)
{
    ...
    FormatMessageW (FORMAT_MESSAGE_FROM_SYSTEM, tsk_null, err, 0, wBuff, sizeof (wBuff) - 1, tsk_null);
...
#elif TNET_UNDER_WINDOWS
    {
        ...
        FormatMessageA
#endif
        (FORMAT_MESSAGE_FROM_SYSTEM, tsk_null, err, 0, *error, sizeof (*error) - 1, tsk_null);
    }
#else
...
#define FREE(x) HeapFree (GetProcessHeap (), 0, (x))
    ...
    // Make an initial call to GetAdaptersInfo to get the necessary size into the ulOutBufLen
    // variable
        ...
        if (/*ioctl(fd, SIOCGIFFLAGS, &ifr) == 0*/ 1)
        {
            if (!(ifr->ifr_flags & IFF_LOOPBACK) && (ifr->ifr_flags & IFF_UP))
            {
                if (/*ioctl(fd, SIOCGIFHWADDR, &ifr) == 0*/ 1)
                {
                    tnet_interface_t *iface = tnet_interface_create (ifr->ifr_name, ifr->ifr_hwaddr.sa_data, 6);
                    ;
                    // tnet_interface_create (ifr->ifr_name, ifr->ifr_hwaddr.sa_data, 6);
                    tsk_list_push_back_data (ifaces, (void **)&(iface));
 ...
 */
tnet_addresses_L_t *tnet_get_addresses (tnet_family_t family, tsk_bool_t unicast, tsk_bool_t anycast, tsk_bool_t multicast, long if_index)
{
...
#define FREE(x) HeapFree (GetProcessHeap (), 0, (x))
    ...
    if (dwRetVal == NO_ERROR)
    {
        ...
        while (pCurrAddresses)
        {
         ...
         //joexie   while (dnsserver && pDnServer)
			while (pDnServer)
            {
                // memset(ip, '\0', sizeof(ip));
...
bail:
    ...
    for (ifa = ifaddr; ifa; ifa = ifa->ifa_next)
    {
        ...
        //不是wifi地址，略过 FIX-ME：允许使用移动网路的情况下，也要优先使用wifi
        if (!g_tnet_use_mobile_network && !tsk_strcontains (ifa->ifa_name, strlen (ifa->ifa_name), "en"))
        {
            continue;
        }
        ...
        if (unicast)
        {
            ...
            if (!tsk_strcontains (ifa->ifa_name, strlen (ifa->ifa_name), "en"))
            {
                tnet_get_sockip (addr, &mobileIp);
                continue;
            }
            else
            {
    ...
    //最后把非wifiIP放入
    if (g_tnet_use_mobile_network && strlen (mobileIp) != 0 && tsk_list_count (addresses, tsk_null, tsk_null) == 0)
    {
        // Push a new address
        tnet_address_t *address = tnet_address_create (mobileIp);
...
#else /* any system without getifaddrs */
    ...
    while (if_num-- > 0)
    {
        ...
        TSK_DEBUG_INFO ("status:%d", buf[if_num].ifr_flags);
        /* 获取网口状态 */
        if ((buf[if_num].ifr_flags & IFF_UP) == 0)
        {
            continue;
        }
        else
        {
            TSK_DEBUG_INFO ("buf[if_num].ifr_flags & IFF_UP = %d", buf[if_num].ifr_flags & IFF_UP);
        }
        ...
        if ((address = tnet_address_create (inet_ntoa (((struct sockaddr_in *)(&buf[if_num].ifr_addr))->sin_addr))))
        {
            TSK_DEBUG_INFO ("Push a new address:%s", address->ip);
...
{
	static const tsk_size_t __tnet_mac_address_len = sizeof(tnet_mac_address) / sizeof(uint8_t);
    int ret = -1;
        ...
        if ((_ret = GetAdaptersInfo (info, &size)) == ERROR_SUCCESS || _ret == ERROR_BUFFER_OVERFLOW)
        {
            ...
            if (info)
            {
                if ((_ret = GetAdaptersInfo (info, &size)) == ERROR_SUCCESS)
                {
                    ...
                    {
                        if (pos->Type == MIB_IF_TYPE_LOOPBACK && pos->Next)
                        { // skip loopback if we still have items to check
                            continue;
                        }
...
/**@ingroup tnet_utils_group
* Retrieves the @a source IP address that has the best route to the specified IPv4 or IPv6 @a
* destination.
    ...
    if (profile != nullptr && profile->NetworkAdapter != nullptr)
    {
        ...
        if (HostNames->Size > 0)
        {
            ...
            do
            {
                ...
                if ((TNET_SOCKET_TYPE_IS_IPV4 (type) && HostName->Current->IPInformation->PrefixLength->Value > 32) ||
                    (TNET_SOCKET_TYPE_IS_IPV6 (type) && HostName->Current->IPInformation->PrefixLength->Value > 128))
                {
                    TSK_DEBUG_INFO ("Type mismatch - Skiping IP address=%s, IanaInterfaceType=%d, PrefixLength=%d", CanonicalName.data (),
                                    HostName->Current->IPInformation->NetworkAdapter->IanaInterfaceType, HostName->Current->IPInformation->PrefixLength->Value);
                    continue;
                }
...
#define ADVANCE(x, n) (x += ROUNDUP ((n)->sa_len))
    ...
    for (ifa = ifaddr; ifa; ifa = ifa->ifa_next)
    {
        ...
        {
            if (IN6_IS_ADDR_LINKLOCAL (&((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr) ^ IN6_IS_ADDR_LINKLOCAL (&((struct sockaddr_in6 *)&destAddr)->sin6_addr))
            {
                continue;
            }
            if (IN6_IS_ADDR_SITELOCAL (&((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr) ^ IN6_IS_ADDR_SITELOCAL (&((struct sockaddr_in6 *)&destAddr)->sin6_addr))
            {
                continue;
            }
        }
    ...
    memcpy (*source, TNET_SOCKET_TYPE_IS_IPV6 (type) ? "::" : "0.0.0.0", TNET_SOCKET_TYPE_IS_IPV6 (type) ? 2 : 7);
#endif
...
/**@ingroup tnet_utils_group
 *
 * Converts human-readable text strings representing hostnames or IP addresses into a dynamically
 *allocated linked list of struct addrinfo structures.
 *			You MUST call @ref tnet_freeaddrinfo() function to free the result.
 *
 * @param [in]	node	A pointer to a NULL-terminated ANSI string that contains a host (node) name
 *or
 *a numeric host address string. For the Internet protocol, the numeric host address string is a
 *dotted-decimal IPv4 address or an IPv6 hex address..
 * @param [in]	service	A pointer to a NULL-terminated ANSI string that contains either a service
 *name or port number represented as a string.
 * @param [in]	hints	A pointer to an addrinfo structure that provides hints about the type of
 *socket the caller supports.
 * @param [out]	res		A pointer to a linked list of one or more addrinfo structures that contains
 ...
 * @param addr [in] A pointer to @b sockaddr structure for which to retrieve the IP address and
 * port.
...
/**@ingroup tnet_utils_group
 * Provides protocol-independent name resolution from an address to an ANSI host name and from a
 * port number to the ANSI service name.
 * @param sa A pointer to a socket address structure that contains the address and port number of
 * the socket. For IPv4, the sa parameter points to a sockaddr_in structure. For IPv6, the sa
 * parameter points to a @b sockaddr_in6 structure.
 * @param salen The length, in bytes, of the structure pointed to by the sa parameter.
 * @param node A pointer to an ANSI string used to hold the host name. On success, a pointer to the
 * host name is returned as a Fully Qualified Domain Name (FQDN) by default. If the host parameter
 * is NULL, this indicates the caller does not want to receive a host name string.
 * @param nodelen The length, in bytes, of the buffer pointed to by the host parameter. The caller
 * must provide a buffer large enough to hold the host name, including the terminating NULL
 * character.
 * @param service A pointer to an ANSI string to hold the service name. On success, a pointer is
 * returned to an ANSI string that represents the service name associated with the port number. If
 * the serv parameter is NULL, this indicates the caller does not want to receive a service name
 * string.
 * @param servicelen The length, in bytes, of the buffer pointed to by the serv parameter. The
 * caller must provide a buffer large enough to hold the service name, including the terminating
 * NULL character.
 * @param flags A value used to customize processing of the @b getnameinfo function. See the Remarks
 * section.
 * @retval Zero if succeed and non-zero error code otherwise.
 */
 ...
 * @param timeout The number of milliseconds to wait. The function will immediately return if the
 * socket
...
int tnet_sockfd_joingroup6 (tnet_fd_t fd, const char *multiaddr, unsigned iface_index)
{
    ...
    // memcpy(&mreq6.ipv6mr_multiaddr, &((struct sockaddr_in6 *) &ss)->sin6_addr, sizeof(struct
    // in6_addr));
...
/**@ingroup tnet_utils_group
 * Converts human-readable text strings representing hostnames or IP addresses into a @b
...
/**@ingroup tnet_utils_group
 * Converts human-readable text strings representing hostnames or IP addresses as socket (File
 * descriptor).
 * @param host The hostname/IP address to convert.
 * @param port The local port associated to the host.
 * @param type The type of the socket to create.
...
int tnet_sockfd_set_mode (tnet_fd_t fd, int nonBlocking)
{
    if (fd != TNET_INVALID_FD)
    {
        ...
        // if(WSAIoctl(fd, FIONBIO, &nonblocking, sizeof(nonblocking), NULL, 0, NULL, NULL, NULL) ==
        // SOCKET_ERROR)
        {
 ...
 * @retval If no error occurs, sendto returns the total number of bytes sent, which can be less than
 * the number indicated by @b size.
...
int tnet_sockfd_sendto (tnet_fd_t fd, const struct sockaddr *to, const void *buf, tsk_size_t size)
{
    tsk_size_t sent = 0;
    int ret = -1;
    static int ignore_count = 0; //发生错误时，连续丢掉多少次包
    if(ignore_count>0)
    {
        --ignore_count;
        goto bail;
    }
   // struct sockaddr_in* pTmp = (struct sockaddr_in*)to;
    //char* addr=inet_ntoa(pTmp->sin_addr);
    //TSK_DEBUG_INFO("Farend ip:%s port:%d",addr,pTmp->sin_port);
    if (fd == TNET_INVALID_FD)
    {
    ...
    try_again:
        ...
        if (ret <= 0)
        {
            int iErrorNo = tnet_geterrno();
            
            tnet_error_t errorBuf;
            tnet_getlasterror(&errorBuf);
            if ((iErrorNo == TNET_ERROR_NOSPACE) || (iErrorNo==TNET_ERROR_WOULDBLOCK)) {
                /*
                if (try_guard--)
                {
                    TSK_DEBUG_ERROR ("send data error:%s  retry",errorBuf);
                    tsk_thread_sleep (10);
                    goto try_again;
                }
                */
            ...
            ignore_count = 20;
            //TSK_DEBUG_ERROR ("send data error failed:%s ",errorBuf);
            goto bail;
        }
        else
        {
 ...
 * @param flags A set of options that modify the behavior of the function call beyond the options
 * specified for the associated socket.
 * All flags which can be passed to @b recvfrom.
 * @param from An optional pointer to a buffer in a @b sockaddr structure that will hold the source
 * address upon return.
...
int tnet_sockfd_recvfrom (tnet_fd_t fd, void *buf, tsk_size_t size, int flags, struct sockaddr *from)
{
    ...
    /*if (iRecvCount >=0) {
        struct sockaddr_in* testaddr = ( struct sockaddr_in*)from;
 ...
 * @retval If no error occurs, send returns the total number of bytes sent, which can be less than
 * the number requested to be sent in the @b size parameter.
 */
    ...
    while (sent < size)
    {
        if ((ret = send (fd, (((const char *)buf) + sent), (int)(size - sent), flags)) <= 0)
        {
            ...
            else
            {
                ...
                // Under Windows XP if WSAGetLastError()==WSAEINTR then try to disable both the ICS
                // and the Firewall
                // More info about How to disable the ISC:
                // http://support.microsoft.com/?scid=kb%3Ben-us%3B230112&x=6&y=11
                goto bail;
            }
        }
        else
 ...
 * All flags which can be passed to @b recv.
 * @retval If no error occurs, recv returns the number of bytes received and the buffer pointed to
 * by the buf parameter will contain this data received. If the connection has been gracefully
 * closed, the return value is zero.
 * Otherwise, non-zero (negative) error code is returned.
 */
 ...
 * @param to A pointer to the @b sockaddr_storage structure to which the connection should be
 * established.
 * @retval Zero if succeed and non-zero error code otherwise.
 */
int tnet_sockfd_connectto (tnet_fd_t fd, const struct sockaddr_storage *to)
{
    ...
    if ((status = WSAConnect (fd, (LPSOCKADDR)to, sizeof (*to), NULL, NULL, NULL, NULL)) == SOCKET_ERROR)
    {
        ...
        {
            TSK_DEBUG_WARN ("TNET_ERROR_WOULDBLOCK/TNET_ERROR_ISCONN/TNET_ERROR_INTR/"
                            "TNET_ERROR_INPROGRESS  -> you should use "
                            "tnet_sockfd_waitUntilWritable() before trying to send data");
            status = 0;
        }
        else
        {
    ...
    //TODO joe IPV4 only
    if ((status = connect (fd, (struct sockaddr *)to, sizeof(struct sockaddr_in))))
    {
        ...
        {
            TSK_DEBUG_INFO ("TNET_ERROR_WOULDBLOCK/TNET_ERROR_ISCONN/TNET_ERROR_INPROGRESS/"
                            "TNET_ERROR_EAGAIN  ==> use tnet_sockfd_waitUntilWritable.");
            status = 0;
        }
        else
        {
