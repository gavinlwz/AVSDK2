...
int tnet_transport_add_socket (const tnet_transport_handle_t *handle, tnet_fd_t fd, tnet_socket_type_t type, tsk_bool_t take_ownership, tsk_bool_t isClient)
{
    ...
    TSK_DEBUG_INFO ("@@ tnet_transport_add_socket");
...
int tnet_transport_add_socket_2 (const tnet_transport_handle_t *handle,
                                 tnet_fd_t fd,
                                 tnet_socket_type_t type,
                                 tsk_bool_t take_ownership,
                                 tsk_bool_t isClient,
                                 const char *dst_host,
                                 tnet_port_t dst_port)
{
    ...
    (void)(dst_host);
    (void)(dst_port);
    return tnet_transport_add_socket (handle, fd, type, take_ownership, isClient);
}
...
int tnet_transport_remove_socket (const tnet_transport_handle_t *handle, tnet_fd_t *pfd)
{
    ...
    for (i = 0; i < context->count; i++)
    {
        if (context->sockets[i]->fd == fd)
        {
            ...
            if (!self_ref)
            { // if self_ref then, pfd no longer valid after removeSocket()
                *pfd = TNET_INVALID_FD;
            }
            break;
        }
...
tsk_size_t tnet_transport_send (const tnet_transport_handle_t *handle, tnet_fd_t from, const void *buf, tsk_size_t size)
{
    ...
    if ((numberOfBytesSent = tnet_sockfd_send (from, buf, size, 0)) <= 0)
    {
...
int removeSocket (int index, transport_context_t *context)
{
    ...
    if (index < (int)context->count)
    {
        ...
        TSK_DEBUG_INFO ("Socket to remove: fd=%d, index=%d, tail.count=%zu", context->sockets[index]->fd, index, context->count);
...
int tnet_transport_stop (tnet_transport_t *transport)
{
    ...
    if (context)
    {
        ...
        // signal
        tsk_safeobj_lock (context); // =>MUST
        if (tnet_transport_have_socket (transport, context->pipeR))
        { // to avoid SIGPIPE=> check that there is at least one reader
...
int addSocket (tnet_fd_t fd, tnet_socket_type_t type, tnet_transport_t *transport, tsk_bool_t take_ownership, tsk_bool_t is_client)
{
    ...
    if (context)
    {
        ...
        TSK_DEBUG_INFO ("Socket added[%s]: fd=%d, tail.count=%zu", transport->description, fd, context->count);
...
int tnet_transport_prepare (tnet_transport_t *transport)
{
    ...
    TSK_DEBUG_INFO ("@@ tnet_transport_prepare start");
  ...
  /* if (TNET_SOCKET_TYPE_IS_STREAM (transport->master->type))
    ...
    TSK_DEBUG_INFO ("master fd=%d isTCP:%d", transport->master->fd, TNET_SOCKET_TYPE_IS_STREAM (transport->master->type));
    // don't take ownership: will be closed by the dctor() when refCount==0
    // otherwise will be closed twice: dctor() and removeSocket()
    if (!TNET_SOCKET_TYPE_IS_STREAM (transport->master->type)) {
        // for tcp connect, fd will be insert to fds after connect
        // see tnet_transport_start in tnet_transport.c
    ...
    TSK_DEBUG_INFO ("== tnet_transport_prepare end");
...
void *tnet_transport_mainthread (void *param)
{
    ...
    TSK_DEBUG_INFO ("Starting [%s] server with IP {%s} on port {%d} using fd {%d} with type {%d}...", transport->description, transport->master->ip, transport->master->port, transport->master->fd,
                    transport->master->type);
    
    TSK_DEBUG_INFO("TSK_RUNNABLE,running:%d started:%d",TSK_RUNNABLE(transport)->running,TSK_RUNNABLE(transport)->started);
    ...
    while (TSK_RUNNABLE (transport)->running || TSK_RUNNABLE (transport)->started)
    {
        ...
        if (ret < 0)
        {
            ...
            TSK_DEBUG_ERROR("poll() have failed.ret=%d",ret);
            goto bail;
        }
        ...
        for (i = 0; i < context->count; i++)
        {
            ...
            if (context->ufds[i].revents & TNET_POLLIN)
            {
                ...
                if (ret < 0 && is_stream)
                {
                    ...
                    TSK_DEBUG_INFO ("ioctlt(%d), len=%zu returned zero or failed", active_socket->fd, len);
                ...
                {
                    TSK_DEBUG_ERROR ("TSK_CALLOC FAILED, len:%d", len);
                    goto TNET_POLLIN_DONE;
                }
