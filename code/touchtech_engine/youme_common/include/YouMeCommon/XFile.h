...
public:
    ...
    //遍历一个目录，获取子目录，或者文件列表
    static bool getPathContents( const XString& strPath ,  std::vector<XString>&  vecAllFiles ,  bool needDir);
#if defined(WIN32)
	...
	static bool delete_dir(const XCHAR* dir_path, bool deleteSelf);

	static XString GetFileExt(const XString& strFilePath);
