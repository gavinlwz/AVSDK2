...
public:
	...
	static inline ValueType XAny_Cast(const CXAny & operand)
	{
		...
		{
			const char*pszType1 = operand.type().name();
			XString strType1 = UTF8TOXString2(pszType1);
			const char*pszType2 = typeid(ValueType).name();
			XString strType2 = UTF8TOXString2(pszType2);

			YouMe_LOG_Warning(__XT("anycast 类型不一致:%s %s"), strType1.c_str(),strType2.c_str());
            assert(false);
            return ValueType();
		}
