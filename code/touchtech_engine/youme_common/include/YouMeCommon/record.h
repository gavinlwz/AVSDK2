...
public:
	...
	bool SetData(const T& val) {
		if (CanAccept(val)) {
			T value = ToNetOrderByVal(val);
			memcpy(m_pBuffInd, &value, sizeof(T));
	...
	bool SetData(const char * val) {
		if (CanAccept(val)) {
			...
			unsigned short netLen = ToNetOrderByVal(usLen);	//字符串长度
			memcpy(m_pBuffInd, &netLen, sizeof(unsigned short));
	...
	bool SetData(const void * val, unsigned short len) {
		if (CanAccept(val)) {
			unsigned short netLen = ToNetOrderByVal((unsigned short)len);	//字符串长度
			memcpy(m_pBuffInd, &netLen, sizeof(unsigned short));
			m_pBuffInd += sizeof(unsigned short);
	...
	bool GetData(T& val) {
		if (m_pBuffInd + sizeof(val) <= m_pBuffEnd) {	//缓冲区未越界
			T value;
			memcpy(&value, m_pBuffInd, sizeof(T));
			val = ::youmecommon::ToHostOrderByVal(value);
			m_pBuffInd += sizeof(val);
			return true;
		}
		return false;
	...
	bool GetData(const char *& val) {
		unsigned short usNetLen = 0;
		memcpy(&usNetLen, m_pBuffInd, sizeof(unsigned short));
		unsigned short usLen = ToHostOrderByVal(usNetLen); //字符串长度
	...
	bool GetData(unsigned char *& val, unsigned short& len) {
		...
		len = ToHostOrderByVal(usNetLen);	//长度
