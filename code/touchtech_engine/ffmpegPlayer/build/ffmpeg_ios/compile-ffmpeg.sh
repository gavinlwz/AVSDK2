...
FF_ALL_ARCHS_IOS8_SDK="armv7 arm64 i386 x86_64 mac_x64"
...
FF_LIBS="libavcodec libavformat libavutil"
do_lipo_ffmpeg () {
    LIB_FILE=$1
    LIPO_FLAGS=
    for ARCH in $FF_ALL_ARCHS
    do
        if [[ $ARCH = "mac_x64" ]]; then
            continue
        fi
        ARCH_LIB_FILE="$UNI_BUILD_ROOT/../../libs/apple/ios_$ARCH/lib/$LIB_FILE"
        echo $ARCH_LIB_FILE
    ...
    outpath="$UNI_BUILD_ROOT/../../libs/apple/universal"
    mkdir -p $outpath
    xcrun lipo -create $LIPO_FLAGS -output "$outpath/$LIB_FILE"
    xcrun lipo -info "$outpath/$LIB_FILE"
    echo "output universal lib: $outpath/$LIB_FILE"
}
...
do_lipo_ssl () {
    ...
    if [ "$LIPO_FLAGS" != "" ]; then
        mkdir "$UNI_BUILD_ROOT/build/universal/lib"
        ...
        echo "output universal lib: $UNI_BUILD_ROOT/build/universal/lib/$LIB_FILE"
    fi
}
...
#            cp -f tools/config.h                        "$UNI_INC_DIR/libffmpeg/config.h"
    ...
    #copy macos lib
    mkdir -p "$UNI_BUILD_ROOT/../../libs/apple/osx/"
    cp "$UNI_BUILD_ROOT/../../libs/apple/mac_x64/lib/"*.a "$UNI_BUILD_ROOT/../../libs/apple/osx/"
    cp "$UNI_BUILD_ROOT/../../ffmpeg-external-libs/libfdk-aac/lib/apple/osx/libfdk-aac.a"  "$UNI_BUILD_ROOT/../../libs/apple/osx/"
...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" -o "$FF_TARGET" = "mac_x64" ]; then
    echo_archs
