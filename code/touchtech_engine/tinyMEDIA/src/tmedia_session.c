...
* For more information about the SOA, please refer to
* http://betelco.blogspot.com/2010/03/sdp-offeranswer-soa_2993.html
*/
...
static const tsdp_message_t *_tmedia_session_mgr_get_lo (tmedia_session_mgr_t *self, tsk_bool_t skip_session_load,int iSessionID );
static int _tmedia_session_mgr_start (tmedia_session_mgr_t *self, int session_index, tmedia_type_t type );
static int _tmedia_session_mgr_stop (tmedia_session_mgr_t *self, int session_index,  tmedia_type_t  type );
static int _tmedia_session_mgr_clear (tmedia_session_mgr_t *self, int iServerSessionID, int session_index,  tmedia_type_t  type );
...
tsk_bool_t tmedia_session_set_2 (tmedia_session_t *self, const tmedia_param_t *param)
{
    ...
    if (param->plugin_type == tmedia_ppt_session)
    {
        if (param->value_type == tmedia_pvt_int32)
        {
            ...
            else if (tsk_striequals (param->key, "sessionid"))
            {
                self->sessionid = *((int32_t *)param->value);
                return tsk_true;
            }
...
tmedia_codecs_L_t *tmedia_session_match_codec (tmedia_session_t *self, const tsdp_header_M_t *M)
{
    ...
    tsk_list_foreach (it1, M->FMTs)
    {
        ...
        {
            /* 'tmedia_codec_id_none' is used for fake codecs (e.g. dtmf or msrp) and should not be
             * filtered beacuse of backward compatibility*/
            ...
            // For example, H.264 codecs without profiles (Jitsi, Tiscali PC client) to distinguish
            // them could match more than once
        ...
        compare_fmtp:
            if ((fmtp = tsdp_header_M_get_fmtp (M, fmt->value)))
            { /* remote have fmtp? */
                if (tmedia_codec_sdp_att_match (codec, "fmtp", fmtp))
                { /* fmtp matches? */
...
int _tmedia_session_load_codecs (tmedia_session_t *self)
{
    ...
    {
        /* 'tmedia_codec_id_none' is used for fake codecs (e.g. dtmf, bfcp or msrp) and should not
         * be filtered beacuse of backward compatibility*/
    ...
    if (1)
    { // code valid for all use-cases but for now it's not fully tested and not needed for the
      // clients
    filter_neg_codecs:
        tsk_list_foreach (item, self->neg_codecs)
        {
            ...
            if (!(tsk_list_find_item_by_pred (self->codecs, __pred_find_codec_by_id, &codec->id)))
            {
                ...
                TSK_DEBUG_INFO ("Codec '%s' with format '%s' was negotiated but [supported codecs] "
                                "updated without it -> removing",
                                codec_name, neg_format);
                // update sdp and remove the codec from the list
                if (self->M.lo && !TSK_LIST_IS_EMPTY (self->M.lo->FMTs))
                {
                    if (self->M.lo->FMTs->head->next == tsk_null && tsdp_header_M_have_fmt (self->M.lo, neg_format))
                    { // single item?
                        // rejecting a media with port equal to zero requires at least one format
                        TSK_DEBUG_INFO ("[supported codecs] updated but do not remove codec with "
                                        "name='%s' and format='%s' because it's the last one",
                                        codec_name, neg_format);
                        self->M.lo->port = 0;
                    }
                    else
...
* @param type the type of the session to create. For example, (@ref tmed_sess_type_audio | @ref
...
int tmedia_session_mgr_set_codecs_supported (tmedia_session_mgr_t *self, tmedia_codec_id_t codecs_supported)
{
    ...
    ret = tmedia_session_mgr_set (self, TMEDIA_SESSION_SET_INT32 (self->type, "codecs-supported", codecs_supported), tsk_null);
...
{
    return _tmedia_session_mgr_start (self, kSessionIndexAll,  tmedia_all );
}

int tmedia_session_mgr_restart( tmedia_session_mgr_t *self , tmedia_type_t type )
{
    int ret = _tmedia_session_mgr_stop( self , kSessionIndexAll,  type );
    
    if( ret == 0   )
    {
        ret = _tmedia_session_mgr_start( self , kSessionIndexAll,  type );
    }
...
{
    int ret = 0;
    ret += _tmedia_session_mgr_stop (self, kSessionIndexAll, tmedia_video );

    ret += _tmedia_session_mgr_stop (self, kSessionIndexAll, tmedia_audio );

    return ret;
}


int tmedia_session_mgr_clear (tmedia_session_mgr_t *self, int iServerSessionID, tmedia_type_t type)
{
    return _tmedia_session_mgr_clear (self, iServerSessionID, kSessionIndexAll, type );
}
/**@ingroup tmedia_session_group
* Gets local offer.
*/
const tsdp_message_t *tmedia_session_mgr_get_lo (tmedia_session_mgr_t *self,int iSessionID)
{
    return _tmedia_session_mgr_get_lo (self, kSkipSessionLoadFalse, iSessionID);
}
...
* Sets remote offer.
*/
int tmedia_session_mgr_set_ro (tmedia_session_mgr_t *self, const tsdp_message_t *sdp, tmedia_ro_type_t ro_type,int iSessionID)
{
    ...
    new_mediatype = tmedia_type_from_sdp (sdp); //tmedia_audio
    had_ro_sdp = (self->sdp.ro != tsk_null);//0
    had_lo_sdp = (self->sdp.lo != tsk_null);//1
    had_ro_provisional = (had_ro_sdp && self->ro_provisional);//0


    /*
    * Make sure that the provisional response is an preview of the final as explained rfc6337
    section 3.1.1. We only check the most important part (IP addr and ports).
    * It's useless to check codecs or any other caps (SRTP, ICE, DTLS...) as our offer haven't
    changed
    ...
    * If the remote party is buggy, then the newly generated local SDP will be sent in the ACK
    request
    */
    ...
    /* This is to hack fake forking from ZTE => ignore SDP with loopback address in order to not
     * start/stop the camera several
    ...
    {
        tsk_bool_t _is_ro_loopback_address = (tsk_striequals ("IP4", C->addrtype) && tsk_striequals ("127.0.0.1", C->addr)) || (tsk_striequals ("IP6", C->addrtype) && tsk_striequals ("::1", C->addr));//120.24.68.210
        for (index = 0; index < m_lines_count; ++index)//m_lines_count=0
        {
            is_ro_loopback_address[index] = _is_ro_loopback_address;
        }
    }

    TSK_DEBUG_INFO ("m_lines_count=%u,\n"
                    ...
                    (unsigned)m_lines_count, (unsigned)__flags_sum ((const tsk_bool_t *)&is_dtls_fingerprint_changed, m_lines_count),
                    ...
                    is_ro_provisional_final_matching, is_ro_media_lines_changed, (unsigned)__flags_sum ((const tsk_bool_t *)&is_ro_network_info_changed, m_lines_count),
                    (unsigned)__flags_sum ((const tsk_bool_t *)&is_ro_loopback_address, m_lines_count), is_media_type_changed,
                    (unsigned)__flags_sum ((const tsk_bool_t *)&is_ro_codecs_changed, m_lines_count), (unsigned)__flags_sum ((const tsk_bool_t *)&is_local_encoder_still_ok, m_lines_count));

    /*
      * It's almost impossible to update the codecs, the connection information etc etc while the
      *sessions are running
      * For example, if the video producer is already started then, you probably cannot update its
      *configuration
      * without stoping it and restarting it again with the right config. Same for RTP Network
      *config (ip addresses, NAT, ports, IP version, ...)
      *
      * "is_loopback_address" is used as a guard to avoid reconf for loopback address used for
      *example by ZTE for fake forking. In all case
    ...
    if (self->started)//没启动
    {
        ...
        {
            if (/* && (!is_ro_loopback_address[index]) && */ ((is_ro_codecs_changed[index] && !is_local_encoder_still_ok[index]) || is_ro_network_info_changed[index] ||
                                                              is_dtls_fingerprint_changed[index] || is_sdes_crypto_changed[index]))
            {
                TSK_DEBUG_INFO ("Stop media index %d to reconf", (int)index);
                stopped_to_reconf[index] = tsk_true;
                tmedia_session_mgr_set (self, TMEDIA_SESSION_SET_INT32 (media_types[index], "stop-to-reconf", stopped_to_reconf[index]), TMEDIA_SESSION_SET_NULL ());
                if ((ret = _tmedia_session_mgr_stop (self, (int)index , tmedia_all )))
                {
    ...
    /*  - if the session is running this means no session update is required unless some important
       changes
    ...
    * if network-initiated: think about tmedia_type_from_sdp() before creating the manager */
    ...
    if (index != m_lines_count)
    { // start new sessions
        for (; index < m_lines_count; ++index)
    ...
    */
    if (C && C->addr)//120.24.68.210
    {
        tmedia_session_mgr_set (self, TMEDIA_SESSION_SET_STR (self->type, "remote-ip", C->addr), TMEDIA_SESSION_SET_NULL ());
    }

    /*	pass complete remote sdp to the sessions to allow them to use session-level attributes
    */
    tmedia_session_mgr_set (self, TMEDIA_SESSION_SET_POBJECT (self->type, "remote-sdp-message", self->sdp.ro), TMEDIA_SESSION_SET_NULL ());

    /* foreach "m=" line in the remote offer create/prepare a session (requires the session to be
     * stopped)*/
    index = 0;
    ...
    while ((M = (const tsdp_header_M_t *)tsdp_message_get_headerAt (sdp, tsdp_htype_M, index++)))
    {
        ...
        if (ms && (tsk_striequals (tmedia_session_get_media (ms), M->media)))
        {
            ...
            if (M->port == 0 || (ret = _tmedia_session_set_ro (TMEDIA_SESSION (ms), M)) == 0)
            {
                ...
                tsk_list_remove_item_by_data (list_tmp_sessions, ms);    // make sure not to use the
                                                                         // session more than once
            }
            else
            {
                TSK_DEBUG_WARN ("_tmedia_session_set_ro() failed");
                ret = 0; // add ghost for this session. Do not goto bail because set_ro() is allowed
...
end_of_sessions_update:
    ...
    * When ICE is active ("is_ice_active" = yes), the media session will be explicitly restarted
    * when conncheck succeed or fail.
    */
    for (index = 0; index < m_lines_count; ++index)
    {
        if (stopped_to_reconf[index])
        {
            if ((ret = _tmedia_session_mgr_start (self, (int)index, tmedia_all )))
            {
...
* @param self the session manager
* @param type the type of the medias to check (you can combine several medias. e.g.
* audio|video|msrp)
...
#define has_media(media_type) (tsk_list_find_object_by_pred (self->sessions, __pred_find_session_by_type, &(media_type)))
    ...
    if (TSK_LIST_IS_EMPTY (self->sessions))
    {
        ...
        // while (tsk_list_remove_item_by_pred(self->sessions, __pred_find_session_by_type,
        // &__ghost_media_type)) ;
        // while (tsk_list_remove_item_by_pred(self->sessions, __pred_find_session_by_type,
        // &__none_media_type)) ;
        /* for each registered plugin create a session instance */
        while ((i < TMED_SESSION_MAX_PLUGINS) && (plugin = __tmedia_session_plugins[i++]))
        {
            ...
            { /* media_type *IS* enabled */
                ...
                else
                {
                    ...
                    {
                        /* do not call "tmedia_session_mgr_set()" here to avoid applying parms
                         * before the creation of all session */
            ...
            { /* media_type *IS NOT* enabled */
                if (has_media (plugin->type))
                {
                    // do not remove to make sure media indexes match -> see rfc 3264 section 8
                    // tsk_list_remove_item_by_pred(self->sessions, __pred_find_session_by_type,
                    // &(plugin->type));
                    _tmedia_session_mgr_disable_session_with_type (self, plugin->type);
                }
            }
        }
        const char* ipType = (self->ipv6 ? "ipv6" : "ipv4");
        /* set default values and apply params*/
        tmedia_session_mgr_set (self,
                                TMEDIA_SESSION_SET_STR (self->type, "local-ip", self->addr), TMEDIA_SESSION_SET_STR (self->type, "local-ipver", ipType),
                                TMEDIA_SESSION_SET_INT32 (self->type, "bandwidth-level", self->bl), TMEDIA_SESSION_SET_NULL ());
...
static const tsdp_message_t *_tmedia_session_mgr_get_lo (tmedia_session_mgr_t *self, tsk_bool_t skip_session_load,int iSessionID)
{
    ...
    else
    {
        ...
        {
            /* Set connection "c=" */
            tsdp_message_add_headers (self->sdp.lo, TSDP_HEADER_C_VA_ARGS ("IN", self->ipv6 ? "IP6" : "IP4", self->public_addr ? self->public_addr : self->addr), tsk_null);
            if (inc_ver)
            {
                ++self->sdp.lo_ver;
            }
    ...
    tmedia_session_mgr_set (self, TMEDIA_SESSION_SET_POBJECT (self->type, "local-sdp-message", self->sdp.lo),TMEDIA_SESSION_SET_INT32(self->type, "sessionid", iSessionID), TMEDIA_SESSION_SET_NULL ());
...
static int _tmedia_session_mgr_start (tmedia_session_mgr_t *self, int session_index, tmedia_type_t type )
{
    ...
    tsk_list_foreach (item, self->sessions)
    {
        if (session_index == kSessionIndexAll || index++ == session_index)
        {
            ...
            if( type != tmedia_all && (session->plugin->type & type) == 0 )
            {
                continue;
            }
            ...
            {
                if( self->session_start_fail_callback != NULL )
                {
                    self->session_start_fail_callback( session->plugin->type );
                }
                TSK_DEBUG_ERROR ("Failed to start %s session", session->plugin->media);
                continue;
            }
            //启动成功对 rtpmanager 增加一个插件引用计数
            tdav_session_av_t *base=(tdav_session_av_t *)session;
            trtp_manager_addPlugRefCount(base->rtp_manager);
        }
    }
...
static int _tmedia_session_mgr_stop (tmedia_session_mgr_t *self, int session_index , tmedia_type_t type )
{
    ...
    tsk_list_foreach (item, self->sessions)
    {
        if (session_index == kSessionIndexAll || index++ == session_index)
        {
            ...
            if( type != tmedia_all && (type & session->plugin->type) == 0  )
            {
                continue;
            }
...
static int _tmedia_session_mgr_clear (tmedia_session_mgr_t *self, int iServerSessionID, int session_index , tmedia_type_t type )
{
    ...
    TSK_DEBUG_INFO ("tmedia_session_mgr_clear()");
    ...
    tsk_list_foreach (item, self->sessions)
    {
        if (session_index == kSessionIndexAll || index++ == session_index)
        {
            TSK_DEBUG_INFO ("clear sessionid: %d",iServerSessionID);
            if (!(session = item->data) || !session->plugin || !session->plugin->clear)
            {
                if (session && session->plugin) {
                    TSK_DEBUG_ERROR ("Invalid session, type:%d", session->plugin->type);
                } else {
                    TSK_DEBUG_ERROR ("Invalid session");
                }
            ...
            if ((ret = session->plugin->clear (session, iServerSessionID)))
            {
                TSK_DEBUG_ERROR ("Failed to clear session");
                continue;
            }
            // session->prepared = tsk_false;
        }
...
void tmedia_session_set_session_start_fail_callback( tmedia_session_mgr_t * self,
                                                                  SessionStartFailCallback_t  callback )
{
    self->session_start_fail_callback = callback;
}
