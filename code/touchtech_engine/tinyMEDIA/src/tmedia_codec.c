...
* Initialize a Codec
* @param self The codec to initialize. Could be any type of codec (e.g. @ref tmedia_codec_audio_t or
* @ref tmedia_codec_video_t).
* @param type
...
int tmedia_codec_init (tmedia_codec_t *self, tmedia_type_t type, const char *name, const char *desc, const char *format)
{
    ...
    if (!self->bandwidth_max_upload)
        self->bandwidth_max_upload =INT_MAX;
        
    if (!self->bandwidth_max_download)
        self->bandwidth_max_download =INT_MAX;
    ...
    if (type & tmedia_audio)
    {
        ...
        if (!audio->in.ptime)
            audio->in.ptime =
            (self->plugin->audio.ptime ? self->plugin->audio.ptime : tmedia_defaults_get_audio_ptime ());
        if (!audio->out.ptime)
            audio->out.ptime =
            (self->plugin->audio.ptime ? self->plugin->audio.ptime : tmedia_defaults_get_audio_ptime ());
        ...
        if (!audio->in.timestamp_multiplier)
            audio->in.timestamp_multiplier =
            tmedia_codec_audio_get_timestamp_multiplier (self->id, self->in.rate);
        if (!audio->out.timestamp_multiplier)
            audio->out.timestamp_multiplier =
            tmedia_codec_audio_get_timestamp_multiplier (self->id, self->out.rate);
    }
...
*/
int tmedia_codec_open(tmedia_codec_t *self, const void* context)
{
    if (!self || !self->plugin)
    {
        TSK_DEBUG_ERROR ("Invalid parameter");
    ...
    TSK_DEBUG_INFO ("tmedia_codec_open start [%s] codec", self->plugin->desc);
    if (self->opened)
    {
    ...
    {
        int ret;
        if ((ret = self->plugin->open (self, context)))
        {
...
int tmedia_codec_plugin_register (const tmedia_codec_plugin_def_t *plugin)
{
    ...
    for (i = 0; i < TMED_CODEC_MAX_PLUGINS; i++)
    {
        ...
        if (__tmedia_codec_plugins[i]->codec_id == plugin->codec_id && plugin->codec_id != tmedia_codec_id_none)
        { // 'tmedia_codec_id_none' is used for fake codecs
            TSK_DEBUG_INFO ("Codec Registration: '%s' ignored because '%s' already registered",
                            plugin->desc, __tmedia_codec_plugins[i]->desc);
            return -3;
        }
    }
 ...
 * @sa @ref tmedia_codec_plugin_is_registered_2() @ref tmedia_codec_plugin_register() and @ref
 ...
 * @sa @ref tmedia_codec_plugin_is_registered() @ref tmedia_codec_plugin_register() and @ref
 * tmedia_codec_plugin_unregister()
 */
 ...
 */
int tmedia_codec_plugin_registered_get_all (const struct tmedia_codec_plugin_def_s *(**plugins)[TMED_CODEC_MAX_PLUGINS],
                                            tsk_size_t *count)
{
    if (!plugins || !count)
    {
...
* @param format The format of the codec to create (e.g. "0" for PCMU or "8" for PCMA or "*" for
* MSRP)
...
* @param self the codec for which to get the rtpmap attribute. Should be created using @ref
* tmedia_codec_create().
* @retval rtpmap string (e.g. "AMR-WB/16000/2" or "H261/90000") if succeed and Null otherwise. It's
* up to the caller to free the
* returned string.
*/
char *tmedia_codec_get_rtpmap (const tmedia_codec_t *self)
{
    ...
    else if (self->type & tmedia_audio)
    {
        ...
        {
            tsk_sprintf (&rtpmap, "%s %s/8000/%d", self->neg_format ? self->neg_format : self->format,
                         self->name, self->plugin->audio.channels);
        }
        else
        {
            tsk_sprintf (&rtpmap, "%s %s", self->neg_format ? self->neg_format : self->format, self->name);
...
* @param self the codec for which to get the fmtp attribute. Should be created using @ref
* tmedia_codec_create().
* @retval sdp attribute attribute string (e.g. "mode-set=0,2,5,7; mode-change-period=2;
* mode-change-neighbor=1"). It's up to the caller to free the
* returned string.
*/
char *tmedia_codec_sdp_att_get (const tmedia_codec_t *self, const char *att_name)
{
    ...
    if (self->plugin->sdp_att_get)
    { /* some codecs, like G711, won't produce fmtp */
...
again:
    ...
    {
        if (!tsk_list_find_item_by_pred (codecs2keep, __pred_find_codec_by_format,
                                         ((tmedia_codec_t *)item->data)->format))
        {
...
int tmedia_codec_to_sdp (const tmedia_codecs_L_t *codecs, tsdp_header_M_t *m)
{
    ...
    tsk_list_foreach (item, codecs)
    {
        ...
        if (is_audio || is_video || is_text)
        {
            ...
            {
                tsdp_header_M_add_headers (m, TSDP_HEADER_A_VA_ARGS ("rtpmap", rtpmap), tsk_null);
                TSK_FREE (rtpmap);
            }
            ...
            if ((imageattr = tmedia_codec_sdp_att_get (codec, "imageattr")))
            {
                ...
                tsdp_header_M_add_headers (m, TSDP_HEADER_A_VA_ARGS ("imageattr", temp), tsk_null);
                TSK_FREE (temp);
                TSK_FREE (imageattr);
            }
            /* add fmtp attributes */
            if ((fmtp = tmedia_codec_sdp_att_get (codec, "fmtp")))
            {
                ...
                {
                    tsk_sprintf (&temp, "%s %s;sx=%d;sy=%d", neg_format, fmtp,
                                 tmedia_defaults_get_screen_x (), tmedia_defaults_get_screen_y ()); // youme clients
                }
                else
                {
                    tsk_sprintf (&temp, "%s %s", neg_format, fmtp);
                }
                tsdp_header_M_add_headers (m, TSDP_HEADER_A_VA_ARGS ("fmtp", temp), tsk_null);
                TSK_FREE (temp);
                TSK_FREE (fmtp);
            }
            ...
            {
                const tmedia_codec_t *codec_t140 =
                tsk_list_find_object_by_pred (codecs, __pred_find_codec_by_format, TMEDIA_CODEC_FORMAT_T140);
                if (codec_t140)
                {
                    const char *neg_format_t140 =
                    codec_t140->neg_format ? codec_t140->neg_format : codec_t140->format;
                    tsk_sprintf (&temp, "%s %s/%s/%s/%s", neg_format, neg_format_t140,
                                 neg_format_t140, neg_format_t140, neg_format_t140);
                    tsdp_header_M_add_headers (m, TSDP_HEADER_A_VA_ARGS ("fmtp", temp), tsk_null);
                    TSK_FREE (temp);
                }
            }
...
/**@ingroup tmedia_codec_group
* Finds a codec by format. If the codec has a dyn. payload type, then this function will also
* compare negociate formats.
...
* @param self The codec to deinitialize. Could be any type of codec (e.g. @ref tmedia_codec_audio_t
* or @ref tmedia_codec_video_t).
* @retval Zero if succeed and non-zero error code otherwise.
*/
