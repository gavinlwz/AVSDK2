...
bail:
		...
		unsigned width, height;
		tsk_size_t s;
		if(tmedia_parse_video_imageattr(att_value,  &in_width, &in_height, &out_width, &out_height) != 0){
			return tsk_false;
		}
...
{	
	...
	if (tsk_striequals(att_name, "fmtp")) {
		...
		return tmedia_get_video_imageattr(TMEDIA_CODEC_VIDEO(h264)->in.width,
                                          TMEDIA_CODEC_VIDEO(h264)->in.height,
                                          TMEDIA_CODEC_VIDEO(h264)->out.width,
                                          TMEDIA_CODEC_VIDEO(h264)->out.height);
	}
	return tsk_null;
}
...
typedef void (*webrtc_decode_h264_callback)(const unsigned char * buff, int bufflen, int64_t timestamp, const void* session, void* header, tsk_bool_t isTexture, int video_id);

typedef void (*webrtc_encode_h264_callback)(const unsigned char * buff, int bufflen, int64_t timestamp, int video_id,  const void* session);
