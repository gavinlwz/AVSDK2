...
#define CHECK_TRUE(_bool, _text)     \
    {                                \
        if (!_bool)                  \
        {                            \
            TSK_DEBUG_ERROR (_text); \
            return -1;               \
        }                            \
    }
#define CHECK_FALSE(_bool, _text)    \
    {                                \
        if (_bool)                   \
        {                            \
            TSK_DEBUG_ERROR (_text); \
...
:
  ...
  m_slPlayer (NULL), m_slEngine (NULL), m_slPlayerPlay (NULL), m_slPlayerSimpleBufferQueue (NULL), m_slOutputMixObject (NULL), m_slSpeakerVolume (NULL), m_slRecorder (NULL), m_slRecorderRecord (NULL),
  m_slAudioIODeviceCapabilities (NULL), m_slRecorderSimpleBufferQueue (NULL), m_slMicVolume (NULL), _playQueueSeq (0), _recCurrentSeq (0), _recBufferTotalSize (0), _recQueueSeq (0)
  ,m_bFakedRecording(false)
  ,m_nStartRecTimeMs(0)
  ,m_bRecDataAvail(false)
  ,m_recResult(SL_RESULT_SUCCESS)
  ,
#endif
  m_nMicDeviceId (0), m_pCallback (pCallback), m_bInitialized (false), m_bSpeakerInitialized (false), m_bSpeakerOn (true), m_bPlayoutInitialized (false), m_bRecordingInitialized (false),
  m_bMicrophoneInitialized (false), m_bStereoPlayout (false), m_bStereoRecording (false), m_nPlayoutSampleRate (PLAYOUT_SAMPLE_RATE), m_nRecordingSampleRate (RECORDING_SAMPLE_RATE),
  m_nRecordingBufferSize (RECORDING_BUFFER_SIZE), m_nPlayoutBufferSize (PLAYOUT_BUFFER_SIZE), m_bPlaying (false), m_bRecording (false), m_nSpeakerVolume (0), m_nMinSpeakerVolume (0), m_nMaxSpeakerVolume (0)
{
#if AUDIO_OPENSLES_UNDER_ANDROID
    ...
    m_pSilenceProducer = new SLAudioSilenceProducer();
#endif
}

SLAudioDevice::~SLAudioDevice ()
{
    if (m_pSilenceProducer) {
        delete m_pSilenceProducer;
        m_pSilenceProducer = NULL;
    }
}
...
void SLAudioDevice::AvailableAudioOutputsChangedCallback (SLAudioIODeviceCapabilitiesItf caller, void *pContext, SLuint32 deviceID, SLint32 numOutputs, SLboolean isNew)
{
}
int SLAudioDevice::Init ()
{
    ...
    TSK_DEBUG_INFO ("SLAudioDevice::Init()");
    ...
    {
        TSK_DEBUG_ERROR ("Failed to create Engine with error code = %lu", slResult);
        goto bail;
    }
    if ((slResult = (*m_slEngineObject)->Realize (m_slEngineObject, SL_BOOLEAN_FALSE)) != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to Realize SL Engine with erro code = %lu", slResult);
        goto bail;
    }
    if ((slResult = (*m_slEngineObject)->GetInterface (m_slEngineObject, SL_IID_ENGINE, (void *)&m_slEngine)) != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to get SL Engine interface with error code = %lu", slResult);
        goto bail;
    }

/*if ((slResult = (*m_slEngineObject)->GetInterface (m_slEngineObject, SL_IID_AUDIOIODEVICECAPABILITIES, (void *)&m_slAudioIODeviceCapabilities)) != SL_RESULT_SUCCESS)
{
    TSK_DEBUG_ERROR ("Failed to get SL AUDIOIODEVICECAPABILITIES interface with error code = %lu", slResult);
    return -1;
}

if ((slResult = (*m_slAudioIODeviceCapabilities)->RegisterAvailableAudioOutputsChangedCallback (m_slAudioIODeviceCapabilities, AvailableAudioOutputsChangedCallback, this)) != SL_RESULT_SUCCESS)
{
    TSK_DEBUG_ERROR ("Failed to get SL RegisterAvailableAudioOutputsChangedCallback  with error code = %lu", slResult);
    return -1;
}*/
    ...
    m_bInitialized = true;
    if (m_pSilenceProducer && tmedia_defaults_get_android_faked_recording()) {
        m_bFakedRecording = true;
        TSK_DEBUG_INFO ("#### intentional faked recording");
    }

    TSK_DEBUG_INFO ("SL engine initialized");
    return 0;

#if AUDIO_OPENSLES_UNDER_ANDROID
bail:
    // If it's for recording, even if the audio devices failed, we still want to output silence data to keep it running.
    // But keep the error code and report it to the server when stopping recording.
    m_recResult = slResult;

    if (m_pSilenceProducer) {
        m_bInitialized = true;
        m_bFakedRecording = true;
        TSK_DEBUG_INFO ("SL engine failed to init, but for recording, we fake it as if it succeeded");
        return 0;
    } else {
        return -1;
    }
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::InitSpeaker()");
...
int SLAudioDevice::SetMaxSpeakerVolume (int nMaxSpeakerVolume)
{
    ...
    TSK_DEBUG_INFO ("SetMaxSpeakerVolume(%d)", nMaxSpeakerVolume);
...
{
    CHECK_TRUE (m_bSpeakerInitialized, "Speaker not initialized");
    TSK_DEBUG_INFO ("SetMinSpeakerVolume(%d)", nMinSpeakerVolume);
...
{
    CHECK_TRUE (m_bSpeakerInitialized, "Speaker not initialized");
    TSK_DEBUG_INFO ("SetSpeakerVolume(%d)", nSpeakerVolume);
...
int SLAudioDevice::SetSpeakerOn (bool bSpeakerOn)
{
    ...
    TSK_DEBUG_INFO ("SetSpeakerOn(%s -> %s)", (m_bSpeakerOn ? "true" : "false"), (bSpeakerOn ? "true" : "false"));
    int ret = 0;
...
int SLAudioDevice::SetPlayoutSampleRate (int nPlayoutSampleRate)
{
    ...
    TSK_DEBUG_INFO ("SetPlayoutSampleRate(%d)", nPlayoutSampleRate);
    ...
    default:
    {
        TSK_DEBUG_ERROR ("%d not valid sampling rate", nPlayoutSampleRate);
        return -1;
    }
    }
}
...
int SLAudioDevice::InitPlayout ()
{
    ...
    TSK_DEBUG_INFO ("SLAudioDevice::InitPlayout()");
    ...
    if (m_bPlaying)
    {
        TSK_DEBUG_ERROR ("Playout already started");
        return -1;
    }
    ...
    if (InitSpeaker ())
    {
        TSK_DEBUG_ERROR ("InitSpeaker() failed");
    }
    ...
    {
        TSK_DEBUG_ERROR ("SLObject or Engiine is NULL");
        return -1;
    }
    ...
    {
        TSK_DEBUG_ERROR ("CreateOutputMix() for playout failed with error code = %lu", slResult);
        return -1;
    }

    if ((slResult = (*m_slOutputMixObject)->Realize (m_slOutputMixObject, SL_BOOLEAN_FALSE)) != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to realize SL Output Mix object for playout with error code = %lu", slResult);
        return -1;
    }
    ...
    // Create the player
    if ((slResult = (*m_slEngine)->CreateAudioPlayer (m_slEngine, &m_slPlayer, &audioSource, &audioSink, 4, ids, req)) != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to create Audio Player with error code = %lu", slResult);
        return -1;
    }
    ...
    {
        TSK_DEBUG_ERROR ("Failed to get player configuration with error code = %lu", slResult);
        return -1;
    }
    else
    {
        static SLint32 _playerStreamType = SL_ANDROID_STREAM_MEDIA;
        if (tmedia_defaults_get_comm_mode_enabled())
        {
            _playerStreamType = SL_ANDROID_STREAM_VOICE;
            TSK_DEBUG_INFO("## player set voice stream:SL_ANDROID_STREAM_VOICE");
        }
        else
        {
            _playerStreamType = SL_ANDROID_STREAM_MEDIA;
            TSK_DEBUG_INFO("## player set voice stream:SL_ANDROID_STREAM_MEDIA");
        }

        static SLint32 _playerStreamTypeSize = sizeof (SLint32);
        TSK_DEBUG_INFO ("_playerStreamType=%ld", _playerStreamType);
        ...
        {
            TSK_DEBUG_ERROR ("Failed to set player stream type with error code = %lu", slResult);
            return -2;
        }
    }
    ...
    {
        TSK_DEBUG_ERROR ("Failed to realize the player with error code = %lu", slResult);
        return -1;
    }
    // Get seek and play interfaces
    if ((slResult = (*m_slPlayer)->GetInterface (m_slPlayer, SL_IID_PLAY, (void *)&m_slPlayerPlay)) != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to get Player interface with error code = %lu", slResult);
        return -1;
    }
    ...
    {
        TSK_DEBUG_ERROR ("Failed to get Player Simple Buffer Queue interface with error code = %lu", slResult);
        return -1;
    }
    ...
    {
        TSK_DEBUG_ERROR ("Failed to register Player Callback");
        return -1;
    }
#endif /* AUDIO_OPENSLES_UNDER_ANDROID */
    ...
    TSK_DEBUG_INFO ("Playout initialized");
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::StartPlayout()");
    ...
    {
        TSK_DEBUG_ERROR ("PlayItf is NULL");
        return -1;
    }
    if (m_slPlayerSimpleBufferQueue == NULL)
    {
        TSK_DEBUG_ERROR ("PlayerSimpleBufferQueue is NULL");
        return -1;
    }
    ...
    uint32_t noSamplesOut (0);
    {
        ...
        {
            TSK_DEBUG_WARN ("%d not expected as samples output count value", noSamplesOut);
        ...
        {
            TSK_DEBUG_ERROR ("Player simpler buffer queue Enqueue failed with error code = %lu and noSamplesOut = %u", slResult, noSamplesOut);
        }
    ...
    {
        TSK_DEBUG_ERROR ("Failed to start playout with error code = %lu", slResult);
        m_bPlaying = false;
        return -1;
    }
#else
    ...
    TSK_DEBUG_INFO ("Payout started - rate=%d", m_nPlayoutSampleRate);
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::StopPlayout()");
    ...
    if ((m_slPlayerPlay != NULL) && (m_slOutputMixObject != NULL) && (m_slPlayer != NULL))
    {
        ...
        {
            TSK_DEBUG_ERROR ("Failed to stop playout with error code = %lu", slResult);
            return -1;
        }

        if ((slResult = (*m_slPlayerSimpleBufferQueue)->Clear (m_slPlayerSimpleBufferQueue)) != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_ERROR ("Failed to clear recorder buffer queue");
            return -1;
        }
    ...
    TSK_DEBUG_INFO ("Playout stopped");
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::InitMicrophone()");
    CHECK_TRUE (m_bInitialized, "Device not initialized");
...
int SLAudioDevice::SetMicrophoneVolume (int nMicrophoneVolume)
{
    ...
    TSK_DEBUG_INFO ("SetMicrophoneVolume(%d)", nMicrophoneVolume);

#if AUDIO_OPENSLES_UNDER_ANDROID
    if (m_bFakedRecording) {
        return 0;
    }
    ...
    {
        SLresult slResult;
        if ((slResult = (*m_slEngineObject)->GetInterface (m_slEngineObject, SL_IID_VOLUME, (void *)&m_slMicVolume)) != SL_RESULT_SUCCESS)
        {
            //TSK_DEBUG_ERROR ("Failed to get 'SL_IID_VOLUME' interface with error code = %lu", slResult);
            return -1;
        }
    }
    ...
    if (m_slMicVolume != NULL)
    {
        ...
        {
            TSK_DEBUG_ERROR ("SetVolume() failed with error code = %lu", slResult);
            return -1;
        }
    }
#endif /* AUDIO_OPENSLES_UNDER_ANDROID */
...
int SLAudioDevice::GetRecordingError(int32_t *pErrCode)
{
    if (NULL == pErrCode) {
        return -1;
    }
    
    if (SL_RESULT_CONTENT_UNSUPPORTED == m_recResult) {
        *pErrCode = YOUME_ERROR_REC_NO_PERMISSION;
    } else if ((SL_RESULT_SUCCESS != m_recResult) || !m_bRecording) {
        *pErrCode = YOUME_ERROR_REC_OTHERS;
    } else {
        // Init OK but no data available yet.
        uint64_t timeSinceRec = tsk_gettimeofday_ms() - m_nStartRecTimeMs;
        while ((timeSinceRec < 500) && !m_bRecDataAvail) {
            usleep(20000);
            timeSinceRec = tsk_gettimeofday_ms() - m_nStartRecTimeMs;
        }

        if (m_bRecDataAvail) {
            *pErrCode = YOUME_SUCCESS;
        } else {
            *pErrCode = YOUME_ERROR_REC_NO_DATA;
        }
    }
...
int SLAudioDevice::GetRecordingErrorExtra(int32_t *pErrExtra)
{
    if (NULL == pErrExtra) {
        return -1;
    }
...
int SLAudioDevice::SetStereoRecording (bool bEnabled)
{
    CHECK_TRUE (m_bInitialized, "Not initialized");
    CHECK_RECORDING_NOT_INITIALIZED ();
    TSK_DEBUG_INFO ("SetStereoRecording(%s)", bEnabled ? "True" : "False");
...
int SLAudioDevice::SetRecordingSampleRate (int nRecordingSampleRate)
{
    ...
    TSK_DEBUG_INFO ("SetRecordingSampleRate(%d)", nRecordingSampleRate);
    ...
    default:
    {
        TSK_DEBUG_ERROR ("%d not valid sampling rate", nRecordingSampleRate);
        return -1;
    }
    }
}
...
int SLAudioDevice::InitRecording ()
{
    ...
    TSK_DEBUG_INFO ("SLAudioDevice::InitRecording()");

    if (m_bRecording)
    {
        TSK_DEBUG_ERROR ("Recording already started");
        return -1;
    }
    ...
    if (InitMicrophone () == -1)
    {
        TSK_DEBUG_ERROR ("InitMicrophone() failed");
    }
    ...
    if (m_slEngineObject == NULL || m_slEngine == NULL)
    {
        TSK_DEBUG_ERROR ("Recording object is NULL");
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to create Recorder with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (tmedia_defaults_get_comm_mode_enabled())
    {
        slStreamType = SL_ANDROID_RECORDING_PRESET_VOICE_COMMUNICATION;
        TSK_DEBUG_INFO("Record: set voice stream");
    }
    else
    {
        TSK_DEBUG_INFO("Record: set media stream");
    }
    slResult = (*m_slRecorder)->GetInterface (m_slRecorder, SL_IID_ANDROIDCONFIGURATION, &slRecorderConfig);
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("GetInterface(SL_IID_ANDROIDCONFIGURATION) failed with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("SetConfiguration(SL_ANDROID_KEY_RECORDING_PRESET) failed with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to realize Recorder with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to get Recorder interface with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to get Recorder Simple Buffer Queue with error code = %lu", slResult);
        goto bail;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to register Recorder Callback with error code = %lu", slResult);
        goto bail;
    }

#endif /* AUDIO_OPENSLES_UNDER_ANDROID */

    TSK_DEBUG_INFO ("Recording initialized");
...
#if AUDIO_OPENSLES_UNDER_ANDROID
bail:
    // We will report the first error. If there's error occured, do not overlap it.
    if (SL_RESULT_SUCCESS == m_recResult) {
        m_recResult = slResult;
    }
    
    if (m_pSilenceProducer) {
        TSK_DEBUG_INFO ("Faked recording initialized");
        m_bRecordingInitialized = true;
        m_bFakedRecording = true;
        return 0;
    } else {
...
int SLAudioDevice::StartRecording ()
{
    ...
    TSK_DEBUG_INFO ("SLAudioDevice::StartRecording()");
...
#if AUDIO_OPENSLES_UNDER_ANDROID
    // The slience producer should be started before the real recording starts.
    // Because if there's OpenSL recorder callback, it will stop the SilenceProducer.
    // To avoid contension, SilenceProducer should be ready before any recorder callback.
    if (m_pSilenceProducer) {
        int firstWaitTimeMs = 2000;
        // If it's a faked recording, we don't need to wait 2 seconds before sending the silence packets.
        if (m_bFakedRecording) {
            firstWaitTimeMs = 10;
        }
        m_pSilenceProducer->start(firstWaitTimeMs, 10, m_nRecordingSampleRate /100, SilenceProducerCallback, (void*)this);
    }
    ...
    {
        TSK_DEBUG_ERROR ("RecordITF is NULL");
        goto bail;
    }

    if (m_slRecorderSimpleBufferQueue == NULL)
    {
        TSK_DEBUG_ERROR ("Recorder Simple Buffer Queue is NULL");
        goto bail;
    }
    ...
    nSample10ms = m_nRecordingSampleRate / 100;
    for (int i = 0; i < (N_REC_QUEUE_BUFFERS - 1); i++)
    {
        ...
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_ERROR ("Failed to Enqueue Empty Buffer to recorder with error code = %lu", slResult);
            goto bail;
        }
        _recQueueSeq++;
    }
    ...
    if (slResult != SL_RESULT_SUCCESS)
    {
        TSK_DEBUG_ERROR ("Failed to start recording with error code = %lu", slResult);
        m_bRecording = false;
        goto bail;
    }
#else
    ...
    m_nStartRecTimeMs = tsk_gettimeofday_ms();
    m_bRecDataAvail   = false;
    TSK_DEBUG_INFO ("Recording started - rate = %d", m_nRecordingSampleRate);
    return 0;
...
bail:
    ...
    // We will only report the first error. If there's previous error occured, do not overlap it.
    if (SL_RESULT_SUCCESS == m_recResult) {
        m_recResult = slResult;
    }
    
    if (m_pSilenceProducer) {
        TSK_DEBUG_INFO ("Faked recording started - rate = %d", m_nRecordingSampleRate);
        m_bRecording      = true;
        m_bFakedRecording = true;
        return 0;
    } else {
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::StopRecording()");
    if (!m_bRecording)
    {
        return 0;
    }
    ...
    if ((m_slRecorderRecord != NULL) && (m_slRecorder != NULL))
    {
        ...
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_ERROR ("Failed to stop recording with error code = %lu", slResult);
            return -1;
        }
        slResult = (*m_slRecorderSimpleBufferQueue)->Clear (m_slRecorderSimpleBufferQueue);
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_ERROR ("Failed to clear recorder buffer queue with error code = %lu", slResult);
            return -1;
        }
    ...
    // The slience producer should be stopped after the real recording stops.
    // Because if there's OpenSL recorder callback, it will stop the SilenceProducer.
    // To avoid contension, the stop should be called when we are sure that there's no recording active.
    if (m_pSilenceProducer) {
        m_pSilenceProducer->stop();
    }
#endif //AUDIO_OPENSLES_UNDER_ANDROID

    TSK_DEBUG_INFO ("Recording stopped");
    m_bRecording = false;
...
{
    TSK_DEBUG_INFO ("SLAudioDevice::PlayoutApplyNewConfig()");
#if AUDIO_OPENSLES_UNDER_ANDROID
    if (m_slPlayer)
    {
        SLresult slResult;
        ...
        if (wasPlaying)
        {
            if ((ret = StopPlayout ()))
            {
                TSK_DEBUG_ERROR ("Failed to stop playout for reconf");
                return ret;
            }
            if ((ret = InitPlayout ()))
            {
                TSK_DEBUG_ERROR ("Failed to init() playout after reconf");
                return ret;
            }
        }
        ...
        if (wasPlaying)
        {
            if ((ret = StartPlayout ()))
            {
                TSK_DEBUG_ERROR ("Failed to start() playout after reconf");
                return ret;
            }
        }
    }
...
uint32_t SLAudioDevice::PullPlayoutData (void *pAudioSamples, const uint32_t nSamples)
{
    ...
    {
        TSK_DEBUG_ERROR ("PullPlayoutData() - Invalid parameter");
        return 0;
    }
    ...
    const_cast<SLAudioDeviceCallback *> (m_pCallback)->NeedMorePlayData (nSamples, BYTES_PER_SAMPLE, m_bStereoPlayout ? 2 : 1, m_nPlayoutSampleRate, pAudioSamples, nSamplesOut);
    return nSamplesOut;
}
...
void SLAudioDevice::PushRecordingData (void *pAudioSamples, const uint32_t nSamples)
{
    if (!pAudioSamples || !nSamples)
    {
        TSK_DEBUG_ERROR ("PushRecordingData() - Invalid parameter");
        return;
    }

    if (m_pCallback)
    {
        const_cast<SLAudioDeviceCallback *> (m_pCallback)->RecordedDataIsAvailable (pAudioSamples, nSamples, BYTES_PER_SAMPLE, m_bStereoRecording ? 2 : 1, m_nRecordingSampleRate);
    }
}
...
void SLAudioDevice::PlayerSimpleBufferQueueCallback (SLAndroidSimpleBufferQueueItf queueItf, void *pContext)
{
    ...
    // TSK_DEBUG_INFO("PlayerSimpleBufferQueueCallback(playing=%s, _playQueueSeq=%d)", (This->m_bPlaying ? "true" : "false"), This->_playQueueSeq);
    ...
    if (This->m_bPlaying && (This->_playQueueSeq < N_PLAY_QUEUE_BUFFERS))
    {
        ...
        {
            if (noSamplesOut)
            { // (noSamplesOut==0) -> jitter buffer cannot provide data
                TSK_DEBUG_ERROR ("noSamp10ms (%u) != noSamplesOut (%d)", noSamp10ms, noSamplesOut);
            }
        ...
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_ERROR ("Player simpler buffer queue Enqueue failed, noSamplesOut=%d, ret=%lu", noSamplesOut, slResult);
            return;
        }
...
{
    // TSK_DEBUG_INFO("RecorderSimpleBufferQueueCallback()");
    ...
    if (This->m_bRecording)
    {
        ...
        // push data
        if (This->m_pSilenceProducer) {
            This->m_pSilenceProducer->stop();
        }
        This->m_bRecDataAvail = true;

        This->PushRecordingData (This->_recQueueBuffer[0], noSamp10ms);
        // enqueue new buffer
        SLresult slResult = (*This->m_slRecorderSimpleBufferQueue)->Enqueue (This->m_slRecorderSimpleBufferQueue, (void *)This->_recQueueBuffer[0], (noSamp10ms << BYTES_PER_SAMPLE_LOG2));
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_WARN ("Failed to enqueue recording buffer with error code = %lu", slResult);
            return;
        }
#else
        ...
        while (dataPos < noSamp10ms)
        {
            ...
            else
            {
                ...
                TSK_DEBUG_WARN ("Could not insert into recording buffer");
        ...
        SLresult slResult = (*This->m_slRecorderSimpleBufferQueue)->Enqueue (This->m_slRecorderSimpleBufferQueue, (void *)This->_recQueueBuffer[This->_recQueueSeq], (noSamp10ms << BYTES_PER_SAMPLE_LOG2));
        if (slResult != SL_RESULT_SUCCESS)
        {
            TSK_DEBUG_WARN ("Failed to enqueue recording buffer with error code = %lu", slResult);
            return;
...
void SLAudioDevice::SilenceProducerCallback(void* pSamples, int nSampleNum, void* pContext)
{
    SLAudioDevice *This = static_cast<SLAudioDevice *> (pContext);
    This->PushRecordingData (pSamples, nSampleNum);
}

////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
// This class is intended to render silence audio data while there's no data from the recorder.
// On iOS, if there's no recording permission, the recording will feed back with silence data,
// this class will silumate this behavior.
//
SLAudioSilenceProducer::SLAudioSilenceProducer()
{
    m_bInited        = false;
    m_bCancelled     = false;
    m_bRunning       = false;
    ...
    memset(m_pcmBuf, 0, sizeof(m_pcmBuf));
    m_bInited = true;
    return;

bail:
    TSK_DEBUG_ERROR("SLAudioSilenceProducer failed to construct");
}
...
bool SLAudioSilenceProducer::start(int firstWaitTimeMs, int frameDurationMs, int frameSampleNum,
                                       RecDataCb_t recDataCb, void* recDataCbContext)
{
    ...
    m_firstWaitTimeMs   = firstWaitTimeMs;
    ...
    m_bRunning = true;
    int ret = pthread_create(&m_threadId, NULL, &SilenceProducerThread, (void*)this);
    if (0 == ret) {
        bRet = true;
    } else {
        m_bRunning = false;
...
void SLAudioSilenceProducer::stop()
{
    if (!m_bInited || !m_bRunning) {
        return;
    }
...
void* SLAudioSilenceProducer::SilenceProducerThread(void* pParam)
{
    SLAudioSilenceProducer* pThis = static_cast<SLAudioSilenceProducer *>(pParam);
    struct timespec timeoutSpec;

    TSK_DEBUG_INFO("#############################################");
    TSK_DEBUG_INFO("## SilenceProducerThread enter...");
    TSK_DEBUG_INFO("#############################################");
    
    // Wait for the recorder to start working
    ...
    if (!pThis->m_bCancelled) {
        getTimeoutSpec(&timeoutSpec, pThis->m_firstWaitTimeMs);
    ...
    do {
        if (pThis->m_recDataCb) {
            pThis->m_recDataCb(pThis->m_pcmBuf, pThis->m_frameSampleNum, pThis->m_recDataCbContext);
        }
        ...
        // Before waiting, we should check the flag first, otherwise, we may miss a signal.
        if (!pThis->m_bCancelled) {
            getTimeoutSpec(&timeoutSpec, pThis->m_frameDurationMs);
            pthread_cond_timedwait(&(pThis->m_waitCond), &(pThis->m_cancelFlagLock), &timeoutSpec);
        }
        if (pThis->m_bCancelled) {
            pthread_mutex_unlock(&(pThis->m_cancelFlagLock));
...
bail:
    TSK_DEBUG_INFO("#############################################");
    TSK_DEBUG_INFO("## SilenceProducerThread exit");
    TSK_DEBUG_INFO("#############################################");
