...
static void* TSK_STDCALL _tdav_consumer_alsa_playback_thread(void *param)
{
	tdav_consumer_alsa_t*  p_alsa = (tdav_consumer_alsa_t*)param;
	int err;

	tsk_thread_set_priority_2(TSK_THREAD_PRIORITY_TIME_CRITICAL);
	tsk_size_t tempBufferSize = p_alsa->alsa_common.n_buff_size_in_bytes;
	uint8_t* tempBufferPtr = tsk_calloc(1, tempBufferSize);
#ifdef OS_LINUX
    prctl(PR_SET_NAME, "consumer_alsa_playback");
#endif
	...
	while (p_alsa->b_started) {
		...
		err = tdav_consumer_audio_get(TDAV_CONSUMER_AUDIO(p_alsa), tempBufferPtr, tempBufferSize); // requires 16bits, thread-safe
...
static int tdav_consumer_alsa_set(tmedia_consumer_t* self, const tmedia_param_t* param)
{
	...
	ret = tdav_consumer_audio_set_param(TDAV_CONSUMER_AUDIO(self), param);

	return ret;
}

static int tdav_consumer_alsa_get(tmedia_consumer_t* self, const tmedia_param_t* param)
{
	tdav_consumer_alsa_t* p_alsa = (tdav_consumer_alsa_t*)self;
	int ret = 0;
...
static int tdav_consumer_alsa_start(tmedia_consumer_t* self)
{
	...
	if (err) {
	 ...
	 tsk_thread_create(&p_alsa->tid[0], _tdav_consumer_alsa_playback_thread,  p_alsa);
...
static int tdav_consumer_alsa_consume(tmedia_consumer_t* self, const void* buffer, tsk_size_t size, const tsk_object_t* proto_hdr)
{
	int err = 0;
	tdav_consumer_alsa_t* p_alsa = (tdav_consumer_alsa_t*)self;
	// TSK_DEBUG_INFO("tdav_consumer_alsa_consume self:%p, buffer:%p, size:%d, start:%d", self, buffer, size, p_alsa->b_started);
