...
static trtp_manager_t *_trtp_manager_create (const char *local_ip, tsk_bool_t ipv6, int sessionid)
{
    ...
    if ((manager = tsk_object_new (trtp_manager_def_t))) {
        ...
        manager->send_thread_cond = NULL; //not use now
...
static int _trtp_manager_recv_data (const trtp_manager_t *self, 
    ...
    if (data_size <= 16) {
        if (16 == data_size) {
            ...
            if (self->sessionid != sessionId) {
                ...
                uint32_t dummyEchoDelay = curTimeMs - self->last_time_send_dummy_packet_ms;
                Config_SetInt("dummy_rtt", dummyEchoDelay <= Config_GetInt("data_rtt",0) ? dummyEchoDelay : 50);
                if ((curTimeMs - self->last_time_print_recv_packet_ms) >= (10 * 1000)) {
                    TSK_DEBUG_INFO("Got dummy packet from me:%d delay:%d", sessionId,Config_GetInt("dummy_rtt",0));
    ...
    if (is_rtcp) {
        ...
        //packet loss rate 2% 4% 6% 8% 10%

        trtp_rtp_packet_t *rtpPacket = tsk_null;
        rtpPacket = trtp_rtp_packet_deserialize(data_ptr,data_size);

        //info: type 105 视频包; 111 音频包;
        int type = rtpPacket->header->payload_type;
        static uint32_t videoCnt = 0;
        static uint32_t audioCnt = 0;

        int packetLossRate = 20;
        int packetLossSeed = 100/packetLossRate;

        if(type == 105){
            ++videoCnt;
            // TSK_DEBUG_ERROR("mark: recv rtp packet seq:[%u]", rtpPacket->header->seq_num);
            if(videoCnt%packetLossSeed == 0)
            {
                TSK_OBJECT_SAFE_FREE(rtpPacket);
                // TSK_DEBUG_ERROR("mark: discard packet seq:[%u]", rtpPacket->header->seq_num);
                return 0;
            }
        }
        TSK_OBJECT_SAFE_FREE(rtpPacket);
        ...
        if (self->rtp.cb.fun) {
            ...
            if ((packet_rtp = trtp_rtp_packet_deserialize (data_ptr, data_size))) {
                ...
                if (self->rtcp.session) {
                    tsk_bool_t qos_stat = tsk_true;
                    if (packet_rtp->header->payload_type == TRTP_H264_MP && self->rtp.video_nack_check_cb.fun) {
                        int retransmit = self->rtp.video_nack_check_cb.fun (self->rtp.video_nack_check_cb.usrdata, packet_rtp);
                        if (1 == retransmit || 2 == retransmit) { // 1:重复包，2:重传包
                            qos_stat = tsk_false;
                            if (1 == retransmit) {
...
// 设置rtp video nack check回调
int trtp_manager_set_video_nack_check_callback(trtp_manager_t *self, trtp_rtp_nack_check_cb_f fun, const void *usrdata)
{
    if (!self)
    {
        TSK_DEBUG_ERROR("Invalid parameter");
    ...
    self->rtp.video_nack_check_cb.fun = fun;
    self->rtp.video_nack_check_cb.usrdata = usrdata;
...
static void* TSK_STDCALL trtp_manager_rtp_send_thread_func(void *arg) {
    ...
    unsigned int oneLoopPktCount = 2;
	int forbidenReducePktCount = 0;
    int useTCP = Config_GetInt("rtp_use_tcp", 0);
    int sleepTime = 5;//ms
    uint64_t ppsStartTime = tsk_gettimeofday_ms();
    uint64_t pps = 0;
            ...
            {
                {
                    ...
                    if ( (!useTCP) ) {
                        ...
                        if( sendCounter % 1000 == 0) // pps update
                        {
                            uint64_t now = tsk_gettimeofday_ms();
                            pps = 1000000 / (tsk_gettimeofday_ms() - ppsStartTime) ;//1000pkt /diff ms/ * 1000 to second
                            ppsStartTime = now;
                            TMEDIA_I_AM_ACTIVE(5,"send pps %lld", pps);
                        }
                        ...
                        if ( ++sendCounter % oneLoopPktCount == 0) {
                             ...
                             */
                            sleepTime = list_count > 20 ? 2 : 4;
                            if(pps > 500)
                            {
                                sleepTime = 2;
                                oneLoopPktCount = pps * 3 / 500;
                            }else{
                                if (list_count > 500) {
                                    sleepTime = 1;
                                    oneLoopPktCount = 8;
                                    if(list_count > 1000) oneLoopPktCount = 10;
                                    TSK_DEBUG_INFO("pkg more %d",list_count);
                                }else if (list_count > 300){
                                    oneLoopPktCount = 6;
                                }else if (list_count > 200){
                                    oneLoopPktCount = 4;
                                }else{
                                    oneLoopPktCount = 2;
                                }
                        ...
                        /* nanosleep test
                         uint64_t usec = list_count > 20 ? 1000 : 5000;
...
/* enlarge socket buffer */
    ...
    ret = setsockopt (self->transport->master->fd, SOL_SOCKET, SO_RCVBUF, (char *)&rcv_buf, sizeof (rcv_buf));
    if (ret)
    {
        TNET_PRINT_LAST_ERROR ("setsockopt(SOL_SOCKET, SO_RCVBUF, %d) has failed with error code %d", rcv_buf, ret);
    }
    ret = setsockopt (self->transport->master->fd, SOL_SOCKET, SO_SNDBUF, (char *)&snd_buf, sizeof (snd_buf));
    if (ret)
    {
...
#if !TRTP_UNDER_WINDOWS_CE
    if (self->transport_p2p && self->transport_p2p->master) {
        TSK_DEBUG_INFO ("SO_RCVBUF = %d, SO_SNDBUF = %d", rcv_buf, snd_buf);
        ret = setsockopt (self->transport_p2p->master->fd, SOL_SOCKET, SO_RCVBUF, (char *)&rcv_buf, sizeof (rcv_buf));
        if (ret)
        {
            TNET_PRINT_LAST_ERROR ("P2P setsockopt(SOL_SOCKET, SO_RCVBUF, %d) has failed with error code %d", rcv_buf, ret);
        }
        ret = setsockopt (self->transport_p2p->master->fd, SOL_SOCKET, SO_SNDBUF, (char *)&snd_buf, sizeof (snd_buf));
        if (ret)
        {
...
// #endif /* !TRTP_UNDER_WINDOWS_CE */
    ...
    if( self->transport_report && self->transport_report->master )
    {
        ...
        ret = setsockopt (self->transport_report->master->fd, SOL_SOCKET, SO_RCVBUF, (char *)&report_rcv_buf, sizeof (report_rcv_buf));
        if (ret)
        {
            TNET_PRINT_LAST_ERROR ("Report setsockopt(SOL_SOCKET, SO_RCVBUF, %d) has failed with error code %d", report_rcv_buf, ret);
        }

        ret = setsockopt (self->transport_report->master->fd, SOL_SOCKET, SO_SNDBUF, (char *)&report_snd_buf, sizeof (report_snd_buf));
        if (ret)
        {
    ...
    if(self->use_rtcp) {
        ...
        if(self->rtcp.session) {
            trtp_rtcp_session_set_callback(self->rtcp.session, self->rtcp.cb.fun, self->rtcp.cb.usrdata);
            trtp_rtcp_session_set_video_swicth_callback(self->rtcp.session, self->rtp.video_switch_cb.fun, self->rtp.video_switch_cb.usrdata);
            trtp_rtcp_session_set_app_bw_and_jcng(self->rtcp.session, self->app_bw_max_upload, self->app_bw_max_download, self->app_jitter_cng);
            trtp_rtcp_session_set_net_transport(self->rtcp.session, self->transport);
            if (self->transport_p2p) {
                trtp_rtcp_session_set_p2p_transport(self->rtcp.session, self->transport_p2p, self->transport_p2p->master->fd, (const struct sockaddr *)&self->p2p_connect_info.p2p_remote_addr);
            }
...
tsk_bool_t trtp_manager_send_rtp_dummy (trtp_manager_t *self)
{
    ...
    if ( (curTimeMs - self->last_time_send_dummy_packet_ms) < 2000 ){
            return tsk_true;
    }
    ...
    TMEDIA_I_AM_ACTIVE (5,"Sent a dummy packet");
    ...
    if (bFirstInsert) {
        ...
        else
        {
            ...
            if (packet->header->frameType == 0) {
                ...
                if (status && (packet->header->frameSerial == status->LastFrameSerial) && (!status->bLossPacket)) {
                ...
                if (tsk_list_count_all(self->rtp_packets_list) > 2* MAX_SEND_LIST_COUNT) {
                    ...
                    if (status && (status->LastFrameSerial == packet->header->frameSerial)) {
