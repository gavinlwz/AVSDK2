...
:                                                               :
 ...
 P (1 bit) : RTCP 报文体4bytes对齐标志

 FMT（5 bits）: 指示FB消息的类型，
        1 NACK；
        2 NACK整帧丢失标志（PID+BLP保存被丢失帧的timestamp值）；
        3 TMMBR；
        4 TMMBN。
 
 PT （8 bits）: 指示RTCP包类型， PT= 205。
        205 传输层FB消息 （RTPFB）；
        206 特定载荷FB消息（PSFB）
 
 length (16 bits): rtcp报文长度（单位是4字节， 不包含第一个4字节）

 length of media source_1 (4 bytes): (不包含length所占4字节长度，仅包含source ssrc和PID/BLP的长度)
    请求source nack请求占用的body长度
 
 SSRC of media source_1 (32 bits): （即所请求源youme的sessionid）
    请求source的ssrc
 
 PID（16 bits）: 为丢失RTP数据包的序列号。

 BLP（16 bits）: 目前表示请求重传包（pid）对应的流id（目前有三路流：0: camera大流，1:camera小流，2:共享流）
 */
...
// for nack request header
static int _trtp_rtcp_report_rtpfb_serialize_to(const trtp_rtcp_report_fb_t* self, void* data, tsk_size_t size)
{
    int ret;
...
// for fir request header
static int _trtp_rtcp_report_psfb_serialize_to(const trtp_rtcp_report_fb_t* self, void* data, tsk_size_t size)
{
    int ret;
    uint8_t* pdata = (uint8_t*)data;
...
static tsk_object_t* trtp_rtcp_report_rtpfb_dtor(tsk_object_t * self)
{
    ...
    if(rtpfb) {
        ...
        case trtp_rtcp_rtpfb_fci_type_nack: {
            if (rtpfb->v_nack->size() > 0) {
                std::vector<trtp_rtcp_report_source_t*>::const_iterator iter = rtpfb->v_nack->begin();
                for ( ; iter != rtpfb->v_nack->end(); ) {
                    trtp_rtcp_report_source_t* item = (trtp_rtcp_report_source_t*)(*iter);
                    if (item) {
                        TSK_FREE(item->pid);
                        TSK_FREE(item->blp);
                        TSK_FREE(item);
                    }
                    iter = rtpfb->v_nack->erase (iter);
                }
            }
            rtpfb->v_nack->clear ();
            delete  rtpfb->v_nack;
...
trtp_rtcp_report_rtpfb_t* trtp_rtcp_report_rtpfb_build_nack(uint32_t ssrc_sender, trtp_rtcp_source_seq_map * source_seqs) {
    ...
    if((rtpfb = trtp_rtcp_report_rtpfb_create_2(trtp_rtcp_rtpfb_fci_type_nack, ssrc_sender))) {
        ...
        for (; iter != source_seqs->end(); ++iter) {
            ...
            uint32_t count = (*iter)->nack_list->size();
            uint16_t videoid =  (*iter)->videoid;
            report_source->ssrc_media = (*iter)->sessionid;
            ...
            if (!report_source->blp || !report_source->pid) {
                TSK_FREE (report_source);
                continue;
            }

            tsk_size_t k = 0;
            std::list<uint16_t>::iterator iter2 = (*iter)->nack_list->begin();
            for (; iter2 != (*iter)->nack_list->end(); ++iter2) {
                report_source->pid[k] = *iter2;
                report_source->blp[k] = videoid;
                k++;
            }

            report_source->count = count;
            report_source->source_size = sizeof(uint32_t) + (count << 2);
            source_length += report_source->source_size + 4; // include length of media source_1 (4 bytes)
            rtpfb->v_nack->emplace_back(report_source);

            //TSK_DEBUG_ERROR("mark send nack packet source[%u] seq count[%u] videoid[%u] size[%u]"
            //            , (*iter)->sessionid, report_source->count, videoid, report_source->source_size);
        }
        ...
        TMEDIA_I_AM_ACTIVE(20, "send nack packet body size[%d]", TRTP_RTCP_PACKET(rtpfb)->header->length_in_bytes);
    }
...
int trtp_rtcp_report_rtpfb_serialize_to(const trtp_rtcp_report_rtpfb_t* self, void* data, tsk_size_t size)
{
    ...
    if((ret = _trtp_rtcp_report_rtpfb_serialize_to(TRTP_RTCP_REPORT_FB(self), pdata, size))) {
    ...
    // TSK_DEBUG_ERROR("mark send nack packet source: %d", self->v_nack.size());
    ...
    for (; viter != self->v_nack->end(); ++viter) {
        ...
        int i  =0;
        for(; i < (*viter)->count; ++i) {
            pdata[0] = ((*viter)->pid[i] >> 8) & 0xFF;
            pdata[1] = ((*viter)->pid[i] & 0xFF);
            pdata[2] = ((*viter)->blp[i] >> 8) & 0xFF;
...
int trtp_rtcp_report_psfb_serialize_to(const trtp_rtcp_report_psfb_t* self, void* data, tsk_size_t size)
{
    ...
    if((ret = _trtp_rtcp_report_psfb_serialize_to(TRTP_RTCP_REPORT_FB(self), pdata, size))) {
        TSK_DEBUG_ERROR("Failed to serialize FB message");
        return ret;
    }
    pdata += TRTP_RTCP_PACKET_FB_MIN_SIZE;
