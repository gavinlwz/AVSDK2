...
void WebRtcNsx_SpeechNoiseProb(NoiseSuppressionFixedC* inst,
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    normTmp = YoumeWebRtcSpl_NormU32(postLocSnr[i]);
    ...
    // Here, LRT_TAVG = 0.5
    zeros = YoumeWebRtcSpl_NormU32(priorLocSnr[i]);
  ...
  if (inst->weightSpecFlat) {
    ...
    tmpU32no1 = YoumeWebRtcSpl_DivU32U16(tmpU32no2 << nShifts, 25);  // Q14
    // compute indicator function: sigmoid map
    // FLOAT code
  ...
  if (inst->weightSpecDiff) {
    ...
    if (inst->featureSpecDiff) {
      normTmp = WEBRTC_SPL_MIN(20 - inst->stages,
                               YoumeWebRtcSpl_NormU32(inst->featureSpecDiff));
      assert(normTmp >= 0);
  ...
  indPriorFX16 = YoumeWebRtcSpl_DivW32W16ResW16(98307 - indPriorFX, 6); // Q14
  ...
  if (inst->priorNonSpeechProb > 0) {
    for (i = 0; i < inst->magnLen; i++) {
      ...
      if (inst->logLrtTimeAvgW32[i] < 65300) {
        ...
        normTmp = YoumeWebRtcSpl_NormW32(invLrtFX);
        normTmp2 = YoumeWebRtcSpl_NormW16((16384 - inst->priorNonSpeechProb));
