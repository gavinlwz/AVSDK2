...
static const uint16_t kGenFuncTable[kGenFuncTableSize] = { 256,   485,   786,   1126,  1484,  1849,  2217,  2586,  2955,  3324,  3693,  4063,  4432,  4801,  5171,  5540,  5909,  6279,  6648,
                                                           7017,  7387,  7756,  8125,  8495,  8864,  9233,  9603,  9972,  10341, 10711, 11080, 11449, 11819, 12188, 12557, 12927, 13296, 13665,
                                                           14035, 14404, 14773, 15143, 15512, 15881, 16251, 16620, 16989, 17359, 17728, 18097, 18466, 18836, 19205, 19574, 19944, 20313, 20682,
                                                           21052, 21421, 21790, 22160, 22529, 22898, 23268, 23637, 24006, 24376, 24745, 25114, 25484, 25853, 26222, 26592, 26961, 27330, 27700,
                                                           28069, 28438, 28808, 29177, 29546, 29916, 30285, 30654, 31024, 31393, 31762, 32132, 32501, 32870, 33240, 33609, 33978, 34348, 34717,
                                                           35086, 35456, 35825, 36194, 36564, 36933, 37302, 37672, 38041, 38410, 38780, 39149, 39518, 39888, 40257, 40626, 40996, 41365, 41734,
                                                           42104, 42473, 42842, 43212, 43581, 43950, 44320, 44689, 45058, 45428, 45797, 46166, 46536, 46905 };
                                      ...
                                      int16_t analogTarget) // Q0
{
    ...
    tmp16no1 += YoumeWebRtcSpl_DivW32W16ResW16 (tmp32no1 + (kCompRatio >> 1), kCompRatio);
    maxGain = WEBRTC_SPL_MAX (tmp16no1, (analogTarget - targetLevelDbfs));
    tmp32no1 = maxGain * kCompRatio;
    zeroGainLvl = digCompGaindB;
    zeroGainLvl -= YoumeWebRtcSpl_DivW32W16ResW16 (tmp32no1 + ((kCompRatio - 1) >> 1), kCompRatio - 1);
    ...
    diffGain = YoumeWebRtcSpl_DivW32W16ResW16 (tmp32no1 + (kCompRatio >> 1), kCompRatio);
    ...
    limiterLvlX = analogTarget - limiterOffset;
    limiterIdx = 2 + YoumeWebRtcSpl_DivW32W16ResW16 ((int32_t)limiterLvlX << 13, kLog10_2 / 2);
    tmp16no1 = YoumeWebRtcSpl_DivW32W16ResW16 (limiterOffset + (kCompRatio >> 1), kCompRatio);
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        inLevel = YoumeWebRtcSpl_DivW32W16 (tmp32, kCompRatio);   // Q14
        ...
        tmpU32no1 += (uint32_t)kGenFuncTable[intPart] << 14;          // Q22, y=log2(1+e^x)
        logApprox = tmpU32no1 >> 8;                                   // Q14
        // Compensate for negative exponent using the relation:
        //  log2(1 + 2^-x) = log2(1 + 2^x) - x
        if (inLevel < 0)
        {
            zeros = YoumeWebRtcSpl_NormU32 (absInLevel);
            zerosScale = 0;
            if (zeros < 15)
            {
        ...
        {
            zeros = YoumeWebRtcSpl_NormW32 (numFIX);
        }
        else
        {
            zeros = YoumeWebRtcSpl_NormW32 (den) + 8;
        }
        ...
        if (limiterEnable && (i < limiterIdx))
        {
            ...
            y32 = YoumeWebRtcSpl_DivW32W16 (tmp32 + 10, 20);
        }
        if (y32 > 39000)
        {
...
int32_t WebRtcAgc_InitDigital (DigitalAgc *stt, int16_t agcMode)
{
    ...
    else
    {
        ...
        // 2017.3.30 降低慢跟踪包络的初值，使得对某些录音音量小的机型能更快收敛（追踪到）
        stt->capacitorSlow = 9448928; // (int32_t)(0.0088f * 32768.0f * 32768.0f);
    }
    stt->capacitorFast = 0;
    stt->gain = 65536;
    stt->gatePrevious = 0;
...
int32_t WebRtcAgc_ProcessDigital (DigitalAgc *stt, const int16_t* const* in_near, size_t num_bands, int16_t* const* out, uint32_t FS, int16_t lowlevelSignal)
{
    ...
    int32_t gains[11] = {0};       // Sub-frame(16 samples) target gain
    int32_t out_tmp, tmp32;
    int32_t env[10] = {0};         // The max amptitude(a^2) of each sub-frame
    int32_t max_nrg;
    int32_t cur_level;
  ...
  logratio = WebRtcAgc_ProcessVad(&stt->vadNearend, out[0], L * 10);
    ...
    // Account for far end VAD, because the record thread and play thread is not sync!!
    ...
    if (stt->agcMode != kAgcModeFixedDigital)
    {
        ...
        if (lowlevelSignal == 1) // Low level signal ,no decay in slow capacitor
        {
            decay = 0;
        }
    }
    ...
    // Actually, it is gain-smoothing processing
    gains[0] = stt->gain;
    for (k = 0; k < 10; k++) // 0~9
    {
        ...
        else
        {
            ...
            if (decay != 0) {
                stt->capacitorSlow = AGC_SCALEDIFF32 ((decay * 3) >> 2, stt->capacitorSlow, stt->capacitorSlow);
            }
        }

        // use maximum of both capacitors as current level
        cur_level = (stt->capacitorFast > stt->capacitorSlow) ? stt->capacitorFast : stt->capacitorSlow;

        // Translate signal level into gain, using a piecewise linear approximation
        // find number of leading zeros
        zeros = YoumeWebRtcSpl_NormU32 ((uint32_t)cur_level);
        if (cur_level == 0)
        {
            zeros = 31;
        }
        tmp32 = (cur_level << zeros) & 0x7FFFFFFF; // The MSB is cleared,
        frac = (int16_t)(tmp32 >> 18); // 13bit available bit in fraction number, it's base on the gap between the stt->gainTable[]
        tmp32 = (stt->gainTable[zeros - 1] - stt->gainTable[zeros]) * frac;
        gains[k + 1] = stt->gainTable[zeros] + (tmp32 >> 13); // Note: gainTable[] is the line gain, not dB gain.
#ifdef WEBRTC_AGC_DEBUG_DUMP
        if (k == 0)
        {
    ...
    //TSK_DEBUG_INFO("curlvl=%d, zero=%d, fastlvl=%d, slowlvl=%d", cur_level, zeros, stt->capacitorFast, stt->capacitorSlow);
    
    // Gate processing (lower gain during absence of speech)
    zeros = (zeros << 9) - (frac >> 3);
    // find number of leading zeros
    zeros_fast = YoumeWebRtcSpl_NormU32 ((uint32_t)stt->capacitorFast);
    ...
    zeros_fast <<= 9;
    zeros_fast -= (int16_t)(tmp32 >> 21); // Should be 21? 3+18=21
    ...
    for (k = 0; k < 10; k++)
    {
        ...
        {
            zeros = 16 - YoumeWebRtcSpl_NormW32 (gains[k + 1]);
        }
        gain32 = (gains[k + 1] >> zeros) + 1;
        gain32 *= gain32;
        // check for overflow
        while (AGC_MUL32 ((env[k] >> 12) + 1, gain32) > WEBRTC_SPL_SHIFT_W32 ((int32_t)32767, 2 * (1 - zeros + 10)))
        {
    ...
    delta = (gains[1] - gains[0]) << (4 - L2); // Q20
    gain32 = gains[0] << 4;                    // Q20
    // iterate over samples
    for (n = 0; n < L; n++)
    {
        for (i = 0; i < num_bands; ++i) {
            tmp32 = out[i][n] * ((gain32 + 127) >> 7); // Q13
            out_tmp = tmp32 >> 16; // Q-3
            if (out_tmp > 4095)
            {
    ...
    for (k = 1; k < 10; k++)
    {
        delta = (gains[k + 1] - gains[k]) << (4 - L2); // Q20
        gain32 = gains[k] << 4;                        // Q20
        // iterate over samples
        for (n = 0; n < L; n++)
        {
            for (i = 0; i < num_bands; ++i) {
                              ...
                              size_t nrSamples)  // (i) number of samples
{
    ...
    for (subfr = 0; subfr < 10; subfr++)
    {
        ...
        if (nrSamples == 160)
        {
            ...
            YoumeWebRtcSpl_DownsampleBy2 (buf1, 8, buf2, state->downState);
        }
        else
        {
            YoumeWebRtcSpl_DownsampleBy2 (in, 8, buf2, state->downState);
            in += 8;
        }
    ...
    state->stdShortTerm = (int16_t)YoumeWebRtcSpl_Sqrt (tmp32);

    // update long-term estimate of mean energy level (Q10)
    tmp32 = state->meanLongTerm * state->counter + dB;
    state->meanLongTerm = YoumeWebRtcSpl_DivW32W16ResW16 (tmp32, YoumeWebRtcSpl_AddSatW16 (state->counter, 1));

    // update long-term estimate of variance in energy level (Q8)
    tmp32 = (dB * dB) >> 12;
    tmp32 += state->varianceLongTerm * state->counter;
    state->varianceLongTerm = YoumeWebRtcSpl_DivW32W16 (tmp32, YoumeWebRtcSpl_AddSatW16 (state->counter, 1));

    // update long-term estimate of standard deviation in energy level (Q10)
    tmp32 = state->meanLongTerm * state->meanLongTerm;
    tmp32 = (state->varianceLongTerm << 12) - tmp32;
    state->stdLongTerm = (int16_t)YoumeWebRtcSpl_Sqrt (tmp32);
    ...
    tmp32 = YoumeWebRtcSpl_DivW32W16 (tmp32, state->stdLongTerm);
    tmpU16 = (13 << 12);
