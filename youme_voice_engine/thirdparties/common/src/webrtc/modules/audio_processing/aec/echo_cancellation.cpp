...
//  WebRtcAec_enable_extended_filter(aecpc->aec,1);
  if (!aecpc->aec) {
...
int32_t WebRtcAec_BufferFarend(void* aecInst,
  ...
  while (WebRtc_youme_available_read(aecpc->far_pre_buf) >= PART_LEN2) {
    // We have enough data to pass to the FFT, hence read PART_LEN2 samples.
    {
      float* ptmp = NULL;
      float tmp[PART_LEN2];
      WebRtc_youme_ReadBuffer(aecpc->far_pre_buf,
  ...
  if (aecpc->startup_phase) {
    ...
    if (aecpc->checkBuffSize) {
      ...
      if (aecpc->counter * nBlocks10ms >= 6) {
        ...
        // 这里其实是用75%的aecpc->msInSndCardBuf折算成64bank数作为aecpc->bufSizeStart
        aecpc->bufSizeStart =
