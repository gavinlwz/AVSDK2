...
void WebRtcIlbcfix_CbMemEnergy(
  ...
  pp=CB+lMem-lTarget;
  energy = YoumeWebRtcSpl_DotProductWithScale( pp, pp, lTarget, scale);

  /* Normalize the energy and store the number of shifts */
  energyShifts[0] = (int16_t)YoumeWebRtcSpl_NormW32(energy);
  ...
  /* Normalize the energy and store the number of shifts */
  energyShifts[base_size] = (int16_t)YoumeWebRtcSpl_NormW32(energy);
