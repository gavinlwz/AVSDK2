...
int WebRtcCng_Encode(CNG_enc_inst* cng_inst, int16_t* speech,
  ...
  outEnergy = YoumeWebRtcSpl_Energy(speechBuf, nrOfSamples, &outShifts);
  while (outShifts > 0) {
  ...
  outEnergy = YoumeWebRtcSpl_DivW32W16(outEnergy, (int16_t)factor);

  if (outEnergy > 1) {
    /* Create Hanning Window. */
    YoumeWebRtcSpl_GetHanningWindow(hanningW, nrOfSamples / 2);
    for (i = 0; i < (nrOfSamples / 2); i++)
      hanningW[nrOfSamples - i - 1] = hanningW[i];

    YoumeWebRtcSpl_ElementwiseVectorMult(speechBuf, hanningW, speechBuf, nrOfSamples,
                                    14);

    YoumeWebRtcSpl_AutoCorrelation(speechBuf, nrOfSamples, inst->enc_nrOfCoefs,
    ...
    stab = YoumeWebRtcSpl_LevinsonDurbin(corrVector, arCoefs, refCs,
                                    inst->enc_nrOfCoefs);
...
int16_t WebRtcCng_Generate(CNG_dec_inst* cng_inst, int16_t* outData,
  ...
  targetEnergy = YoumeWebRtcSpl_Sqrt(inst->dec_used_energy);

  En = (int16_t) YoumeWebRtcSpl_Sqrt(En) << 6;
  ...
  for (i = 0; i < nrOfSamples; i++) {
    excitation[i] = YoumeWebRtcSpl_RandN(&inst->dec_seed) >> 1;
  }

  /* Scale to correct energy. */
  YoumeWebRtcSpl_ScaleVector(excitation, excitation, inst->dec_used_scale_factor,
                        nrOfSamples, 13);
  ...
  YoumeWebRtcSpl_FilterAR(lpPoly, WEBRTC_CNG_MAX_LPC_ORDER + 1, excitation,
