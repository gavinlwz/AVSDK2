...
int WebRtcIlbcfix_InitEncode(  /* (o) Number of bytes encoded */
  ...
  YoumeWebRtcSpl_MemSetW16(iLBCenc_inst->anaMem, 0, LPC_FILTERORDER);
  ...
  YoumeWebRtcSpl_MemSetW16(iLBCenc_inst->lpc_buffer, 0, LPC_LOOKBACK + BLOCKL_MAX);

  /* Set the filter state of the HP filter to 0 */
  YoumeWebRtcSpl_MemSetW16(iLBCenc_inst->hpimemx, 0, 2);
  YoumeWebRtcSpl_MemSetW16(iLBCenc_inst->hpimemy, 0, 4);

#ifdef SPLIT_10MS
  /*Zeroing the past samples for 10msec Split*/
  YoumeWebRtcSpl_MemSetW16(iLBCenc_inst->past_samples,0,160);
