...
void WebRtcIlbcfix_Refiner(
  ...
  maxtemp = YoumeWebRtcSpl_MaxAbsValueW32(corrVecTemp, corrdim);

  scalefact = YoumeWebRtcSpl_GetSizeInBits(maxtemp) - 15;
  ...
  /* Find maximum */
  tloc = YoumeWebRtcSpl_MaxIndexW32(corrVecUps, ENH_UPS0 * corrdim);
  ...
  *updStartPos = searchSegStartPos * 4 + tloc + 4;
  ...
  if (ENH_FL0 > (searchSegStartPos + tloc2)) {
    ...
    YoumeWebRtcSpl_MemSetW16(vect, 0, st);
    ...
    if ((st + ENH_VECTL) > idatal) {
      ...
      YoumeWebRtcSpl_MemSetW16(&vect[ENH_VECTL - en], 0, en);
    } else {
  ...
  YoumeWebRtcSpl_FilterMAFastQ12(&vect[6], vect, filt, ENH_FLO_MULT2_PLUS1,
                            ENH_BLOCKL);

  /* Add the contribution from this vector (scaled with gain) to the total
     surround vector */
  YoumeWebRtcSpl_AddAffineVectorToVector(surround, vect, gain, 32768, 16,
                                    ENH_BLOCKL);
