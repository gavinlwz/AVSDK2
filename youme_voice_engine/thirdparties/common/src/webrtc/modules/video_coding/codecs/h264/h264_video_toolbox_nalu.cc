    ...
    namespace webrtc {
            ...
            *out_header = nullptr;
            ...
            if (is_keyframe) {
                ...
                for (size_t i = 0; i < param_set_count; ++i) {
                    ...
                    annexb_buffer->AppendData(reinterpret_cast<const char*>(param_set),param_set_size);
#else
                    if (i == 0) {
                        // sps
                        annexb_buffer->AppendData(kAnnexBHeaderBytes, sizeof(kAnnexBHeaderBytes));
                        
                        rtc::Optional<SpsParser::SpsState> spsState;
                        rtc::scoped_ptr<rtc::Buffer> sps_buffer(new rtc::Buffer());
                        SpsVuiRewriter::ParseResult result = SpsVuiRewriter::ParseAndRewriteSps(param_set+1, param_set_size-1, &spsState, sps_buffer.get());
                        if(result == SpsVuiRewriter::ParseResult::kVuiRewritten)
                        {
                            annexb_buffer->AppendData(reinterpret_cast<const char*>(param_set), 1);
                            annexb_buffer->AppendData(sps_buffer->data(), sps_buffer->size());
                            param_set_size = sps_buffer->size()+1;
                        }else {
                            annexb_buffer->AppendData(reinterpret_cast<const char*>(param_set),param_set_size);
                        }
                    } else {
        ...
        bool H264AnnexBBufferToCMSampleBuffer(const uint8_t* annexb_buffer,
            ...
            *out_sample_buffer = nullptr;
            ...
            int out_len = 0;
            AvccBufferWriter writer(reinterpret_cast<uint8_t*>(out_buffer),
                                    out_size);
            while (reader.BytesRemaining() > 0) {
                const uint8_t* nalu_data_ptr = nullptr;
                size_t nalu_data_size = 0;
                if (reader.ReadNalu(&nalu_data_ptr, &nalu_data_size)) {
                    writer.WriteNalu(nalu_data_ptr, nalu_data_size);
                    out_len += nalu_data_size + 4;
                }
            }
            
            CMBlockBufferRef block_buffer = nullptr;
//            OSStatus status = CMBlockBufferCreateWithMemoryBlock(
//                                                                 nullptr, out_buffer, out_len, nullptr, nullptr, 0,
//                                                                 reader.BytesRemaining(), kCMBlockBufferAssureMemoryNowFlag,
//                                                                 &block_buffer);
            OSStatus status = CMBlockBufferCreateWithMemoryBlock(NULL, out_buffer,
                                                        out_len,
                                                        kCFAllocatorNull,
                                                        NULL,
                                                        0,
            ...
            // Create sample buffer.
            const size_t sampleSize = out_len;
            ...
            CMBlockBufferRef block_buffer = nullptr;
            OSStatus status = CMBlockBufferCreateWithMemoryBlock(
                                                                 nullptr, nullptr, reader.BytesRemaining() +offset, nullptr, nullptr, 0,
                                                                 reader.BytesRemaining() +offset, kCMBlockBufferAssureMemoryNowFlag,
                                                                 &block_buffer);
            if (status != kCMBlockBufferNoErr) {
                LOG(LS_ERROR) << "Failed to create block buffer.";
                return false;
            ...
            RTC_DCHECK(block_buffer_size == reader.BytesRemaining() + offset);
        ...
        bool AnnexBBufferReader::ReadNalu(const uint8_t** out_nalu,
            ...
            *out_length = 0;
            ...
            uint8_t * current = (uint8_t *)&start_[offset_];
            if (current[0] == 0 && current[1] == 0 && current[2] == 0 &&
                     current[3] == 1) {
                 data_offset = offset_ + 4;
            }
            else if (current[0] == 0 && current[1] == 0 && current[2] == 1) {
                 data_offset = offset_ + 3;
            }
            else{
                 data_offset = offset_ + sizeof(kAnnexBHeaderBytes);
            }
        ...
        size_t AnnexBBufferReader::FindNextNaluHeader(const uint8_t* start,
            ...
            // NALUs are separated by an 00 00 00 01 or 00 00 01 header. Scan the byte stream
            ...
            while (current < end) {
                if (current[2] > 1) {
                  ...
                  else if (current[2] == 1 && current[1] == 0 && current[0] == 0) {
                        return current - start;
                } else {
                    ++current;
                }
