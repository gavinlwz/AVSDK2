...
size_t YoumeWebRtcSpl_AutoCorrelation(const int16_t* in_vector,
  ...
  smax = YoumeWebRtcSpl_MaxAbsValueW16(in_vector, in_vector_length);
  ...
  if (smax == 0) {
    ...
    int nbits = YoumeWebRtcSpl_GetSizeInBits((uint32_t)in_vector_length);
    // Number of bits to normalize smax.
    int t = YoumeWebRtcSpl_NormW32(WEBRTC_SPL_MUL(smax, smax));
