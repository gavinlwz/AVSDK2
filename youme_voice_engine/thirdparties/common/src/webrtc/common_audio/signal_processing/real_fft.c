...
struct RealFFT* YoumeWebRtcSpl_CreateRealFFT(int order) {
...
void YoumeWebRtcSpl_FreeRealFFT(struct RealFFT* self) {
  if (self != NULL) {
    free(self);
  }
}

// The C version FFT functions (i.e. YoumeWebRtcSpl_RealForwardFFT and
// YoumeWebRtcSpl_RealInverseFFT) are real-valued FFT wrappers for complex-valued
// FFT implementation in SPL.

int YoumeWebRtcSpl_RealForwardFFT(struct RealFFT* self,
  ...
  YoumeWebRtcSpl_ComplexBitReverse(complex_buffer, self->order);
  result = YoumeWebRtcSpl_ComplexFFT(complex_buffer, self->order, 1);
...
int YoumeWebRtcSpl_RealInverseFFT(struct RealFFT* self,
  ...
  YoumeWebRtcSpl_ComplexBitReverse(complex_buffer, self->order);
  result = YoumeWebRtcSpl_ComplexIFFT(complex_buffer, self->order, 1);
