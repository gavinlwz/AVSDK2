...
static size_t GetBufferReadRegions(RingBuffer* buf,
  ...
  const size_t readable_elements = WebRtc_youme_available_read(buf);
...
RingBuffer* WebRtc_youme_CreateBuffer(size_t element_count, size_t element_size) {
  ...
  WebRtc_youme_InitBuffer(self);

  return self;
}

void WebRtc_youme_InitBuffer(RingBuffer* self) {
  self->read_pos = 0;
  self->write_pos = 0;
...
void WebRtc_youme_FreeBuffer(void* handle) {
...
size_t WebRtc_youme_ReadBuffer(RingBuffer* self,
                         void** data_ptr,
                         void* data,
                         size_t element_count) {
...

  {
    ...
    const size_t read_count = GetBufferReadRegions(self,
                                                   ...
                                                   &buf_ptr_2,
                                                   &buf_ptr_bytes_2);
    //read_count=0表示没有可读取的数据。
    //此时buf_ptr_1指向无效数据，如果下面当做有效数据使用，可能会出现访问越界异常
    // if(read_count<=0)
    // {
    //     return 0;
    // }
    ...
    WebRtc_youme_MoveReadPtr(self, (int) read_count);

    return read_count;
  }
}

size_t WebRtc_youme_WriteBuffer(RingBuffer* self,
                          const void* data,
                          size_t element_count) {
  if (!self) {
    return 0;
  ...
  {
    const size_t free_elements = WebRtc_youme_available_write(self);
...
int WebRtc_youme_MoveReadPtr(RingBuffer* self, int element_count) {
  if (!self) {
    return 0;
  }

  {
    ...
    // instead of "size_t".
    const int free_elements = (int) WebRtc_youme_available_write(self);
    const int readable_elements = (int) WebRtc_youme_available_read(self);
...
size_t WebRtc_youme_available_read(const RingBuffer* self) {
  if (!self) {
    return 0;
  }
...
size_t WebRtc_youme_available_write(const RingBuffer* self) {
  if (!self) {
    return 0;
  }

  return self->element_count - WebRtc_youme_available_read(self);
}


int WebRtc_youme_MoveReadPtr_Back(RingBuffer* self, int element_count) {
...
size_t WebRtc_youme_get_write_pos(const RingBuffer* self) {
  if (!self) {
    return 0;
  }
...
size_t WebRtc_youme_get_element_count(const RingBuffer* self) {
  if (!self) {
    return 0;
  }
