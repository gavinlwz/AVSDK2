                      ...
                      int optmask)
{
  ...
  channel = (ares_channel)malloc(sizeof(struct ares_channeldata));
  if (!channel) {
    *channelptr = NULL;
    return ARES_ENOMEM;
  }
                      ...
                      int *optmask)
{
  ...
  if (channel->nservers) {
    ...
    if (ipv4_nservers) {
      options->servers = (struct in_addr*)malloc(ipv4_nservers * sizeof(struct in_addr));
      if (!options->servers)
        return ARES_ENOMEM;
  ...
  /* copy domains */
  if (channel->ndomains) {
    options->domains = (char**)malloc(channel->ndomains * sizeof(char *));
    if (!options->domains)
      return ARES_ENOMEM;
  ...
  /* copy sortlist */
  if (channel->nsort) {
    options->sortlist = (struct apattern*)malloc(channel->nsort * sizeof(struct apattern));
    if (!options->sortlist)
      return ARES_ENOMEM;
                           ...
                           int optmask)
{
  ...
  if ((optmask & ARES_OPT_SORTLIST) && (channel->nsort == -1) &&
      (options->nsort>0)) {
    channel->sortlist = (struct apattern*)malloc(options->nsort * sizeof(struct apattern));
    if (!channel->sortlist)
      return ARES_ENOMEM;
  ...
  for (p = str; p;)
    {
      ...
      newserv = (struct server_state*)realloc(*servers, (*nservers + 1) *
...
static int init_id_key(rc4_key* key,int key_data_len)
{
  ...
  key_data_ptr = (unsigned char*)calloc(1,key_data_len);
  if (!key_data_ptr)
    return ARES_ENOMEM;
