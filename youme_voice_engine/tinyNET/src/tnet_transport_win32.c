...
int tnet_transport_add_socket_2(const tnet_transport_handle_t *handle, tnet_fd_t fd, tnet_socket_type_t type, tsk_bool_t take_ownership, tsk_bool_t isClient, const char* dst_host, tnet_port_t dst_port)
{
...
void* TSK_STDCALL tnet_transport_mainthread(void *param)
{
	...
	while (TSK_RUNNABLE(transport)->running || TSK_RUNNABLE(transport)->started) {
		...
		if (networkEvents.lNetworkEvents & FD_READ) {
			...
			/* Alloc data */
			if (wsaBuffer.len > wsaBufferSize){
				if (!(wsaBuffer.buf = tsk_calloc(wsaBuffer.len, sizeof(uint8_t)))) {
					goto done;
				}
				wsaBufferSize = wsaBuffer.len;
			}
			...
			{
				dataBuffer = tsk_calloc(readCount, sizeof(uint8_t));
				memcpy(dataBuffer, wsaBuffer.buf, readCount);
...
bail:
	if (wsaBuffer.buf) TSK_FREE(wsaBuffer.buf);
