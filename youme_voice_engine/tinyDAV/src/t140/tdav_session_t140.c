...
static int tdav_session_t140_start (tmedia_session_t *self)
{
    ...
    if (base->rtp_manager)
    {
        ...
        pt = base->red.codec ? (base->red.codec->neg_format ? atoi (base->red.codec->neg_format) : atoi (base->red.codec->format)) : (t140->encoder.payload_type);
...
static int _tdav_session_t140_rtp_cb (const void *callback_data, const struct trtp_rtp_packet_s *packet)
{
    ...
    else
    {
        ...
        // Decode data
        tsk_bool_t bPLC = tsk_false;
        out_size = t140->decoder.codec->plugin->decode (t140->decoder.codec, packet->payload.data, packet->payload.size, &t140->decoder.buffer, &t140->decoder.buffer_size, packet->header, &bPLC);
        if (out_size)
        {
...
static tsk_size_t _tdav_session_t140_encap_red (tdav_session_t140_t *self, void **out_data_ptr, tsk_size_t *out_data_size, tsk_bool_t *last_data)
{
    ...
    *last_data = tsk_false;
    ...
    for (idx = 0; idx < T140_BLOCK_MAX_TRANSMIT_COUNT; ++idx)
    {
        if ((idx + 1) == T140_BLOCK_MAX_TRANSMIT_COUNT)
        {                                                                              // last header ?
            *((uint8_t *)&pout_data_ptr32[idx]) = (self->encoder.payload_type & 0x7F); // F=0 | PT
        }
        else
        {
            pout_data_ptr32[idx] = tnet_htonl (((0x80 | (self->encoder.payload_type & 0x7F)) << 24) |         // F=1 | PT
...
static int _tdav_session_t140_recv_red (tdav_session_t140_t *self, const struct trtp_rtp_packet_s *packet)
{
    ...
    for (i = 0, block_add = tsk_false; i < red_hdrs_count; ++i, block_add = tsk_false)
    {
        ...
        {
            t140_block_t *block = _tdav_session_t140_block_create (pt, seq_num, (packet->header->timestamp + timestamp_offset), tsk_false, pdata, block_length);
...
static int _tdav_session_t140_recv_raw (tdav_session_t140_t *self, const void *data_ptr, tsk_size_t data_size)
{
    tsk_size_t i = 0;
    tsk_size_t j = 0;
    tsk_size_t cmd_extra_len = 0;
    tsk_size_t utf8_start = 0;
    tsk_size_t utf8_end = 0;
    tsk_bool_t is_cmd = tsk_false;
    enum tmedia_t140_data_type_e cmd_type = 0;
...
static int _tdav_session_t140_timer_cb (const void *arg, tsk_timer_id_t timer_id)
{
    ...
    else if (timer_id == t140->encoder.timer_buffering.id)
    {
        ...
        t140->encoder.timer_buffering.id = last_data ? TSK_INVALID_TIMER_ID : tsk_timer_manager_schedule (t140->h_timer, t140->encoder.timer_buffering.timeout, _tdav_session_t140_timer_cb, t140);
    }
...
{
    TSK_DEBUG_ERROR ("@@ tdav_session_t140_ctor start");
    _tdav_session_video_init
...
/* plugin definition*/
static const tmedia_session_plugin_def_t tdav_session_t140_plugin_def_s = { &tdav_session_t140_def_s,
                                                                            ...
                                                                            /* T.140 part */
                                                                            { tdav_session_t140_set_ondata_cbfn, tdav_session_t140_send_data } };
...
static t140_block_t *_tdav_session_t140_block_create (uint8_t pt, uint16_t seq_num, uint32_t timestamp, tsk_bool_t delivered, const void *data_ptr, tsk_size_t data_size)
{
    ...
    if (data_ptr && data_size)
    {
        ...
        {
            TSK_DEBUG_ERROR ("Failed to alloc data with size = %zu", data_size);
            goto bail;
        }
