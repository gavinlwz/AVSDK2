...
static void *TSK_STDCALL _tdav_consumer_oss_playback_thread (void *param)
{
    ...
    tsk_size_t n_buffer_in_bytes =
    (p_oss->n_bits_per_sample == 8) ? p_oss->n_buff16_size_in_bytes : p_oss->n_buff_size_in_bytes;
    ...
    while (p_oss->b_started)
    {
        ...
        if (err >= 0)
        {
            ...
            {
                OSS_DEBUG_ERROR ("Failed to read data from audio interface failed (%d -> %s)", err,
                                 strerror (errno));
...
static int tdav_consumer_oss_prepare (tmedia_consumer_t *self, const tmedia_codec_t *codec)
{
    ...
    {
        OSS_DEBUG_ERROR ("ioctl(SOUND_PCM_WRITE_BITS, %d) failed: %d->%s", bits_per_sample, err,
                         strerror (errno));
        goto bail;
    }
    ...
    p_oss->n_buff_size_in_bytes =
    (TMEDIA_CONSUMER (p_oss)->audio.ptime * sample_rate * ((bits_per_sample >> 3) * channels)) / 1000;
    ...
    OSS_DEBUG_INFO (
    "prepared: req_bits_per_sample=%d; req_channels=%d; req_rate=%d, resp_bits_per_sample=%d; "
    "resp_channels=%d; resp_rate=%d /// n_buff_size_in_samples=%u;n_buff_size_in_bytes=%u",
    TMEDIA_CONSUMER (p_oss)->audio.bits_per_sample, TMEDIA_CONSUMER (p_oss)->audio.in.channels,
    TMEDIA_CONSUMER (p_oss)->audio.in.rate, bits_per_sample, channels, sample_rate,
...
/* plugin definition*/
static const tmedia_consumer_plugin_def_t tdav_consumer_oss_plugin_def_s = { &tdav_consumer_oss_def_s,
