...
int audio_opensles_instance_prepare_consumer (audio_opensles_instance_handle_t *_self, tmedia_consumer_t **_consumer)
{
    ...
    {
        TSK_DEBUG_ERROR ("SpeakerIsAvailable() failed with error code=%d", ret);
    }
    else
    {
        if (!_bool)
        {
            TSK_DEBUG_ERROR ("SpeakerIsAvailable() returned false");
        }
    ...
    {
        TSK_DEBUG_ERROR ("InitSpeaker() failed with error code=%d", ret);
    }
    else if ((ret = self->device->SetSpeakerOn (audio_consumer_opensles_is_speakerOn (consumer))))
    {
        TSK_DEBUG_ERROR ("SetSpeakerOn() failed with error code=%d", ret);
    }

    if ((ret = self->device->PlayoutIsAvailable (&_bool)))
    {
        TSK_DEBUG_ERROR ("PlayoutIsAvailable() failed with error code =%d", ret);
    }
    else
    {
        if (!_bool)
        {
            TSK_DEBUG_ERROR ("PlayoutIsAvailable() returned false");
        }
    ...
    {
        TSK_DEBUG_ERROR ("SetStereoPlayout(%d==2) failed with error code=%d", (*_consumer)->audio.in.channels, ret);
    }

    // if((ret = self->device->SetPlayoutBuffer(AudioDeviceModule::kFixedBufferSize, (*_consumer)->audio.ptime))){
    //	TSK_DEBUG_ERROR("SetPlayoutBuffer(%d ms) failed with error code=%d", (*_consumer)->audio.ptime, ret);
    //}
    // always request 10ms buffers
    if ((ret = self->device->SetPlayoutBuffer (10)))
    {
        TSK_DEBUG_ERROR ("SetPlayoutBuffer(%d ms) failed with error code=%d", 10, ret);
    }

    int playoutSampleRate = (*_consumer)->audio.out.rate;
    if ((ret = self->device->SetPlayoutSampleRate (playoutSampleRate)))
    {
        TSK_DEBUG_ERROR ("SetPlayoutSampleRate(%d) failed with error code=%d", playoutSampleRate, ret);
    }

    if ((ret = self->device->InitPlayout ()))
    {
        TSK_DEBUG_ERROR ("AudioDeviceModule::InitPlayout() failed with error code = %d", ret);
        goto done;
    }
    ...
    {
        TSK_DEBUG_ERROR ("StereoPlayout() failed with error code=%d", ret);
    }
    else
    {
    ...
    {
        TSK_DEBUG_ERROR ("PlayoutSampleRate() failed with error code=%d", ret);
    }
    else
    {
...
int audio_opensles_instance_prepare_producer (audio_opensles_instance_handle_t *_self, tmedia_producer_t **_producer)
{
    ...
    {
        TSK_DEBUG_ERROR ("RecordingIsAvailable() failed with error code =%d", ret);
    }
    else
    {
        if (!_bool)
        {
            TSK_DEBUG_ERROR ("RecordingIsAvailable() returned false");
        }
    ...
    {
        TSK_DEBUG_ERROR ("MicrophoneIsAvailable() failed with error code =%d", ret);
    }
    else
    {
        if (!_bool)
        {
            TSK_DEBUG_ERROR ("MicrophoneIsAvailable() returned false");
        }
        else
        {
            if ((ret = self->device->InitMicrophone ()))
            {
                TSK_DEBUG_ERROR ("InitMicrophone() failed with error code =%d", ret);
            }
        }
    }
    ...
    {
        TSK_DEBUG_ERROR ("SetStereoRecording(%d==2) failed with error code=%d", (*_producer)->audio.channels, ret);
    }

    int recordingSampleRate = (*_producer)->audio.rate;
    if ((ret = self->device->SetRecordingSampleRate (recordingSampleRate)))
    {
        TSK_DEBUG_ERROR ("SetRecordingSampleRate(%d) failed with error code=%d", recordingSampleRate, ret);
    }

    if ((ret = self->device->InitRecording ()))
    {
        TSK_DEBUG_ERROR ("AudioDeviceModule::InitRecording() failed with error code = %d", ret);
        goto done;
    }
    ...
    {
        TSK_DEBUG_ERROR ("StereoRecording() failed with error code=%d", ret);
    }
    else
    {
    ...
    {
        TSK_DEBUG_ERROR ("RecordingSampleRate() failed with error code=%d", ret);
    }
    else
    {
...
{
    audio_opensles_instance_t *self = (audio_opensles_instance_t *)_self;
    if (!self || !self->device || !pErrCode)
    {
        TSK_DEBUG_ERROR ("Invalid parameter");
        return -1;
    }
    return self->device->GetRecordingError (pErrCode);
}

int audio_opensles_instance_get_recording_error_extra (audio_opensles_instance_handle_t *_self, int32_t *pErrExtra)
{
    audio_opensles_instance_t *self = (audio_opensles_instance_t *)_self;
    if (!self || !self->device || !pErrExtra)
    {
        TSK_DEBUG_ERROR ("Invalid parameter");
        return -1;
    }
    return self->device->GetRecordingErrorExtra (pErrExtra);
}
