...
static void* TSK_STDCALL __playback_thread(void *param)
{
	...
	for(;;){
		//WaitForMultipleObjects 当2个event 同时触发时候，只会返回数组中索引最小的那个。
		//设计的时候应该将退出的event 放在数组的0 索引，否则就可能导致卡死。为了不修改之前的逻辑，所以加了个变量
...
static int tdav_consumer_waveapi_set_param(tmedia_consumer_t *_self, const tmedia_param_t *param)
{
	tdav_consumer_waveapi_t *self = (tdav_consumer_waveapi_t *)_self;

	if (tmedia_ppt_consumer == param->plugin_type)
	{
	}
...
static int tdav_consumer_waveapi_get_param(tmedia_consumer_t *_self, tmedia_param_t *param)
{
...
int tdav_consumer_waveapi_prepare(tmedia_consumer_t* self, const tmedia_codec_t* codec)
{
	...
	consumer->wfx.nSamplesPerSec = TMEDIA_CONSUMER(consumer)->audio.out.rate;
...
static const tmedia_consumer_plugin_def_t tdav_consumer_waveapi_plugin_def_s = 
{
	...
	tdav_consumer_waveapi_set_param,
	tdav_consumer_waveapi_get_param,
