...
static tsk_object_t *tdav_rscode_ctor (tsk_object_t *_self, va_list *app)
{
    ...
    if (self)
    {
        ...
        self->iMaxListTimeDelay = Config_GetInt("RS_DELAY", 300);
        self->iCurPopTimeDelay = 0;
        self->lastAjustDelayTime = 0;
        self->iGroupRecvCount = 0;
        self->iLastPopGroup = 0;
        self->rscodeBreak = tsk_false;
        self->lastAdjustTime = tsk_gettimeofday_ms();
        self->ilastHighLossTime = 0;
    ...
    if(RSC_TYPE_DECODE == self->rs_type) {
        ...
        if (self->iPacketGroupSeq != iPacketGroupSeq)
        {
            if (packet->header->csrc_count >= 5) {
                self->iLastVideoID = packet->header->csrc[4];
            } else {
                self->iLastVideoID = packet->header->csrc[3];
            }
...
//                printf("\n");
                ...
                if (iRecvDataPacket == self->iNPAR) {
                    ...
                    /*
                    if( (tmedia_get_video_nack_flag() & tdav_session_video_nack_half) && self->lastAjustDelayTime)
                    {
                        if( tsk_gettimeofday_ms()  - self->lastAjustDelayTime > 5000)
                        {
                            self->lastAjustDelayTime = 0;
                            int delay = Config_GetInt("RS_DELAY", 200);
                            self->iMaxListTimeDelay = self->iMaxListTimeDelay > delay ?   self->iMaxListTimeDelay - 50 : delay;
                        }
                    }
                     */
                    break;
                ...
                if (iLostPacket > self->iCheckCodeLen) {
                    ...
                    /*
                    self->lastAjustDelayTime = tsk_gettimeofday_ms();
                    self->iMaxListTimeDelay = Config_GetInt("RS_DELAY", 200) * 2; //丢包时缓冲时长加大一倍
                     */
                    TSK_DEBUG_INFO("rscode drop:%d %d groupseq:%d %d", iLostPacket, self->iCheckCodeLen, (int)self->iPacketGroupSeq, iPacketGroupSeq);
            ...
            if (self->iPacketGroupSeq < iPacketGroupSeq || (iPacketGroupSeq < 10)) {
                //当前组序号重新赋值
      ...
      */
        ...
        if (0 == self->iCurrentBufferIndex 
            && (currentTimeMs - self->lastAdjustTime > 2000)) { //2s调整一次
            ...
            int  iMaxRTT = Config_GetInt("data_rtt",0);
            if(iMaxRTT < 300)
            {
                maxLossLimit = 100;
            }
            ...
            //TSK_DEBUG_INFO("rscode send currentLossRate %d rtt: %d",currentLossRate, iMaxRTT);
...
static tsk_list_item_t* tdav_rscode_pop_packet_by_seq_num(tdav_rscode_t *self, int seq_num) {
    ...
    uint8_t videoId;
    uint64_t pktDiffTime = 0;
    int nack = (self->iCurPopTimeDelay > 0 ? tdav_session_video_nack_half : tdav_session_video_nack_none) & tmedia_get_video_nack_flag();

    if(self->in_rtp_packets_list && self->in_rtp_packets_list->tail){
        tsk_list_item_t* tailItem = self->in_rtp_packets_list->tail;
        pktDiffTime = ((trtp_rtp_packet_t*)tailItem->data)->header->timestampl - ((trtp_rtp_packet_t*)self->in_rtp_packets_list->head->data)->header->timestampl;
    ...
    do {
        ...
        if ( self->in_rtp_packets_list->head &&
            ( 
              ...
              || (nack && pktDiffTime > self->iCurPopTimeDelay)
              || (nack && (( diff > 7 ) || pktCount > 300))
              || (!nack && (( diff > 0 &&(diff > 1 || (++self->iGroupRecvCount) > 3)) || pktCount > 80))
             )
            ){
            ...
            if (((trtp_rtp_packet_t*)item->data)->header->csrc_count >= 5) {
               videoId = ((trtp_rtp_packet_t*)item->data)->header->csrc[4];
            } else {
               videoId = ((trtp_rtp_packet_t*)item->data)->header->csrc[3];
            }
            
            uint16_t headNum = ((trtp_rtp_packet_t*)item->data)->header->seq_num;
            int delta = seq_num - headNum;
            if(videoId ==  self->iLastVideoID && delta > 10 && headNum > 10 && delta < 200 ) {
                ...
                if ((csrcCount >= 6) && (((trtp_rtp_packet_t*)item->data)->header->csrc[2]!= 0)){
                    uint32_t redundancyFlag = ((trtp_rtp_packet_t*)item->data)->header->csrc[1];
                    uint8_t* pGrounNAP = (uint8_t*)(&((trtp_rtp_packet_t*)item->data)->header->csrc[2]);
                    uint16_t iPacketGroupSeq = pGrounNAP[0] << 8 | pGrounNAP[1];
                    if ((redundancyFlag && (self->iPacketGroupSeq > iPacketGroupSeq)) || (iPacketGroupSeq - self->iPacketGroupSeq > 10)) {
                        // 1、若是旧分组到达的包，丢弃冗余包，数据包往下传递
                        // 2、抖动到达的超前包全部丢弃
    ...
    /*
     * 根据接收丢包率和序列号连续性对数据包缓冲长度进行调整，如果rtt 超过150ms，就不考虑重传
     */
    if(item){
        if(((trtp_rtp_packet_t*)item->data)->header->seq_num == seq_num && Config_GetInt("video_dn_lossrate", -1 ) < 2 ){
            if(self->iCurPopTimeDelay > 29 && (((trtp_rtp_packet_t*)item->data)->header->timestampl - self->lastAjustDelayTime > 3000) )
            {
                //距离上次调节时间超过5s再调节
                self->lastAjustDelayTime = ((trtp_rtp_packet_t*)item->data)->header->timestampl;
                self->iCurPopTimeDelay -= 30;
            }
        }else if(self->iCurPopTimeDelay < self->iMaxListTimeDelay){
            if(Config_GetInt("dummy_rtt",0) < 90){
                self->lastAjustDelayTime = ((trtp_rtp_packet_t*)item->data)->header->timestampl;
                self->iCurPopTimeDelay += 30;
            }else{
                self->iCurPopTimeDelay = 0;
    ...
    TMEDIA_I_AM_ACTIVE(2000,"rscode PopTimeDelay %d",self->iCurPopTimeDelay);
    return item;
}
...
static void* TSK_STDCALL tdav_rscode_thread_func(void *arg) {
    ...
    TSK_DEBUG_INFO("video rscode thread enters");
        ...
        else
        {
            ...
            if (item != NULL) {
				...
				if (tmpPacket->header->seq_num > 65500)
				{
					...
					if (Count_StdSet(self->stdSetInstance) >= 600) {
						DeleteItem_StdSet(self->stdSetInstance,300);
					}
				}
...
//            }
    ...
    //TSK_OBJECT_SAFE_FREE(self->in_rtp_packets_list);
    TSK_DEBUG_INFO("video rscode thread exits");
    return tsk_null;
}
...
int tdav_rscode_start(tdav_rscode_t *self) {
    ...
    if(!self->thread_handle) {
        ...
        tsk_thread_set_priority(self->thread_handle, TSK_THREAD_PRIORITY_HIGH);
    }
...
int tdav_rscode_push_rtp_packet(tdav_rscode_t *self, trtp_rtp_packet_t *packet) {
    ...
    if(RSC_TYPE_ENCODE == self->rs_type || packet->header->seq_num < 200)
    {
        //序号翻转时，小的序列号放后边
