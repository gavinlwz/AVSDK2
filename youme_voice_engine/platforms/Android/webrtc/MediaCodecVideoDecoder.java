/*
 *  Copyright 2014 The WebRTC@AnyRTC project authors. All Rights Reserved.
 *
...
public class MediaCodecVideoDecoder {
  ...
  private static final long MAX_DECODE_TIME_MS = 300;
  ...
  private static final String[] supportedH264HwCodecPrefixes =
    {"OMX.qcom.", "OMX.Intel.", "OMX.Exynos.", "OMX.rk.", "OMX.k3.", "OMX.MTK.", "OMX.Nvidia.",
    "OMX.ittiam.", "OMX.SEC.", "OMX.IMG.", "OMX.hi.", "OMX.hisi.", "OMX.TI.DUCATI1.", "OMX.LG.",
    "OMX.allwinner.", "OMX.amlogic.avc.", "OMX.MARVELL.VIDEO.",
    "MX.Action.", "OMX.BRCM.", "OMX.brcm.", "OMX.cosmo.", "OMX.duos.", "OMX.hantro.", "OMX.MS.",
    "OMX.RENESAS.", "OMX.RTK.", "OMX.sprd.h264.", "OMX.ST.", "OMX.vpu.", "OMX.WMT.", "OMX.bluestacks."};
  ...
  private static Size supportedMinSize = new Size(64,64);
  private static Size supportedMaxSize = new Size(1080,1920);
  ...
  public static boolean isH264HwSupported() {
    if(Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
      ...
      boolean s = supports(H264_MIME_TYPE,false,MediaCodecInfo.CodecProfileLevel.AVCProfileHigh, MediaCodecInfo.CodecProfileLevel.AVCLevel31);
      Logging.d(TAG, "AVCLevel31 decode support:"+s);
      return s;
    }
  }
  ...
  public  int getFallbackSoftWare()
  {
	  int tmp = fallbackSoftWare;
	  fallbackSoftWare = 0;
	  Log.d(TAG, "hw_decode_enable hw_decode_enable:"+tmp);
	  return tmp;
  }
  ...
  @SuppressLint("NewApi") private static DecoderProperties findDecoder(
      String mime, String[] supportedCodecPrefixes) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
      return null; // MediaCodec.setParameters is missing.
    }
    try {
//    Log.d(TAG, "Trying to find HW decoder for mime " + mime);
    for (int i = 0; i < MediaCodecList.getCodecCount(); ++i) {
      ...
      for (String mimeType : info.getSupportedTypes()) {
    	  //Log.d(TAG, "mediacodec decode getSupportedTypes:" + mimeType);
...
//        Log.d(TAG, "   Color: 0x" + Integer.toHexString(colorFormat));
         ...
         if (videoCaps != null) 
    	  {
                ...
                Log.d(TAG, "mediadecode supportedMinSize w:"+ supportedMinSize.width + " h:"+supportedMinSize.height +
      ...
      for (int supportedColorFormat : supportedColorList) {
        for (int codecColorFormat : capabilities.colorFormats) {
          if (codecColorFormat == supportedColorFormat) {
        	...
        	String msg = "Found target decoder " + name +
     ...
     Log.d(TAG, "No HW decoder found for mime " + mime);
    }catch (Exception e) {
        Log.w(TAG, "Media decoder found failed", e);
     }
    NativeEngine.logcat(NativeEngine.LOG_INFO, TAG, "No HW decoder found for mime " + mime);
  ...
  private boolean initDecode(
    ...
    try {
      ...
      if (useSurface) {
    	if(surfaceTextureHelper.getSurfaceTexture() == null)
    	   return false;
      ...
      if (mediaCodec == null) {
        Log.d(TAG, "Can not create media decoder");
        return false;
      }
      ...
      if (useSurface && surface != null) {
    	   surface.release();
    	   surface = null;
    	   textureListener.release();
      }	
      Log.d(TAG, "initDecode failed", e);
      return false;
    }
  }
  ...
  private static class TextureListener
    ...
    private boolean updataImageError =false;
    ...
    @Override
    public void onTextureFrameError(){
    	updataImageError = true;
    }
    
    public boolean getError(){
    	return updataImageError;
    }
    ...
    while (true) {
      ...
      switch (result) {
        ...
        case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
          ...
          int w = Math.max(stride, sliceHeight);
          int h = Math.min(stride, sliceHeight);
          ...
          if(textureListener != null){
        	  textureListener.surfaceTextureHelper.setFrameSize(width, height);
          }
          break;
        ...
        default:
          ...
          if (decodeTimeMs > MAX_DECODE_TIME_MS) {
            NativeEngine.logcat(NativeEngine.LOG_WARN, TAG, "Very high decode time: " + decodeTimeMs + "ms"
  ...
  private DecodedTextureBuffer dequeueTextureBuffer(int dequeueTimeoutMs) {
    ...
    if(textureListener.getError()){
    	throw new IllegalStateException("dequeueTexture() egl error.");
    }
