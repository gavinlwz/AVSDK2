...
static const fmtp_size_t fmtp_sizes[] = 
{
    ...
    {"SQVGA", tmedia_pref_video_size_sqvga, tsk_false, 160, 120},
	{"SQCIF", tmedia_pref_video_size_sqcif, tsk_true, 128, 96},
    {"SSQVGA", tmedia_pref_video_size_ssqvga, tsk_false, 80, 60},
};
...
static const struct plugin_decl_s __plugin_def_types[] = {
    { tsk_plugin_def_type_consumer, (plugin_register)tmedia_consumer_plugin_register, (plugin_unregister)tmedia_consumer_plugin_unregister },
    { tsk_plugin_def_type_producer, (plugin_register)tmedia_producer_plugin_register, (plugin_unregister)tmedia_producer_plugin_unregister },
    { tsk_plugin_def_type_session, (plugin_register)tmedia_session_plugin_register, (plugin_unregister)tmedia_session_plugin_unregister },
    { tsk_plugin_def_type_codec, (plugin_register)tmedia_codec_plugin_register, (plugin_unregister)tmedia_codec_plugin_unregister },
	{ tsk_plugin_def_type_converter, (plugin_register)tmedia_converter_video_plugin_register, (plugin_unregister)tmedia_converter_video_plugin_unregister },
    { tsk_plugin_def_type_denoiser, (plugin_register)tmedia_denoise_plugin_register, (plugin_unregister)tmedia_denoise_plugin_unregister },
};
static const tsk_size_t __plugin_def_types_count =
sizeof (__plugin_def_types) / sizeof (__plugin_def_types[0]);
static const tsk_plugin_def_media_type_t __plugin_def_media_types[] = { tsk_plugin_def_media_type_audio,
                                                                        tsk_plugin_def_media_type_video,
                                                                        tsk_plugin_def_media_type_screencast };
static const tsk_size_t __plugin_def_media_types_count =
sizeof (__plugin_def_media_types) / sizeof (__plugin_def_media_types[0]);

static tsk_size_t _tmedia_plugin_register_or_unregister (struct tsk_plugin_s *plugin,
                                                         enum tsk_plugin_def_type_e type,
                                                         enum tsk_plugin_def_media_type_e media,
                                                         tsk_bool_t _register)
{
    ...
    for (i = 0; i < __plugin_def_types_count; ++i)
    {
        ...
        {
            if ((_register ? __plugin_def_types[i].fn_register : __plugin_def_types[i].fn_unregister) &&
                (type & __plugin_def_types[i].type) == __plugin_def_types[i].type &&
                (media & __plugin_def_media_types[j]) == __plugin_def_media_types[j])
            {
                for (index = 0;
                     (plugin_def_ptr_const = tsk_plugin_get_def_2 (plugin, __plugin_def_types[i].type,
                                                                   __plugin_def_media_types[j], index));
                     ++index)
                {
                    if ((_register ? __plugin_def_types[i].fn_register :
                                     __plugin_def_types[i].fn_unregister)(plugin_def_ptr_const) == 0)
                    {
                        ++ret;
                    }
                }
...
tsk_size_t tmedia_plugin_register (struct tsk_plugin_s *plugin,
                                   enum tsk_plugin_def_type_e type,
                                   enum tsk_plugin_def_media_type_e media)
{
...
tsk_size_t tmedia_plugin_unregister (struct tsk_plugin_s *plugin,
                                     enum tsk_plugin_def_type_e type,
                                     enum tsk_plugin_def_media_type_e media)
{
...
int tmedia_parse_video_imageattr(const char* imageattr, unsigned* in_width, unsigned* in_height, unsigned* out_width, unsigned* out_height)
{
...
char* tmedia_get_video_imageattr( unsigned in_width, unsigned in_height, unsigned out_width, unsigned out_height)
{
