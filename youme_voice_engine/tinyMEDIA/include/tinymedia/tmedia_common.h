...
#define ENABLE_SHARE 0x00000001     //只支持屏幕共享功能
#define ENABLE_SAVE  0x00000002     //只支持屏幕录制功能
...
typedef enum tmedia_type_e
{
    ...
    tmedia_t140 = (0x01 << 6),
    tmedia_bfcp = (0x01 << 7),
    tmedia_bfcp_audio = (0x01 << 8) /*must*/ | tmedia_bfcp, /* do not add "| audio". Otherwise it
                                                               will be impossible to start an
                                                               "video+bfcp-audio" session. */
    tmedia_bfcp_video = (0x01 << 9) /*must*/ | tmedia_bfcp, /* do not add "| video". Otherwise it
                                                               will be impossible to start an
                                                               "audio+bfcp-video" session. */
...
{ /* must be sorted like this */
    tmedia_pref_video_size_ssqvga, // 80 x 60 (0)
	tmedia_pref_video_size_sqcif, // 128 x 98 (1)
    tmedia_pref_video_size_sqvga, // 160 x 120(2)
	tmedia_pref_video_size_qcif, // 176 x 144 (3)
	tmedia_pref_video_size_qvga, // 320 x 240 (4)
	tmedia_pref_video_size_cif, // 352 x 288  (5)
...
typedef enum tmedia_chroma_e
{
    ...
    tmedia_chroma_rgb24,    // will be stored as bgr24 on x86 (little endians) machines; e.g.
                            // WindowsPhone7
    ...
    tmedia_chroma_uyvy422, // used by iOS4 producer (iPhone and iPod Touch 3G) - Microsoft:
                           // MFVideoFormat_YUY2
...
TINYMEDIA_API tsk_size_t tmedia_plugin_register (struct tsk_plugin_s *plugin,
                                                 enum tsk_plugin_def_type_e type,
                                                 enum tsk_plugin_def_media_type_e media);
TINYMEDIA_API tsk_size_t tmedia_plugin_unregister (struct tsk_plugin_s *plugin,
                                                   enum tsk_plugin_def_type_e type,
                                                   enum tsk_plugin_def_media_type_e media);
...
TINYMEDIA_API int tmedia_parse_video_imageattr(const char* imageattr, unsigned* in_width, unsigned* in_height, unsigned* out_width, unsigned* out_height);
TINYMEDIA_API char* tmedia_get_video_fmtp(tmedia_pref_video_size_t pref_vs);
TINYMEDIA_API char* tmedia_get_video_imageattr( unsigned in_width, unsigned in_height, unsigned out_width, unsigned out_height);
