...
- (NSArray*) getWindowList{
    ...
    NSMutableArray* winList = [NSMutableArray arrayWithCapacity:count];

    for( int i = 0; i < count; ++i ){
        YMWindowInfo* winInfo = [[YMWindowInfo alloc] init];
        winInfo.winID = arrayId[i];
        winInfo.winOwner = [NSString stringWithUTF8String:arrayOwner[i]];
        winInfo.winTitle = [NSString stringWithUTF8String:arrayWindow[i]];
        NSLog(@"wintitle: %s owner:%s", arrayOwner[i], arrayWindow[i]);
        [winList addObject:winInfo];
    }
