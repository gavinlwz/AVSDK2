...
{
    TSK_DEBUG_INFO("StatisticMonitering thread Enter");
    while( true ) {
        ...
        //搜集所需要的通知，为了避免死锁，先收集，再通知
        {
            ...
            for( auto iter = m_mapVideoBlock.begin(); iter !=  m_mapVideoBlock.end(); ++iter )
            {
                ...
                TSK_DEBUG_INFO("AVStatistic:video Block: %d _ %d", iter->first, iter->second );
            }
    ...
    TSK_DEBUG_INFO("StatisticMonitering thread exits");    
}
