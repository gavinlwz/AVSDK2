...
int youme_setVideoSmooth( int enable )
{
    return IYouMeVoiceEngine::getInstance()->setVideoSmooth(enable);
}
...
int youme_setShareExclusiveWnd( HWND windowid )
{
	return IYouMeVoiceEngine::getInstance()->setShareExclusiveWnd(windowid);;
}
...
int youme_setShareExclusiveWnd( int windowid )
{
    return IYouMeVoiceEngine::getInstance()->setShareExclusiveWnd( windowid );
}
...
void youme_getWinList(char* winList)
{
#ifdef WIN32 
    HWND arrayHwnd[100] = {0};
    char arrayName[100][512] = {0};
	...
	strcpy(winList, XStringToUTF8(jsonRoot.toSimpleString()).c_str());
	
#elif MAC_OS
    char arrayOwner[100][512] = {0};
    char arrayWindow[100][512] = {0};
    int  arrayId[100] = {0};
...
void youme_getScreenList (char *screenList)
{
#ifdef WIN32
    HWND arrayHwnd[100] = { 0 };
    char arrayName[100][512] = { 0 };
    ...
    // char *pName = arrayName;
    int count = 0;
    IYouMeVoiceEngine::getInstance ()->GetScreenInfoList (arrayHwnd, *arrayName, &count);
    ...
    {
        youmecommon::Value jsonWindowInfo;
        jsonWindowInfo["id"] = youmecommon::Value ((long)arrayHwnd[i]);
        jsonWindowInfo["name"] = youmecommon::Value (arrayName[i]);

        jsonRoot["screenlist"].append (jsonWindowInfo);
    }

    TSK_DEBUG_INFO ("screen count[%d] handle[%s]", count, XStringToUTF8 (jsonRoot.toSimpleString ()).c_str ());
    strcpy (screenList, XStringToUTF8 (jsonRoot.toSimpleString ()).c_str ());

#elif MAC_OS
    /* macos not support now
    char arrayOwner[100][512] = { 0 };
    char arrayWindow[100][512] = { 0 };
    int arrayId[100] = { 0 };
    ...
    int count = 0;
    //IYouMeVoiceEngine::getInstance ()->GetWindowInfoList (*arrayOwner, *arrayWindow, arrayId, &count);
    TSK_DEBUG_ERROR ("MAC is not support now!!");
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        jsonWindowInfo["id"] = youmecommon::Value (arrayId[i]);
        jsonWindowInfo["owner"] = youmecommon::Value (arrayOwner[i]);
        jsonWindowInfo["screen"] = youmecommon::Value (arrayWindow[i]);
