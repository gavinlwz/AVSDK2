...
void VideoRenderMgr::setRenderRotation(std::string& userId, int rotation) {
	TSK_DEBUG_INFO("@@ setRenderRotation user:%s, rotation:%d", userId.c_str(), rotation);
	std::lock_guard<std::mutex> l(m_mutex);
	if (!m_init)
		return;

	m_videoRenderWnd.setRenderRotation(userId.c_str(), rotation);
	//onSetRenderRotation(userId, rotation);
}
...
void VideoRenderMgr::onVideoFrameCallback(const char* userId, void * data, int len, int width, int height, int fmt, uint64_t timestamp){
	...
	{
		std::lock_guard<std::mutex> ldata(m_mutex_data);
...
bool VideoRenderMgr::onCreateRender(const char* userId, HWND  hWnd){
	std::lock_guard<std::mutex> l(m_mutex_data);
...
void VideoRenderMgr::onSetRenderRotation(const char* userId, int rotation) {
	std::lock_guard<std::mutex> l(m_mutex);
	if (!m_init)
		return;

	{
		...
		if (iter != m_mapRenders.end()){
			...
			SDL_Display_Set_Rotation(info->sdl, rotation);
		}
	}
}