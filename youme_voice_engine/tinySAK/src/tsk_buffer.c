...
int tsk_buffer_append (tsk_buffer_t *self, const void *data, tsk_size_t size)
{
    if (self && size)
    {
        ...
        if (oldsize)
        {
            self->data = tsk_realloc (self->data, newsize + 1);
        }
        else
        {
            self->data = tsk_calloc (size + 1, sizeof (uint8_t));
        }

        memset (TSK_BUFFER_TO_U8 (self) + oldsize, 0, size + 1);
...
int tsk_buffer_realloc (tsk_buffer_t *self, tsk_size_t size)
{
    if (self)
    {
        ...
        { // only realloc if different sizes
...
int tsk_buffer_remove (tsk_buffer_t *self, tsk_size_t position, tsk_size_t size)
{
    ...
    {
        if ((position == 0) && ((position + size) >= self->size))
        { /* Very common case. */
            return tsk_buffer_cleanup (self);
        }
        else if ((position + size) < self->size)
        {
            memcpy (((uint8_t *)self->data) + position, ((uint8_t *)self->data) + position + size, self->size - (position + size));
...
int tsk_buffer_insert (tsk_buffer_t *self, tsk_size_t position, const void *data, tsk_size_t size)
{
    if (self && size)
    {
        ...
        memmove (((uint8_t *)self->data) + position + size, ((uint8_t *)self->data) + position, tomove /*self->size - (position + size)*/);
