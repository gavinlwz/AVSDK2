...
//验证tokenV3
//#define     strJoinAppKey   @"YOUMEB16C6C9183F888FC210FBAB338DA6F04A19B93E5"
//#define     strAppKey       @"YOUMEB16C6C9183F888FC210FBAB338DA6F04A19B93E5"
//#define     strAppSecret    @"pHLyyX+I4UOTGF478IjJEdRtqh5K7MoRQZu6/4x6frAzuZA9ozWmuHX6C9bodBh/5NDOL1wWbQ7HUeLIbyBFt6Fs+zKj4YGbKGqE8fSHell5XsnebTRDyYZTFDIa1yxobNUdY7jVYH1masslVVvUY7WEFMtnOHnfzwSrbm8k+EEBAAE="
...
@property (nonatomic, assign) BOOL isSmoothMode;
@property (nonatomic, assign) int64_t blockTimes ;
@property (atomic,retain) NSArray<YMWindowInfo*> *winList;
@property (atomic,assign) int selectWinID;
...
- (void)viewDidLoad {
    ...
    [self comboBoxSetWinList].delegate = self;
    [self updateComboBoxValue];
    [self updateComboBoxWinListValue];
...
-(void) updateComboBoxWinListValue{
    // macos 10.15+ 需要先获取权限后才能获取窗口名称
    [[YMVoiceService getInstance] checkSharePermission];
    //初始化窗口列表comboBox
    self.winList = [[YMVoiceService getInstance] getWindowList];
    self.comboBoxSetWinList.delegate = nil;
    [[self comboBoxSetWinList] deselectItemAtIndex:[[self comboBoxSetWinList] indexOfSelectedItem]];
    [[self comboBoxSetWinList] removeAllItems];
    self.comboBoxSetWinList.delegate = self;
    
    for (int i = 0; i < self.winList.count; i++) {
        if(i == 0){
            self.selectWinID = [[self.winList objectAtIndex:i] winID];
        }
        [[self comboBoxSetWinList] addItemWithObjectValue: [NSString stringWithFormat:@"[%@] %@",[[self.winList objectAtIndex:i] winOwner], [[self.winList objectAtIndex:i] winTitle]]];
    }
    [[self comboBoxSetWinList] selectItemAtIndex:0];
...
-(void)YMSDKSetup
{
    ...
    if (bTokenV3) {
        [[YMVoiceService getInstance]setServerMode:SERVER_MODE_FIXED_IP_MCU];
        [[YMVoiceService getInstance]setServerIP:@"39.106.60.66" port:6006];
    }
...
- (void)createUI{
    ...
    if(self.isSmoothMode){
        self.smoothButton.title = @"smooth";
    }else{
        self.smoothButton.title = @"min delay";
    }
    //======     Demo的简单UI      =======
}
...
-(NSString*)sha1:(NSString*)srcStr
{
    const char *cstr = [srcStr cStringUsingEncoding:NSUTF8StringEncoding];
    NSData *data = [NSData dataWithBytes:cstr length:srcStr.length];
...
-(void)joinChannel
{
    ...
    if( params )
    {
        ...
        [[YMVoiceService getInstance] setVideoPreviewFps:30];
        [[YMVoiceService getInstance] setVideoFps:params->fps];
        [self.fpsLabel setStringValue:[NSString stringWithFormat:@"%dfps", params->fps]];
    ...
    if (self->bTokenV3) {
        ...
        NSString *timeStamp = [NSString stringWithFormat:@"%ld", curTime];
        NSString* sha1src = [NSString stringWithFormat:@"%@%@%@%@%@", strAppKey, strAppKey, m_roomid, self.currentUserId, timeStamp];
        NSString* token = [self sha1:sha1src];
        NSLog(@"bruce >>> sha1src=%@", sha1src);
        NSLog(@"bruce >>> token=%@", token);
        [[YMVoiceService getInstance] setTokenV3: token timeStamp:curTime];
    } else {
...
-(void)leaveChannel
{
    ...
    //退出频道后，重置标志位
    m_bShareOpen = m_bShareOpen ? false : m_bShareOpen;
    m_bSaveShareOpen = m_bSaveShareOpen ? false : m_bSaveShareOpen;
}
...
- (void)comboBoxSelectionDidChange:(NSNotification *)notification {
    ...
    }else if( notification.object == self.comboBoxSetWinList )
    {
        if ([[self comboBoxSetWinList] indexOfSelectedItem] >= 0 && self.winList) {
            NSString* selected = [self.comboBoxSetWinList objectValueOfSelectedItem];
            for (YMWindowInfo* winInfo in self.winList) {
                if([[NSString stringWithFormat:@"[%@] %@",[winInfo winOwner],[winInfo winTitle]] isEqualToString:selected]){
                    self.selectWinID = winInfo.winID;
                    break;
                }
            }
        }
...
- (void) updateButtonState
{
    ...
    if( m_bShareOpen )
    {
        ...
        [[self buttonOpenWindowShare ] setTitle: @"停止共享"];
    }
    else{
        [[self buttonOpenShare ] setTitle: @"开始共享"];
        [[self buttonOpenWindowShare ] setTitle: @"窗口共享"];
    }
...
- (IBAction)onClickButtonOpenShare:(id)sender {
    ...
    if(m_bShareOpen) {
        ...
        ret = YOUME_SUCCESS;
          ...
          [[YMVoiceService getInstance] setVideoFpsForShare:15];
        [[YMVoiceService getInstance] setVideoNetResolutionWidthForShare:1080 height:720];
        
        int windowID = self.view.window.windowNumber;
        printf("window id:%d\n", windowID );
        //2894
        [[YMVoiceService getInstance] setShareExclusiveWnd: windowID];
...
- (IBAction)startShareWindow:(id)sender {
    YouMeErrorCode ret;
    if(m_bShareOpen) {
        [[YMVoiceService getInstance] stopShare];
        ret = YOUME_SUCCESS;
    }else {
        ret = [[YMVoiceService getInstance] startShare:2 windowid:self.selectWinID];
    }
...
- (IBAction)onClickButtonPreviewMirror:(id)sender {
    ...
    if (m_bPreviewMirror) {
        [[YMVoiceService getInstance] setLocalVideoPreviewMirror:false];
    } else {
        [[YMVoiceService getInstance] setLocalVideoPreviewMirror:true];
    }
    m_bPreviewMirror = !m_bPreviewMirror;
}
...
//        NSString* dir = @"/Users/pinky/Downloads/";
        NSString* filePath = [documentPath stringByAppendingString:@"/screen.mp4"];
...
- (void) onAVStatistic:(YouMeAVStatisticType_t)type  userID:(NSString*)userID  value:(int) value {
    if(type == YOUME_AVS_VIDEO_FRAMERATE){
        NSLog(@"onAVStatistic userid: %@ fps: %d", userID, value);
    }
    
    if(type == YOUME_AVS_VIDEO_BLOCK){
        NSLog(@"onAVStatistic block userid: %lld", (self.blockTimes+= value));
    }
}
...
-(void)youmeEventJoinOKWithErrCode:(NSNumber*)iErrorCodeNum  roomid:(NSString *)roomid param:(NSString*)param {
    ...
    [[YMVoiceService getInstance] setPcmCallbackEnable: PcmCallbackFlag_Mix| PcmCallbackFlag_Remote | PcmCallbackFlag_Record  outputToSpeaker:true nOutputSampleRate:48000 nOutputChannel:2];
...
- (IBAction)setFPS:(id)sender {
    int fps = 15;
    if(sender == self.fps3Button)
    {
        fps = 3;
    }else if(sender == self.fps5Button){
        fps = 5;
    }else if(sender == self.fps10Button){
        fps = 10;
    }else if(sender == self.fps15Button){
        fps = 15;
    }else if(sender == self.fps20Button){
        fps = 20;
    }else if(sender == self.fps30Button){
        fps = 30;
    }else if(sender == self.fps60Button){
        fps = 60;
    }
    [[YMVoiceService getInstance] setVideoFps: fps];
    [[YMVoiceService getInstance] setVideoFpsForShare: fps];
    [self.fpsLabel setStringValue:[NSString stringWithFormat:@"%dfps", fps]];
}

- (IBAction)changeBitRate:(id)sender {
    int minBitrate = [self.minBitrateText.stringValue intValue];
    int maxBitrate = [self.maxBitrateText.stringValue intValue];
    [[YMVoiceService getInstance] setVideoCodeBitrate:maxBitrate minBitrate:minBitrate];
}

- (IBAction)changeSmoothMode:(id)sender {
    if(self.isSmoothMode){
        self.isSmoothMode = NO;
        self.smoothButton.title = @"min delay";
        [[YMVoiceService getInstance] setVideoSmooth:0];
    }else{
        self.isSmoothMode = YES;
        self.smoothButton.title = @"smooth";
        [[YMVoiceService getInstance] setVideoSmooth:1];
    }
}
